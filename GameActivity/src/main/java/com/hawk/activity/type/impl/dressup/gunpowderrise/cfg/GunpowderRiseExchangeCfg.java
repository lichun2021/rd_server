...
@HawkConfigManager.XmlResource(file = "activity/dress_smoke_again/dress_smoke_again_exchange.xml")
public class GunpowderRiseExchangeCfg extends HawkConfigBase {
	/**
	 * 唯一ID
	 */
	@Id
	...
	public GunpowderRiseExchangeCfg() {
		id = 0;
		pay = "";
		gain = "";
		exchangeCount = 0;
	...
	protected final boolean checkValid() {
		...
		if (!valid) {
			throw new InvalidParameterException(String.format("GunpowderRiseExchangeCfg reward error, id: %s , needItem: %s", id, pay));
		}
		valid = ConfigChecker.getDefaultChecker().checkAwardsValid(gain);
		if (!valid) {
			throw new InvalidParameterException(String.format("GunpowderRiseExchangeCfg reward error, id: %s , gainItem: %s", id, gain));
		}
