 ...
 * 用积分兑换道具的配置
 */
@HawkConfigManager.XmlResource(file = "activity/recovery_exchange/recovery_exchange_exchange.xml")
public class RecoveryExchangeExchangeCfg extends HawkConfigBase {
    /**
     * 唯一ID
     */
    @Id
    ...
    public RecoveryExchangeExchangeCfg() {
        id = 0;
        needItem = "";
        gainItem = "";
        times = 0;
    ...
    protected final boolean checkValid() {
        ...
        if (!valid) {
            throw new InvalidParameterException(String.format("RecoveryExchangeExchangeCfg needItem error, id: %s , needItem: %s", id, needItem));
        }
        valid = ConfigChecker.getDefaultChecker().checkAwardsValid(gainItem);
        if (!valid) {
            throw new InvalidParameterException(String.format("RecoveryExchangeExchangeCfg gainItem error, id: %s , gainItem: %s", id, gainItem));
        }
