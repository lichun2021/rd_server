...
public class BackToNewFlyOldTimeController extends PlayerComeBackTimeController {
    @Override
    public long getShowTimeByTermId(int termId, String playerId) {
        ...
        BackToNewFlyOldEntity entity = (BackToNewFlyOldEntity)optional.get();
    ...
    protected Optional<HawkDBEntity> getDBEntity(String playerId) {
        Optional<ActivityBase> optional = ActivityManager.getInstance().
                getActivity(Activity.ActivityType.BACK_TO_NEW_FLY_OLD_VALUE);
        if(!optional.isPresent()){
            return Optional.empty();
        }
        BackToNewFlyOldActivity activity = (BackToNewFlyOldActivity)optional.get();
