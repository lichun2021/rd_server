...
public class PowerUpBuildingParser extends AchieveParser<BattlePointChangeEvent> {

	@Override
	public AchieveType geAchieveType() {
		return AchieveType.POWER_UP_BUILDING;
	}
	...
	protected boolean updateAchieve(AchieveItem achieveData, AchieveConfig achieveConfig, BattlePointChangeEvent event) {
		...
		if(powerData.getBuildBattlePoint() == changeData.getBuildBattleChange()){
			HawkLog.logPrintln("PowerUpBuildingParser discard powerChange, playerId: {}, changeData: {}", event.getPlayerId(), changeData);
			return false;
		}
		if (changeData.getBuildBattleChange() > 0) {
			int val = achieveData.getValue(0) + changeData.getBuildBattleChange();
