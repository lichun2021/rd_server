...
public class NewlyExperienceTimeController extends PlayerComeBackTimeController {
	...
	public long getShowTimeByTermId(int termId, String playerId) {
		...
		NewlyExperienceEntity entity = (NewlyExperienceEntity)optional.get();
	...
	protected Optional<HawkDBEntity> getDBEntity(String playerId) {
		Optional<ActivityBase> optional = ActivityManager.getInstance().
				getActivity(Activity.ActivityType.NEWLY_EXPERIENCE_VALUE);
		if(!optional.isPresent()){
			return Optional.empty();
		}
		NewlyExperienceActivity activity = (NewlyExperienceActivity)optional.get();
		return activity.getPlayerDataEntity(playerId);
	}

	@Override
	public Class<? extends HawkConfigBase> getTimeCfgClass() {
		return NewlyExperienceTimeCfg.class;
	}
