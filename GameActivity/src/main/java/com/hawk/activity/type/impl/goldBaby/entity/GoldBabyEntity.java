package com.hawk.activity.type.impl.goldBaby.entity;
...
@Entity
@Table(name = "activity_gold_baby")
public class GoldBabyEntity extends HawkDBEntity implements IActivityDataEntity{
	 ...
	 */
	@Transient
	private List<GoldBabyRewardPool> poolList = new ArrayList<>();
	 ...
	 * -从数据库加载时会调用无参构造方法
	 */
    public GoldBabyEntity() {
    }
    
    public GoldBabyEntity (String playerId, int termId) {
	...
	public void addPool(GoldBabyRewardPool pool) {
		this.poolList.add(pool);
		notifyUpdate();
	}
	
	public void fillPoolList(List<GoldBabyRewardPool> poolList) {
		this.poolList.clear();
		this.poolList.addAll(poolList);
		notifyUpdate();
	}

	public List<GoldBabyRewardPool> getPoolList() {
		return poolList;
	}

	public void setPoolList(List<GoldBabyRewardPool> poolList) {
	...
	public void afterRead() {
		...
		List<GoldBabyRewardPool> pools = new ArrayList<>();
		for (String data : array) {
			pools.add(GoldBabyRewardPool.valueOf(data));
		}
	 ...
	 * @return
	 */
	public GoldBabyRewardPool getPoolById(int poolId){
		for(GoldBabyRewardPool pool: poolList){
	...
	public void resetDailyAchieve() {
		...
		for (AchieveItem achieveItem : achieveItemList){
			...
			GoldBabyDailyAchieveCfg cfg = HawkConfigManager.getInstance().getConfigByKey(GoldBabyDailyAchieveCfg.class, achieveItem.getAchieveId());
			if (cfg == null) {
				continue;
			}
