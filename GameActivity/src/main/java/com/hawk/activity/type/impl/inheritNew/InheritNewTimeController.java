...
public class InheritNewTimeController extends ITimeController {

	@Override
	public Class<? extends HawkConfigBase> getTimeCfgClass() {
		return InheritNewActivityTimeCfg.class;
	}
	...
	public int getActivityTermId(long now, String playerId) {
		BackNewPlayerInfo backInfo = ActivityManager.getInstance().getDataGeter().getBackPlayerInfoByIdNew(playerId);
		if (backInfo == null) {
			return 0;
		} else {
			InheritNewKVCfg cfg = HawkConfigManager.getInstance().getKVInstance(InheritNewKVCfg.class);
			...
			// 活动开启,且为回归老帐号/被传承者,有效期内展示
			if (currTermId != 0) {
				// 回归老帐号
				if (playerId.equals(backInfo.getCurOldPlayerId()) && currTime <= HawkTime.getAM0Date(new Date(backInfo.getBackTime())).getTime() + cfg.getValidTime()) {
					return currTermId;
				}
				// /被传承者
				else if (playerId.equals(backInfo.getCurrInheritPlayerId())
						&& currTime <= HawkTime.getAM0Date(new Date(backInfo.getInheritTime())).getTime() + cfg.getValidTime()) {
					return currTermId;
				}
			}
			return 0;
	...
	protected long getCurEndTime(long now, String playerId) {
		...
		if (backInfo == null) {
			...
			// 承接源
				...
				if (currTermId != 0 && currTermId == backInfo.getTermId()) {
					endTime = HawkTime.getAM0Date(new Date(backInfo.getInheritTime())).getTime() + cfg.getValidTime();
				}
			}
			// 老服回归角色
			else if(playerId.equals(backInfo.getCurOldPlayerId())){
				if (currTermId != 0 && currTermId == backInfo.getTermId()) {
					endTime = HawkTime.getAM0Date(new Date(backInfo.getBackTime())).getTime() + cfg.getValidTime();
				}
			}
		}
		return endTime;
