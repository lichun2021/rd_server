...
public class PowerUpTechParser extends AchieveParser<BattlePointChangeEvent> {

	@Override
	public AchieveType geAchieveType() {
		return AchieveType.POWER_UP_TECH;
	}
	...
	protected boolean updateAchieve(AchieveItem achieveData, AchieveConfig achieveConfig, BattlePointChangeEvent event) {
		...
		if(powerData.getBuildBattlePoint() == changeData.getBuildBattleChange()){
			HawkLog.logPrintln("PowerUpTechParser discard powerChange, playerId: {}, changeData: {}", event.getPlayerId(), changeData);
			return false;
		}
		if ((changeData.getTechBattleChange() + changeData.getPlantScienceBattlePoint()) > 0) {
			int val = achieveData.getValue(0) + changeData.getTechBattleChange() + changeData.getPlantScienceBattlePoint();
