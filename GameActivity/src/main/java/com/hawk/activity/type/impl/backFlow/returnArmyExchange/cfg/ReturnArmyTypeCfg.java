package com.hawk.activity.type.impl.backFlow.returnArmyExchange.cfg;
...
@HawkConfigManager.XmlResource(file = "activity/return_army_exchange/return_army_type.xml")
public class ReturnArmyTypeCfg extends HawkConfigBase {
	@Id
	...
	public boolean isAdapt(BackFlowPlayer bplayer) {
		...
		if (bplayer.getVipLevel() >= this.getVipStart() && bplayer.getVipLevel() <= this.getVipEnd() && lossDays >= this.getLossDaysStart() && lossDays <= this.getLossDaysEnd()) {
			return true;
		}
		return false;
	}
