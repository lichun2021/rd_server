...
import com.hawk.activity.type.impl.developFast.cfg.*;
import com.hawk.activity.type.impl.developFast.entity.DevelopFastEntity;
import com.hawk.activity.type.impl.developFast.entity.DevelopFastTask;
 ...
 */
public class DevelopFastActivity extends ActivityBase implements AchieveProvider {
     ...
     */
    public DevelopFastActivity(int activityId, ActivityEntity activityEntity) {
    ...
    public ActivityType getActivityType() {
        return ActivityType.DEVELOP_FAST;
    }
    ...
    public ActivityBase newInstance(ActivityCfg config, ActivityEntity activityEntity) {
        //创建活动实例
        DevelopFastActivity activity = new DevelopFastActivity(config.getActivityId(), activityEntity);
        //注册活动 用于接收成就任务数据
    ...
    protected HawkDBEntity loadFromDB(String playerId, int termId) {
        //根据条件从数据库中检索
        List<DevelopFastEntity> queryList = HawkDBManager.getInstance()
                .query("from DevelopFastEntity where playerId = ? and termId = ? and invalid = 0", playerId, termId);
        //如果有数据的话，返回第一个数据
        if (queryList != null && queryList.size() > 0) {
            DevelopFastEntity entity = queryList.get(0);
            return entity;
        }
        //否则返回空
        return null;
    ...
    protected HawkDBEntity createDataEntity(String playerId, int termId) {
        DevelopFastEntity entity = new DevelopFastEntity(playerId, termId);
        return entity;
    }
    ...
    public boolean isActivityClose(String playerId) {
        ...
        //活动活动总配置
        DevelopFastKVCfg cfg = HawkConfigManager.getInstance().getKVInstance(DevelopFastKVCfg.class);
        //如果开服时间早于配置时间
        if(openTime < cfg.getServerOpenTimeValue()){
            return true;
        }
        if(openTime > cfg.getServerEndOpenTimeValue()){
            return true;
        }
        return false;
    }
    ...
    public void onPlayerLogin(String playerId) {
        ...
        //获得玩家当期数据
        Optional<DevelopFastEntity> opEntity = getPlayerDataEntity(playerId);
        //不存在则返回
        if (!opEntity.isPresent()) {
            return;
        }
        //获得玩家当期数据
        DevelopFastEntity entity = opEntity.get();
        //如果自定义任务数据不存在就生成一下
        if(entity.getTaskList().isEmpty()){
            //获得自定义任务配置
            ConfigIterator<DevelopFastLevelCfg> iterator = HawkConfigManager.getInstance().getConfigIterator(DevelopFastLevelCfg.class);
            List<DevelopFastTask> list = new ArrayList<>();
            while (iterator.hasNext()){
                //构建自定义任务实例
                DevelopFastLevelCfg cfg = iterator.next();
                //添加进自定义任务列表中
                list.add(DevelopFastTask.valueOf(cfg.getId()));
            }
            //存数据
    ...
    public void syncActivityDataInfo(String playerId) {
        ...
        //同步给前端
        PlayerPushHelper.getInstance().pushToPlayer(playerId, HawkProtocol.valueOf(HP.code2.DEVELOP_FAST_SYNC, builder));
    }
    ...
    private void rewardSupplement(String playerId) {
        ...
        String spKey = playerId + ":DevelopFastSupplement:" + lastTermId;
        ...
        DevelopFastEntity entity = (DevelopFastEntity)dbEntity;
        Map<Integer,Integer> buyMap = entity.getBuyMap();
        List<Reward.RewardItem.Builder> list = new ArrayList<>();
        //遍历任务
        for(DevelopFastTask developFastTask : entity.getTaskList()) {
            if(developFastTask.getState() == Activity.DevelopFastTaskState.DF_NOT_RECEIVED_VALUE){
                DevelopFastLevelCfg cfg = HawkConfigManager.getInstance().getConfigByKey(DevelopFastLevelCfg.class, developFastTask.getTaskId());
                int buy = buyMap.getOrDefault(cfg.getType(), 0);
                if(buy == 0){
                    list.addAll(cfg.getRewardList());
                    developFastTask.setState(Activity.DevelopFastTaskState.DF_RECEIVED_VALUE);
                }else {
                    list.addAll(cfg.getRewardList());
                    list.addAll(cfg.getRewardList());
                    developFastTask.setState(Activity.DevelopFastTaskState.DF_RECEIVED_TWO_VALUE);
                }
            }else if(developFastTask.getState() == Activity.DevelopFastTaskState.DF_RECEIVED_VALUE){
                DevelopFastLevelCfg cfg = HawkConfigManager.getInstance().getConfigByKey(DevelopFastLevelCfg.class, developFastTask.getTaskId());
                int buy = buyMap.getOrDefault(cfg.getType(), 0);
                if(buy != 0){
                    list.addAll(cfg.getRewardList());
        ...
        for(AchieveItem achieveItem : entity.getItemList()) {
            ...
            DevelopFastAchieveCfg cfg = HawkConfigManager.getInstance().getConfigByKey(DevelopFastAchieveCfg.class, achieveItem.getAchieveId());
    ...
    private int getLastTermId() {
        ...
        DevelopFastTimeCfg lastCfg = null;
        List<DevelopFastTimeCfg> list = HawkConfigManager.getInstance().getConfigIterator(DevelopFastTimeCfg.class).toList();
        for(DevelopFastTimeCfg cfg : list){
    ...
    public Activity.DevelopFastTask.Builder makeBuilder(int type, DevelopFastEntity entity){
        //构建前端数据
        ...
        for(DevelopFastTask developFastTask : entity.getTaskList()){
            ...
            //添加数据
            task.addTasks(developFastTask.toPB());
        }
        //添加显示成就
     ...
     * @param level 养成线当前等级
     */
    public void fillAchieveIds(Activity.DevelopFastTask.Builder task, DevelopFastEntity entity, int type, int level){
        //获得全部成就
        ...
        //等级类型分类
        Map<Integer, Map<Integer, List<DevelopFastAchieveCfg>>> sortMap = new HashMap<>();
        //遍历成就
        for(AchieveItem item : itemList){
            //获得成就配置
            DevelopFastAchieveCfg cfg = HawkConfigManager.getInstance().getConfigByKey(DevelopFastAchieveCfg.class, item.getAchieveId());
            //过滤非当前养成线任务
        ...
        for(int i = 1; i <= level; i++){
            ...
            //获得当前等级type分类
            Map<Integer, List<DevelopFastAchieveCfg>> map = sortMap.get(i);
            //如果为空直接跳过
            if(map == null){
                continue;
            }
            ...
            for(int conditionType : map.keySet()){
                //排序
                List<DevelopFastAchieveCfg> list = map.get(conditionType);
                Collections.sort(list, new Comparator<DevelopFastAchieveCfg>() {
                    @Override
                    public int compare(DevelopFastAchieveCfg o1, DevelopFastAchieveCfg o2) {
                ...
                //按顺序显示
                for(DevelopFastAchieveCfg cfg : list){
    ...
    public boolean checkAuthBuy(String playerId, String payGiftId){
        ...
        //获得配置
        DevelopFastAuthorityCfg cfg = getPayCfg(id);
        if(cfg == null){
            return false;
        }
        //获得当期活动数据
     ...
     * @return 返回养成线礼包配置
     */
    public DevelopFastAuthorityCfg getPayCfg(int payGiftId){
        //养成线礼包配置
        ConfigIterator<DevelopFastAuthorityCfg> iterator = HawkConfigManager.getInstance().getConfigIterator(DevelopFastAuthorityCfg.class);
        //筛选出对应配置
        while (iterator.hasNext()){
            DevelopFastAuthorityCfg cfg = iterator.next();
     ...
     * @param entity 玩家活动数据
     */
    public void checkBuyAll(DevelopFastEntity entity){
        //如果数据为空直接返回
        if(entity == null){
            return;
        }
    ...
    public void onAuthBuyEvent(DevelopFastBuyEvent event){
        ...
        //获得当期活动数据
        Optional<DevelopFastEntity> opEntity = getPlayerDataEntity(event.getPlayerId());
        //如果没有就返回
        if (!opEntity.isPresent()) {
            return;
        }
    ...
    private List<Reward.RewardItem.Builder> getDoneAchieveReward(DevelopFastEntity entity, int type){
        List<Reward.RewardItem.Builder> list = new ArrayList<>();
    ...
    public void reachCondition(String playerId, int type, int value){
        ...
        for(DevelopFastTask developFastTask : entity.getTaskList()) {
            ...
            //过滤已经完成的任务
            if(developFastTask.getState() != Activity.DevelopFastTaskState.DF_NOT_REACH_VALUE){
                continue;
            }
            //过滤最高等级任务
            if(cfg.getConditionValue() == -1){
            ...
            if(score >= cfg.getConditionValue()){
                //设置任务状态为已完成
                developFastTask.setState(Activity.DevelopFastTaskState.DF_NOT_RECEIVED_VALUE);
            }
        }
        //入库
        entity.notifyUpdate();
    ...
    public void checkAchieveItem(DevelopFastEntity entity, int type, int level){
    ...
    public boolean isProviderNeedUpdate(String playerId, int achieveId) {
        DevelopFastAchieveCfg cfg = HawkConfigManager.getInstance().getConfigByKey(DevelopFastAchieveCfg.class, achieveId);
        if(cfg == null){
            return false;
        }
    ...
    private void initAchieve(String playerId) {
        ...
        //成就任务配置
        ConfigIterator<DevelopFastAchieveCfg> iterator = HawkConfigManager.getInstance().getConfigIterator(DevelopFastAchieveCfg.class);
        List<AchieveItem> list = new ArrayList<>();
        //构造成就任务数据
        while (iterator.hasNext()) {
            DevelopFastAchieveCfg cfg = iterator.next();
    ...
    public List<Reward.RewardItem.Builder> getRewardList(String playerId, AchieveConfig achieveConfig) {
        ...
        //获得成就对应养成线
        int type = ((DevelopFastAchieveCfg)achieveConfig).getCategory();
        //获得玩家当期数据
        Optional<DevelopFastEntity> opEntity = getPlayerDataEntity(playerId);
        //如果没有直接返回
        if (!opEntity.isPresent()) {
    ...
    public Result<Integer> award(String playerId, int taskId){
        ...
        //获得玩家数据
        DevelopFastEntity entity = opEntity.get();
        DevelopFastTask task = null;
        //遍历玩家任务
        for(DevelopFastTask developFastTask : entity.getTaskList()) {
            //筛选活动任务
            if(developFastTask.getTaskId() != taskId){
                continue;
            }else {
                task = developFastTask;
                break;
            }
        }
        //如果任务不存在返回错误码
        ...
        //获得配置
        DevelopFastLevelCfg cfg = HawkConfigManager.getInstance().getConfigByKey(DevelopFastLevelCfg.class, task.getTaskId());
        //如果配置不存在返回错误码
        if(cfg == null){
            return Result.fail(1);
        }
    ...
    public Result<Integer> buyInfo(String playerId, Activity.DevelopFastTaskType type){
        ...
        List<Reward.RewardItem.Builder> list = new ArrayList<>();
        //遍历任务
        ConfigIterator<DevelopFastLevelCfg> iterator1 = HawkConfigManager.getInstance().getConfigIterator(DevelopFastLevelCfg.class);
        while (iterator1.hasNext()){
            DevelopFastLevelCfg cfg = iterator1.next();
            if(cfg.getType() != type.getNumber()){
                continue;
            }
            list.addAll(cfg.getRewardList());
        }
        ConfigIterator<DevelopFastAchieveCfg> iterator2 = HawkConfigManager.getInstance().getConfigIterator(DevelopFastAchieveCfg.class);
        while (iterator2.hasNext()){
            DevelopFastAchieveCfg cfg = iterator2.next();
        ...
        //同步前端
        PlayerPushHelper.getInstance().pushToPlayer(playerId, HawkProtocol.valueOf(HP.code2.DEVELOP_FAST_BUY_RESP_VALUE, builder));
        //返回成功
        return Result.success();
    }
}
