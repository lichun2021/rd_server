...
@Entity
@Table(name = "activity_hidden_treasure")
public class HiddenTreasureEntity extends ActivityDataEntity {
	@Id
	...
	/** 开箱子总数  */
    @IndexProp(id = 11)
	@Column(name = "lottoryCount", nullable = false)
	private int lottoryCount;
	...
	@Transient
	private List<HiddenTreasureBox> nineBox = new ArrayList<>();

	@Transient
	private Map<Integer, Integer> exchangeNumMap = new ConcurrentHashMap<>();

	public HiddenTreasureEntity() {
	}

	public HiddenTreasureEntity(String playerId) {
		this.playerId = playerId;
		this.achieveItems = "";
		this.loginDays = "";
	}

	public HiddenTreasureEntity(String playerId, int termId) {
		this.playerId = playerId;
		this.termId = termId;
		this.achieveItems = "";
		this.loginDays = "";
	...
	private void loadNineBox() {
		...
		arr.forEach(str -> {
			HiddenTreasureBox crack = new HiddenTreasureBox();
	...
	public String nineBoxSerialize() {
		...
		nineBox.stream().map(HiddenTreasureBox::serializ).forEach(arr::add);
	...
	public List<HiddenTreasureBox> getNineBox() {
		return nineBox;
	}

	public void setNineBox(List<HiddenTreasureBox> nineBox) {
	...
	public void setLottoryCount(int lottoryCount) {
		this.lottoryCount = lottoryCount;
	}
