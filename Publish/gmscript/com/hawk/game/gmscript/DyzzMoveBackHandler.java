 ...
 * 
 * @author jm
 * localhost:port/script/DyzzMoveBack
 */
public class DyzzMoveBackHandler extends HawkScript {
	@Override
	...
	private String doClearImmigrationPlayer(String playerId) {
		...
		CrossService.getInstance().clearDyzzImmigrationPlayer(playerId, false);
	...
	private String doClearEmigrationStats(String playerId) {
		...
		if (HawkOSOperator.isEmptyString(playerId)) {
			for (Entry<String, String> entry : CrossService.getInstance().getEmigrationPlayers().entrySet()) {
				CrossService.getInstance().clearDyzzEmigrationPlayer(entry.getKey());
				strList.add(entry.getKey());
			}
		} else {
			CrossService.getInstance().clearDyzzEmigrationPlayer(playerId);
	...
	private String doClearImmigrationStats(String playerId) {
		...
		if (HawkOSOperator.isEmptyString(playerId)) {
			for (Entry<String, String> entry : CrossService.getInstance().getImmigrationPlayers().entrySet()) {
				CrossService.getInstance().clearDyzzImmigrationPlayer(entry.getKey());
				strList.add(entry.getKey());
			}
		} else {
			CrossService.getInstance().clearDyzzImmigrationPlayer(playerId);
			strList.add(playerId);
		}  
	...
	public HawkTuple3<String, Integer, String> moveBackPlayer(String playerId) {
		...
		label: 
		{
			try {
				...
				GsApp.getInstance().postMsg(player.getXid(), new DYZZPrepareMoveBackMsg());
