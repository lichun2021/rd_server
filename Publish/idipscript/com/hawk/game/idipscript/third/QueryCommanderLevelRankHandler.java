 ...
 * 指挥官等级榜单查询
 *
 * localhost:8080/script/idip/4183
 * 
 * @author lating
 */
@HawkScript.Declare(id = "idip/4183")
public class QueryCommanderLevelRankHandler extends IdipScriptHandler {
	
	@Override
	protected IdipResult onRequest(JSONObject request, HawkScriptHttpInfo httpInfo) {
		IdipResult result = IdipResult.fromRequest(request);
		
		Set<Tuple> rankSet = LocalRedis.getInstance().getRankList(RankType.PLAYER_GRADE_KEY, IdipUtil.RANK_RECORD_COUNT);
		...
		for (Tuple tuple : rankSet) {
			...
			dataJson.put("Lv", playerData.getPlayerBaseEntity().getLevel());                // 指挥官等级
			array.add(dataJson);
		}
