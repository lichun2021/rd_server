...
import com.hawk.game.msg.xqhx.XQHXPrepareMoveBackMsg;
...
public class XqhxMoveBackHandler extends HawkScript {
    @Override
    ...
    private String doClearImmigrationPlayer(String playerId) {
        ...
        CrossService.getInstance().clearXqhxImmigrationPlayer(playerId, false);
    ...
    private String doClearEmigrationStats(String playerId) {
        ...
        if (HawkOSOperator.isEmptyString(playerId)) {
            for (Map.Entry<String, String> entry : CrossService.getInstance().getEmigrationPlayers().entrySet()) {
                CrossService.getInstance().clearXqhxEmigrationPlayer(entry.getKey());
                strList.add(entry.getKey());
            }
        } else {
            CrossService.getInstance().clearXqhxEmigrationPlayer(playerId);
    ...
    private String doClearImmigrationStats(String playerId) {
        List<String> strList = new ArrayList<>();
        if (HawkOSOperator.isEmptyString(playerId)) {
            for (Map.Entry<String, String> entry : CrossService.getInstance().getImmigrationPlayers().entrySet()) {
                CrossService.getInstance().clearXqhxImmigrationPlayer(entry.getKey());
                strList.add(entry.getKey());
            }
        } else {
            CrossService.getInstance().clearXqhxImmigrationPlayer(playerId);
            strList.add(playerId);
        }
    ...
    public HawkTuple3<String, Integer, String> moveBackPlayer(String playerId) {
        int errorCode = Script.ScriptError.SUCCESS_OK_VALUE;
        String msg = "ok";
        label:
        {
            try {
                ...
                if (player == null) {
                    errorCode = Script.ScriptError.ACCOUNT_NOT_EXIST_VALUE;
                ...
                if (HawkOSOperator.isEmptyString(toServer)) {
                    errorCode = Script.ScriptError.NOT_IN_CROSS_VALUE;
                    msg = "player not in cross";

                    break label;
                }

                GsApp.getInstance().postMsg(player.getXid(), new XQHXPrepareMoveBackMsg());

            } catch (Exception e) {
                HawkException.catchException(e);

                errorCode = Script.ScriptError.EXCEPTION_VALUE;
                msg = "exception";
