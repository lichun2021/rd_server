...
import com.hawk.game.msg.xhjz.XHJZPrepareMoveBackMsg;
...
public class XhjzMoveBackHandler extends HawkScript {
    @Override
    ...
    private String doClearImmigrationPlayer(String playerId) {
        ...
        CrossService.getInstance().clearXhjzImmigrationPlayer(playerId, false);
    ...
    private String doClearEmigrationStats(String playerId) {
        ...
        if (HawkOSOperator.isEmptyString(playerId)) {
            for (Map.Entry<String, String> entry : CrossService.getInstance().getEmigrationPlayers().entrySet()) {
                CrossService.getInstance().clearXhjzEmigrationPlayer(entry.getKey());
                strList.add(entry.getKey());
            }
        } else {
            CrossService.getInstance().clearXhjzEmigrationPlayer(playerId);
    ...
    private String doClearImmigrationStats(String playerId) {
        ...
        if (HawkOSOperator.isEmptyString(playerId)) {
            for (Map.Entry<String, String> entry : CrossService.getInstance().getImmigrationPlayers().entrySet()) {
                CrossService.getInstance().clearXhjzImmigrationPlayer(entry.getKey());
                strList.add(entry.getKey());
            }
        } else {
            CrossService.getInstance().clearXhjzImmigrationPlayer(playerId);
            strList.add(playerId);
        }
    ...
    public HawkTuple3<String, Integer, String> moveBackPlayer(String playerId) {
        ...
        label:
        {
            try {
                ...
                GsApp.getInstance().postMsg(player.getXid(), new XHJZPrepareMoveBackMsg());
