 ...
 * 
 * @author jm
 * localhost:port/script/YqzzMoveBack
 */
public class YqzzMoveBackHandler extends HawkScript {
	@Override
	...
	private String doClearImmigrationPlayer(String playerId) {
		...
		CrossService.getInstance().clearYqzzImmigrationPlayer(playerId, false);
	...
	private String doClearEmigrationStats(String playerId) {
		...
		if (HawkOSOperator.isEmptyString(playerId)) {
			for (Entry<String, String> entry : CrossService.getInstance().getEmigrationPlayers().entrySet()) {
				CrossService.getInstance().clearYqzzEmigrationPlayer(entry.getKey());
				strList.add(entry.getKey());
			}
		} else {
			CrossService.getInstance().clearYqzzEmigrationPlayer(playerId);
	...
	private String doClearImmigrationStats(String playerId) {
		...
		if (HawkOSOperator.isEmptyString(playerId)) {
			for (Entry<String, String> entry : CrossService.getInstance().getImmigrationPlayers().entrySet()) {
				CrossService.getInstance().clearYqzzImmigrationPlayer(entry.getKey());
				strList.add(entry.getKey());
			}
		} else {
			CrossService.getInstance().clearYqzzImmigrationPlayer(playerId);
			strList.add(playerId);
		}  
	...
	public HawkTuple3<String, Integer, String> moveBackPlayer(String playerId) {
		...
		label: 
		{
			try {
				...
				GsApp.getInstance().postMsg(player.getXid(), new YQZZPrepareMoveBackMsg());
