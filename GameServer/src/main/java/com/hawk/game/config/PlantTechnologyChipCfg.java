 ...
 *
 */
@HawkConfigManager.XmlResource(file = "xml/plant_technology_chip.xml")
public class PlantTechnologyChipCfg extends HawkConfigBase {
	...
	protected boolean checkValid() {
		if (frontStage != 0) {
			PlantTechnologyChipCfg cfg = HawkConfigManager.getInstance().getConfigByKey(PlantTechnologyChipCfg.class, frontStage);
			if (cfg == null) {
		...
		if (postStage != 0) {
			PlantTechnologyChipCfg cfg = HawkConfigManager.getInstance().getConfigByKey(PlantTechnologyChipCfg.class, postStage);
			if (cfg == null) {
