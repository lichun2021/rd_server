...
public class XHJZWarMatchWaitState implements IXHJZWarState {
    @Override
    public void enter() {
        XHJZWarService.getInstance().onMatchWait();
    }
    ...
    public XHJZWar.XWStateInfo.Builder getStateInfo(Player player) {
        ...
        stateInfo.setState(XHJZWar.XWState.XW_MATCH_WAIT);
        XHJZWarTimeCfg cfg = XHJZWarService.getInstance().getTimeCfg();
        if(cfg != null){
