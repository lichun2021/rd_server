package com.hawk.game.service.tblyTeam.state.impl.main;

import com.hawk.game.config.TiberiumTimeCfg;
import com.hawk.game.player.Player;
import com.hawk.game.protocol.TiberiumWar.*;
import com.hawk.game.service.tblyTeam.TBLYWarService;
import com.hawk.game.service.tblyTeam.state.ITBLYWarState;
import org.hawk.os.HawkTime;

import java.util.concurrent.TimeUnit;

public class TBLYWarSignupState implements ITBLYWarState {
    @Override
    public void enter() {
        TBLYWarService.getInstance().onSignup();
    }

    @Override
    public void tick() {
        TiberiumTimeCfg cfg = TBLYWarService.getInstance().getTimeCfg();
        if(cfg != null) {
            long now = HawkTime.getMillisecond();
            if (now >= cfg.getSignEndTimeValue()) {
                TBLYWarService.getInstance().toNext();
            }
        }
    }

    @Override
    public void leave() {

    }

    @Override
    public TWStateInfo.Builder getStateInfo(Player player) {
        TWStateInfo.Builder builder = TWStateInfo.newBuilder();
        builder.setState(TWState.SIGN_UP);
        TiberiumTimeCfg cfg = TBLYWarService.getInstance().getTimeCfg();
        if (cfg != null) {
            builder.setSignStartTime(cfg.getSignStartTimeValue());
            builder.setSignEndTime(cfg.getSignEndTimeValue());
            builder.setWarStartTime(cfg.getWarStartTimeValue());
        }else {
            builder.setSignStartTime(HawkTime.getMillisecond());
            builder.setSignEndTime(HawkTime.getMillisecond() + TimeUnit.DAYS.toMillis(1));
            builder.setWarStartTime(HawkTime.getMillisecond() + TimeUnit.DAYS.toMillis(2));
        }
        return builder;
    }
}
