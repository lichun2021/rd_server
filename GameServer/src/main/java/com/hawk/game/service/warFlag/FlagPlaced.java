 ...
 *
 */
public class FlagPlaced implements FlagState {

	private long lastBuildLifeAdd = 0L;
	
	public IFlag flag;
	
	public FlagPlaced(IFlag flag) {
		this.flag = flag;
	}
	
	@Override
	public void stateTick() {
		if (lastBuildLifeAdd == 0L) {
			lastBuildLifeAdd = HawkTime.getMillisecond();
			return;
		}
		
		// tick周期
		if (HawkTime.getMillisecond() - lastBuildLifeAdd < WarFlagConstProperty.getInstance().getAutoLifeAddPeroid()) {
			return;
		}
		lastBuildLifeAdd = HawkTime.getMillisecond();
		
		flag.setLife(flag.getLife() + WarFlagConstProperty.getInstance().getAutoLifeAdd());
	...
	public void marchReach(String guildId) {
		...
		if (!marchs.isEmpty()) {
			...
			flag.setState(FlageState.FLAG_BUILDING_VALUE);
