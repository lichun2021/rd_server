...
public class XHJZWarMatchEndState implements IXHJZWarState {
    @Override
    public void enter() {
        XHJZWarService.getInstance().onMatchEnd();
    }
    ...
    public XHJZWar.XWStateInfo.Builder getStateInfo(Player player) {
        ...
        stateInfo.setState(XHJZWar.XWState.XW_MATCH);
        XHJZWarTimeCfg cfg = XHJZWarService.getInstance().getTimeCfg();
        if(cfg != null){
