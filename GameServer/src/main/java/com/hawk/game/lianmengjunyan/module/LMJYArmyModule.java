...
public class LMJYArmyModule extends ILMJYBattleRoomModule {

	public LMJYArmyModule(LMJYBattleRoom appObj) {
		super(appObj);
	}

	/** 训练士兵 */
	@ProtocolHandler(code = HP.code.ADD_SOLDIER_C_VALUE)
	private boolean onCreateSoldier(LMJYProtocol protocol) {
		...
		if (success) {
			...
			LocalRedis.getInstance().lmjyIncreaseCreateSoldier(player.getId(), armyId, soldierCount);
		}
	...
	private boolean onCureSoldier(LMJYProtocol protocol) {
		ILMJYPlayer player = protocol.getPlayer();
		if (battleRoom.maShangOver()) {
			player.responseSuccess(protocol.getType());
			return true;
	...
	private QueueEntity addQueue(ILMJYPlayer player, int queueType, int queueStatus, String itemId,
	...
	private List<ItemInfo> cureConsume(ILMJYPlayer player, List<ArmySoldierPB> cureList, int recoverTime, boolean immediate, boolean useGold, int hpCode) {
	 ...
	 * @param immediate
	 * @return */
	private Map<Integer, Integer> cureSoldier(ILMJYPlayer player, List<ArmySoldierPB> cureList, boolean immediate) {
	...
	private boolean onCurePlantSoldier(LMJYProtocol protocol) {
		ILMJYPlayer player = protocol.getPlayer();
		if (battleRoom.maShangOver()) {
			player.responseSuccess(protocol.getType());
			return true;
