 ...
 * @author lwt
 * @date 2019年1月2日 */
public class LMJYMassSingleMarch extends ILMJYMassMarch implements ILMJYReportPushMarch {

	public LMJYMassSingleMarch(ILMJYPlayer parent) {
		super(parent);
	}
	...
	public void onMarchReach(Player parent) {
		// 行军
		LMJYMarchEntity atkLeaderMarch = getMarchEntity();
		Set<ILMJYWorldMarch> massJoinMarchs = getMassJoinMarchs(true);
		// 目标点
		ILMJYWorldPoint targetPoint = getParent().getParent().getWorldPoint(getMarchEntity().getTerminalX(), getMarchEntity().getTerminalY()).orElse(null);
		if (Objects.isNull(targetPoint)) {// 烧出去了
			onMarchReturn(atkLeaderMarch.getTerminalId(), getParent().getPointId(), this.getArmys());
			// 队员行军返回
			for (ILMJYWorldMarch tmpMarch : massJoinMarchs) {
				tmpMarch.onMarchReturn(atkLeaderMarch.getTerminalId(), tmpMarch.getParent().getPointId(), tmpMarch.getArmys());
			}
			return;
		}
		// 防守方玩家
		ILMJYPlayer defplayer = (ILMJYPlayer) targetPoint;
		...
		// 填充参与集结信息
		for (ILMJYWorldMarch massJoinMarch : massJoinMarchs) {
		...
		// 防守方援军
		List<ILMJYWorldMarch> helpMarchList = defplayer.assisReachMarches();
		for (ILMJYWorldMarch iWorldMarch : helpMarchList) {
		...
		battleIncome.getBattle().setDuntype(DungeonMailType.LMJY);
		// 战斗数据输出
		BattleOutcome battleOutcome = BattleService.getInstance().doBattle(battleIncome);
		battleOutcome.setDuntype(DungeonMailType.LMJY);
		...
		// 防御者战后部队结算
		ArmyService.getInstance().onArmyBack(defplayer, battleOutcome.getBattleArmyMapDef().get(defplayer.getId()), Collections.emptyList(), 0, null);
	...
	public GuildWarTeamInfo.Builder getGuildWarPassivityInfo() {
		...
		List<ILMJYWorldMarch> assistandMarchs = getParent().getParent().getPointMarches(
				leader.getPointId(),
	...
	public boolean isEvident() {
		return ILMJYReportPushMarch.super.isEvident() || this.getMarchEntity().getMarchStatus() == WorldMarchStatus.MARCH_STATUS_WAITING_VALUE;
	}
