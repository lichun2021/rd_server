...
/** 集结行军 */
public abstract class ILMJYMassMarch extends ILMJYWorldMarch {

	public ILMJYMassMarch(ILMJYPlayer parent) {
		super(parent);
	}
	...
	public void waitingStatusMarchProcess() {
		...
		Set<ILMJYWorldMarch> reachJoinMarchs = getMassJoinMarchs(true);
		if (reachJoinMarchs.size() <= 0) {
			...
			YuriMailService.getInstance().sendMail(MailParames.newBuilder()
					...
					.setDuntype(DungeonMailType.LMJY)
	...
	@Override
	public Set<ILMJYWorldMarch> getMassJoinMarchs(boolean needReach) {
		// 等待和到达的行军
		Set<ILMJYWorldMarch> retMarchs = new HashSet<>();
		// 不是队长行军
		if (isMassMarch()) {
			// 已到达队长家的行军
			List<ILMJYWorldMarch> passiveMarchs = getParent().getParent().getWorldMarchList();
			for (ILMJYWorldMarch march : passiveMarchs) {
				// 类型不匹配
	...
	public void teamMarchReached(ILMJYMassJoinMarch lmjyMassJoinMarch) {
		// TODO Auto-generated method stub
	}

	public void teamMarchCallBack(ILMJYMassJoinMarch lmjyMassJoinMarch) {
		// TODO Auto-generated method stub
	}
