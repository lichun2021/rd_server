...
public class LMJYPlayer extends ILMJYPlayer {
	...
	public LMJYPlayer(Player player) {
		super(player.getXid());
		this.source = player;
	}
	...
	public void init() {
		...
		this.guildId = source.getGuildId();
		this.setPlayerData(LMJYPlayerData.valueOf(this));
		LMJYPlayerPush playerPush = new LMJYPlayerPush(this);
			...
			if (getOnFireEndTime() > now) {
				...
				if (beforePct > 0.3 && afterPct < 0.3) {
					ChatParames parames = ChatParames.newBuilder()
							...
							.setKey(NoticeCfgId.LMJY_CITY_DEF_30)
							.addParms(getName())
							.build();
	...
	public void checkSuperVipSkinToMassMarch() {
		try {
			...
			List<ILMJYWorldMarch> marchs = getParent().getPlayerMarches(getId());
	...
	public void quitGame() {
		source.setLmjyRoomId("");
		source.setLmjyState(null);
		source.getPush().syncPlayerInfo();
		
		try {
			if (getParent().isCampAwin()) {
				for (ArmyEntity army : source.getData().getArmyEntities()) {
					int march = army.getMarch();
					if (march > 0) {
						army.addMarch(-march);
						army.addFree(march);
						LogUtil.logArmyChange(this, army, march, 0, ArmySection.FREE, ArmyChangeReason.MARCH_BACK);
						HawkLog.logPrintln("LMJY PLAYER QUIT MARBACK playerId:{} soldierId: {} march: {}", source.getId(), army.getArmyId(), march);
					}
				}
			}
		} catch (Exception e) {
		...
		LMJYRoomManager.getInstance().invalidate(this);
	}
	...
	public String getLmjyRoomId() {
		return source.getLmjyRoomId();
	}

	@Override
	public void setLmjyRoomId(String lmjyRoomId) {
		source.setLmjyRoomId(lmjyRoomId);
	}

	@Override
	public PState getLmjyState() {
		return source.getLmjyState();
	}

	@Override
	public void setLmjyState(PState lmjyState) {
		source.setLmjyState(lmjyState);
	}
	...
	public boolean sendProtocol(HawkProtocol protocol) {
		if (Objects.equals(getLmjyRoomId(), getParent().getXid().getUUID())) {
	...
	public boolean onProtocol(HawkProtocol protocol) {
		HawkTaskManager.getInstance().postProtocol(getParent().getXid(), LMJYProtocol.valueOf(protocol, this));
		return true;
	}
	...
	public void increaseGuildContribution(int value, Action action, boolean needLog) {
		...
		source.increaseGuildContribution(value, action, needLog);
	}
	...
	public boolean hasGuild() {
		...
		return Objects.nonNull(guildId);
	}
