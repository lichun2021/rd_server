...
public class LMJYSpyMarch extends ILMJYWorldMarch implements ILMJYReportPushMarch {

	public LMJYSpyMarch(ILMJYPlayer parent) {
		super(parent);
	 ...
	 * @param point
	 * @return */
	public boolean sendDetectMail(Player player, Player targetPlayer, ILMJYWorldPoint point) {
		...
		MailParames.Builder playerParamesBuilder = MailParames.newBuilder().setPlayerId(player.getId()).addSubTitles(subTitle)
				.addContents(builder).setOppPfIcon(targetPlayer.getPfIcon()).addTips(targetPlayer.getName()).setDuntype(DungeonMailType.LMJY)
		...
		MailParames.Builder targetParamesBuilder = MailParames.newBuilder().setPlayerId(targetPlayer.getId())
				.setOppPfIcon(player.getPfIcon()).addTips(player.getName()).setDuntype(DungeonMailType.LMJY)
	...
	public DetectMail.Builder createDetectMail(final int radarLevel, Player player, Player targetPlayer, ILMJYWorldPoint point, boolean isMirror) {
		...
		if (pointType == WorldPointType.RESOURCE_VALUE || pointType == WorldPointType.QUARTERED_VALUE) {
			// 资源点 驻扎点
			List<ILMJYWorldMarch> marchs = getParent().getParent().getPointMarches(point.getPointId(), WorldMarchStatus.MARCH_STATUS_MARCH_QUARTERED);
			if (!marchs.isEmpty()) {
				leaderMarch = marchs.get(0);
			}
		} else if (pointType == WorldPointType.PLAYER_VALUE) {
			// 玩家城堡---基地
			List<ILMJYWorldMarch> massMarches = getParent().getParent().getPlayerMarches(targetPlayer.getId(), WorldMarchStatus.MARCH_STATUS_WAITING);
			if (!massMarches.isEmpty()) {
				leaderMarch = massMarches.get(0);
			}
			List<ILMJYWorldMarch> marchs = getParent().getParent().getPointMarches(point.getPointId(), WorldMarchStatus.MARCH_STATUS_MARCH_ASSIST);
		...
		if (pointType == WorldPointType.PLAYER_VALUE && !targetPlayer.getData().getArmyEntities().isEmpty()) {
			...
			if (radarLevel >= GsConst.RadarLevel.LV21 && leaderMarch != null) {
				List<ILMJYWorldMarch> massMarches = getParent().getParent().getPointMarches(point.getPointId(), WorldMarchStatus.MARCH_STATUS_MARCH,
						WorldMarchStatus.MARCH_STATUS_WAITING, WorldMarchType.MASS_JOIN);
		...
		if (pointType == WorldPointType.PLAYER_VALUE) {
			...
			// 藏兵洞
			List<ILMJYWorldMarch> hiddenMarchs = getParent().getParent().getPointMarches(point.getPointId(), WorldMarchType.HIDDEN_MARCH);
	...
	public Set<String> attackReportRecipients() {
		// 防守方援军
		List<ILMJYWorldMarch> helpMarchList = getParent().getParent().getPointMarches(getMarchEntity().getTerminalId(),
