...
public class LMJYAttackPlayerMarch extends ILMJYWorldMarch implements ILMJYReportPushMarch {

	public LMJYAttackPlayerMarch(ILMJYPlayer parent) {
		super(parent);
	}
	...
	public void onMarchReach(Player parent) {
		...
		atkMarchs.add(this);
		// 防守方援军
		List<ILMJYWorldMarch> helpMarchList = helpMarches();
		// 防守方行军
		...
		battleIncome.setDuntype(DungeonMailType.LMJY);
		// 战斗数据输出
		...
		for (Player defer : defPlayers) {
			((ILMJYPlayer) defer).setLastAttacker(getParent());
		}
	}
	...
	private List<ILMJYWorldMarch> helpMarches() {
