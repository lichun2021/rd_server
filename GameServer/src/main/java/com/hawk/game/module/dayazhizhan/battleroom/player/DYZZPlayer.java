...
import com.hawk.game.module.dayazhizhan.battleroom.DYZZConst;
...
public class DYZZPlayer extends IDYZZPlayer {
	...
	public DYZZPlayer(Player player) {
		super(player.getXid());
		this.source = player;
	}
	...
	public void onLogin(Player player) {
		...
		player.setDYZZRoomId(getParent().getId());
		source = player;
		getPush().syncPlayerWorldInfo();
		getPush().syncPlayerInfo();
		getPush().syncGuildInfo();
		getPush().syncArmyInfo(ArmyChangeCause.DEFAULT);
		getPush().pushHeroList();
		getPush().pushSuperSoldier();
		getPush().pushCampNoticeTime();
		getParent().onPlayerLogin(this);
	}
	...
	public void init() {
		...
		int maxCityDef = getParent().getCfg().getCitydefense(); // 配置的
		this.setMaxCityDef(maxCityDef);
		this.setCityDefVal(maxCityDef);
		this.setPlayerData(DYZZPlayerData.valueOf(this));
		DYZZPlayerPush playerPush = new DYZZPlayerPush(this);
		this.setPlayerPush(playerPush);
		this.setRogueCollec(new DYZZRogueCollection(this));

		// 补城防
		DYZZPlayerEffect effect = (DYZZPlayerEffect) getEffect();
		int realmaxCityDef = getData().getRealMaxCityDef(); // 真实的
		effect.putEffVal(EffType.WAR_CITYDEF_VAL, maxCityDef - realmaxCityDef);
		// 补个人行军
		int singlebattle = getParent().getCfg().getSinglebattle();
		int maxMarchSoldierNum = getMaxMarchSoldierNum(EffectParams.getDefaultVal());
		effect.putEffVal(EffType.TROOP_STRENGTH_NUM, singlebattle - maxMarchSoldierNum);
		// 补集结
		int leaderSoldierCnt = getMaxMarchSoldierNum(EffectParams.getDefaultVal());
		int massbattle = getParent().getCfg().getMassbattle();
		effect.putEffVal(EffType.GUILD_TEAM_ARMY_NUM, massbattle - leaderSoldierCnt);

		// 补行军线数量
		int queue = getParent().getCfg().getQueue();
		int maxMarchNum = getMaxMarchNum();
		effect.putEffVal(EffType.MARCH_TROOP_NUM, queue - maxMarchNum);

		this.nextLoghonor = getParent().getCurTimeMil() + DYZZConst.MINUTE_MICROS;
		effect.putEffVal(EffType.MARCH_SPEED_ITEM_BACK_628, source.getEffect().getEffVal(EffType.MARCH_SPEED_ITEM_BACK_628));
		this.checkEquipResearchUnlock();
		this.setDeployedSwInfo(getData().getDeployedSwInfo());
	...
	public ArmourBriefInfo genArmourBriefInfo(ArmourSuitType suit) {
		return ArmourBriefInfo.newBuilder().setSuitType(ArmourSuitType.ARMOUR_NONE).build();
	}

	@Override
	public <T extends HawkObjModule> T getModule(int moduleId) {
		return source.getModule(moduleId);
	}
	...
	public boolean onTick() {
		...
		if (now - lastTickTime > 1000) {
			...
			setDYZZState(DYZZState.GAMEING);
			getData().lockOriginalData();
			lastTickTime = now;
			getRogueCollec().ontick();
			if (getOnFireEndTime() > now) {
				...
				// .setChatType(ChatType.CHAT_DYZZ_SYS)
				// .setKey(NoticeCfgId.DYZZ_CITY_DEF_30)
				// .addParms(getName())
				// .build();
		...
		if (now > nextLoghonor) {
			nextLoghonor = now + DYZZConst.MINUTE_MICROS;
		}
	...
	public void checkSuperVipSkinToMassMarch() {
		try {
			...
			List<IDYZZWorldMarch> marchs = getParent().getPlayerMarches(getId());
...
//		getData().getHeroEntityList().clear();
//		getData().getSuperSoldierEntityList().clear();
//		getData().getArmyEntities().clear();
	}
	...
	public void quitGame() {
		...
		source.setDYZZRoomId("");
		source.setDYZZState(null);
		source.getPush().syncPlayerInfo();

		DYZZRoomManager.getInstance().invalidate(this);
	}

	@Override
	public String getDYZZRoomId() {
		return source.getDYZZRoomId();
	}

	@Override
	public void setDYZZRoomId(String DYZZRoomId) {
		source.setDYZZRoomId(DYZZRoomId);
	}

	@Override
	public DYZZState getDYZZState() {
		return source.getDYZZState();
	}

	@Override
	public void setDYZZState(DYZZState DYZZState) {
		source.setDYZZState(DYZZState);
	}
	...
	public boolean sendProtocol(HawkProtocol protocol) {
		if (Objects.equals(getDYZZRoomId(), getParent().getXid().getUUID())) {
	...
	public boolean onProtocol(HawkProtocol protocol) {
		HawkTaskManager.getInstance().postProtocol(getParent().getXid(), DYZZProtocol.valueOf(protocol, this));
		return true;
	}
