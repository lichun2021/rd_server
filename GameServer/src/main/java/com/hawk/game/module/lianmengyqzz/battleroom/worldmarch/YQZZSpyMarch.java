...
import com.hawk.game.protocol.Mail.PBDetectYQZZBuilding;
...
public class YQZZSpyMarch extends IYQZZWorldMarch implements IYQZZReportPushMarch {

	public YQZZSpyMarch(IYQZZPlayer parent) {
		super(parent);
	}
	...
	public void onMarchReach(Player parent) {
		...
		if (point instanceof YQZZResource) {
			doResourceSpy((YQZZResource) point);
		}

		else if (point instanceof YQZZPylon) {
			doPlyonPointSpy((YQZZPylon) point);
		}
	...
	/** 侦查能量塔行军 */
	private void doPlyonPointSpy(YQZZPylon point) {
		YQZZPylonMarch oldMarch = point.getCollectMarch();
		if (oldMarch == null) {
			return;
		}
		...
		MailParames.Builder playerParamesBuilder = MailParames.newBuilder().setPlayerId(player.getId())
				...
				.addTips(getParent().getName())
				.addContents(DetectMail.newBuilder().setDetectTHRes(builder)).setDuntype(DungeonMailType.YQZZ);
	...
	private void doResourceSpy(YQZZResource point) {
		...
		if (oldMarch == null) {
			...
			// subTitle = new Object[] { point.getX(), point.getY() };
		...
		Player targetPlayer = oldMarch.getParent();
		DetectMail.Builder builder = createDetectMail(radarLevel, player, targetPlayer, point, false);
		...
		// 发送邮件---侦查玩家资源点成功（被侦察方）
		Object[] content = new Object[] { player.getIcon(), player.getName(), player.getPower(), point.getX(), point.getY(), player.getPfIcon(), targetConstructionFactoryLevel,
		...
		MailParames mparames = playerParamesBuilder.setDuntype(DungeonMailType.YQZZ).build();
		FightMailService.getInstance().sendMail(mparames);

		FightMailService.getInstance().sendMail(targetParamesBuilder.setDuntype(DungeonMailType.YQZZ).build());
	}

	private void doBuildSpy(IYQZZWorldPoint point) {
		IYQZZBuilding build = (IYQZZBuilding) point;
		if (point.getProtectedEndTime() > getParent().getParent().getCurTimeMil()) {
			return;
		}
		List<IYQZZWorldMarch> quarteredMarchs = getParent().getParent().getPointMarches(getMarchEntity().getTerminalId(), WorldMarchStatus.MARCH_STATUS_MARCH_QUARTERED);

		// if (quarteredMarchs.isEmpty()) {
		// return;
		// }

		if (Objects.equals(point.getGuildId(), getParent().getGuildId())) {
			// FightMailService.getInstance().sendMail(MailParames.newBuilder()
			// .setPlayerId(getPlayerId())
			// .addSubTitles(point.getX(), point.getY())
			// .addContents(point.getX(), point.getY())
			// .setDuntype(DungeonMailType.YQZZ)
			// .setMailId(MailId.YQZZ_SPY_BUILD_FAIL)
			// .build());
			return;
		}
		...
		PBDetectYQZZBuilding.Builder builder = PBDetectYQZZBuilding.newBuilder().setLevel(radarLevel).setPointX(point.getX()).setPointY(point.getY());
		...
		// 侦查邮件
		MailParames.Builder playerParamesBuilder = MailParames.newBuilder().setPlayerId(getParent().getId()).setMailId(MailId.YQZZ_SPY_BUILD_SUCCESS)
				.addSubTitles(point.getX(), point.getY())
				.setDuntype(DungeonMailType.YQZZ)
				.addContents(DetectMail.newBuilder().setYqzzBuild(builder)).addTips(point.getX()).addTips(point.getY());
		...
		for (IWorldMarch tempMarch : quarteredMarchs) {
			FightMailService.getInstance()
					.sendMail(MailParames.newBuilder().setPlayerId(tempMarch.getPlayerId()).setMailId(MailId.YQZZ_SPY_BUILD_BE).addSubTitles(point.getX(), point.getY())
	...
	private void doPlayerSpy(IYQZZWorldPoint point) {
	 ...
	 * @return
	 */
	public boolean sendDetectMail(Player player, Player targetPlayer, IYQZZWorldPoint point) {
		if (player == null || targetPlayer == null || point == null || point.getProtectedEndTime() > getParent().getParent().getCurTimeMil()) {
			return false;
		}
		...
		MailParames.Builder playerParamesBuilder = MailParames.newBuilder().setPlayerId(player.getId()).addSubTitles(subTitle).addContents(builder)
				...
				.addTips(targetPlayer.getName()).setDuntype(DungeonMailType.YQZZ)
				.addMidTitles(targetPlayer.getGuildTag(), targetPlayer.getName());
		...
		FightMailService.getInstance().sendMail(playerParamesBuilder.setDuntype(DungeonMailType.YQZZ).build());

		FightMailService.getInstance().sendMail(targetParamesBuilder.setDuntype(DungeonMailType.YQZZ).build());
		return true;
	}

	public DetectMail.Builder createDetectMail(final int radarLevel, Player player, Player targetPlayer, IYQZZWorldPoint point, boolean isMirror) {
		...
		if (pointType == WorldPointType.RESOURCE_VALUE || pointType == WorldPointType.QUARTERED_VALUE) {
			leaderMarch = ((YQZZResource) point).getMarch();
		} else if (pointType == WorldPointType.PLAYER_VALUE) {
			// 玩家城堡---基地
			List<IYQZZWorldMarch> massMarches = getParent().getParent().getPlayerMarches(targetPlayer.getId(), WorldMarchStatus.MARCH_STATUS_WAITING);
			if (!massMarches.isEmpty()) {
				leaderMarch = massMarches.get(0);
			}
			List<IYQZZWorldMarch> marchs = getParent().getParent().getPointMarches(point.getPointId(), WorldMarchStatus.MARCH_STATUS_MARCH_ASSIST);
		...
		if (pointType == WorldPointType.PLAYER_VALUE && !targetPlayer.getData().getArmyEntities().isEmpty()) {
			...
			if (radarLevel >= GsConst.RadarLevel.LV21 && leaderMarch != null) {
				List<IYQZZWorldMarch> massMarches = getParent().getParent().getPointMarches(point.getPointId(), WorldMarchStatus.MARCH_STATUS_MARCH,
		...
		if (pointType == WorldPointType.PLAYER_VALUE) {
			...
			// 藏兵洞
			List<IYQZZWorldMarch> hiddenMarchs = getParent().getParent().getPointMarches(point.getPointId(), WorldMarchType.HIDDEN_MARCH);
			if (radarLevel >= GsConst.RadarLevel.LV10) {
	...
	public Set<String> attackReportRecipients() {
		// 防守方援军
		List<IYQZZWorldMarch> helpMarchList = getParent().getParent().getPointMarches(getMarchEntity().getTerminalId(), WorldMarchStatus.MARCH_STATUS_MARCH_ASSIST,
