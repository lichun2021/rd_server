...
import com.hawk.game.module.lianmenxhjz.battleroom.XHJZConst;
...
import com.hawk.game.module.lianmenxhjz.battleroom.XHJZWorldScene;
import com.hawk.game.module.lianmenxhjz.battleroom.cfg.XHJZBattleCfg;
import com.hawk.game.module.lianmenxhjz.battleroom.cfg.XHJZFuelBuffCfg;
...
import com.hawk.game.module.lianmenxhjz.battleroom.player.module.guildformation.XHJZGuildFormationModule;
...
import com.hawk.game.protocol.Army.ArmySoldierPB.Builder;
...
public abstract class IXHJZWorldMarch implements IWorldMarch {

	private final IXHJZPlayer parent;
	private XHJZMarchEntity marchEntity;
	private List<Position> nextPoints;
	private List<Position> marchPoints;
	private double gasoline;
	private XHJZFuelBuffCfg lastFuelBuffGasoline = null;
	private WorldMarchPB lastBuilder;
	public IXHJZWorldMarch(IXHJZPlayer parent) {
		this.parent = parent;
	}
	
	public int getXHJZTerminalX(){
		Position endp = marchPoints.get(marchPoints.size()-1);
		return endp.getX();
	}
	
	public int getXHJZTerminalY(){
		Position endp = marchPoints.get(marchPoints.size()-1);
		return endp.getY();
	}

	public final IXHJZPlayer getParent() {
		return parent;
	}

	@Override
	public XHJZMarchEntity getMarchEntity() {
		return marchEntity;
	}

	public void setMarchEntity(XHJZMarchEntity marchEntity) {
		this.marchEntity = marchEntity;
	}

	public long getReachTime(){
		return marchEntity.getReachTime();
	}
	
	public int getFuelBuff(EffType effType) {
		if (lastFuelBuffGasoline != null && gasoline >= lastFuelBuffGasoline.getMinMax().first && gasoline <= lastFuelBuffGasoline.getMinMax().second) {
			return lastFuelBuffGasoline.getEffectVal(effType);
		}

		for (XHJZFuelBuffCfg bcfg : HawkConfigManager.getInstance().getConfigIterator(XHJZFuelBuffCfg.class)) {
			if (bcfg.getMinMax().first <= gasoline && bcfg.getMinMax().second >= gasoline) {
				lastFuelBuffGasoline = bcfg;
				break;
			}
		}
		if (lastFuelBuffGasoline != null && gasoline >= lastFuelBuffGasoline.getMinMax().first && gasoline <= lastFuelBuffGasoline.getMinMax().second) {
	...
	public WorldMarchStatus getMassJoinMarchStatus(String playerId) {
		try {
			if (getPlayerId().equals(playerId)) {
				return WorldMarchStatus.valueOf(this.getMarchStatus());
			}

			Set<IXHJZWorldMarch> massJoinMarchs = getMassJoinMarchs(false);
	...
	public GuildWarShowPB getGuildWarShoPb(IXHJZPlayer player) {
		// 行军状态
		...
		if (marchStatus.equals(WorldMarchStatus.MARCH_STATUS_WAITING)) {
			...
			showPb.setEndTime(ps.getReachTime());
		}
		if (Objects.equals(player.getGuildId(), getParent().getGuildId())) {
			showPb.setIsatk(true);
		}
		try {
			XHJZGuildFormationObj formationObj = getParent().getParent().getGuildFormation(player.getGuildId());
			GuildFormationCell formation = formationObj.getGuildFormation(getMarchId());
			if (formation != null) {
	...
	public  boolean onArmyBack() {
//		final Player player, List<ArmyInfo> armyList, List<Integer> heroId, int supersoldierId, IXHJZWorldMarch worldMarch
		IXHJZPlayer player=	getParent();
		List<ArmyInfo> armyList = getMarchEntity().getArmys();
		List<Integer> heroId = getMarchEntity().getHeroIdList();
		int supersoldierId = getMarchEntity().getSuperSoldierId();
		
		
		for (ArmyInfo army : armyList) {
			if (army.getDeadCount() > 0 || army.getWoundedCount() > 0) {
				army.setWoundedCount(0);
				army.setDeadCount(0);
			}
		}
		player.setGasoline(player.getGasoline() + this.getGasoline());
		boolean result = ArmyService.getInstance().onArmyBack(player, armyList, heroId, supersoldierId, this);
		return result;
	}
	...
	public WorldMarchPB.Builder toBuilder(WorldMarchRelation relation) {
		if(lastBuilder!=null){
			return lastBuilder.toBuilder().setRelation(relation).setXhjzGasoline((int) gasoline);
		}
		WorldMarchPB.Builder builder = WorldMarchPB.newBuilder();
		XHJZMarchEntity entity = getMarchEntity();
		...
		builder.setTalentType(getMarchEntity().getEffectParams().getWorldmarchReq().getTalentType());
		...
		for (int i = 0; i < entity.getArmys().size(); i++) {
			...
			Builder armySoldierPB = info.toArmySoldierPB(getParent());
			builder.addArmy(armySoldierPB);
		}
		...
		builder.addAllXhjzPoints(marchPoints);
		builder.setXhjzGasoline((int) gasoline);
		
		if(getMarchStatus() == WorldMarchStatus.MARCH_STATUS_MARCH_QUARTERED_VALUE){
			IXHJZWorldPoint point = getParent().getParent().getWorldPoint(getTerminalId()).orElse(null);
			if(point instanceof IXHJZBuilding){
				IXHJZBuilding build = (IXHJZBuilding) point;
				XHJZQuateredState qstate = build.getMarchQuateredStatus(this);
				builder.setXhjzQuateredState(qstate);
			}
		}
		builder.setXhjzArmycnt(getFreeArmyNumber());
		builder.setCamp(getParent().getCamp().intValue());
		builder.setXhjzBianduiNum(marchEntity.getXhjzBianduiNum());
		lastBuilder = builder.build();
		return builder;
	}
	
	public int getFreeArmyNumber(){
		XHJZMarchEntity entity = getMarchEntity();
		int xhjzArmycnt = 0;
		for (int i = 0; i < entity.getArmys().size(); i++) {
			ArmyInfo info = entity.getArmys().get(i);
			...
			xhjzArmycnt+= info.getFreeCnt();
		}
		return xhjzArmycnt;
	}

	public GuildWarSingleInfo.Builder getGuildWarSingleInfo() {
		return getGuildWarSingleInfo(null);
	}
	 ...
	 * @return
	 */
	public GuildWarSingleInfo.Builder getGuildWarSingleInfo(GuildFormationCell formation) {
...
//		builder.setFormation(getFormationInfo());
//		builder.addAllXhjzPoints(marchPoints);
//		builder.setXhjzGasoline(gasoline);
//		return builder;
//	}
	...
	public boolean speedUp(int timeReducePercent, int speedUpTimes) {
		...
		if (this instanceof IXHJZReportPushMarch) {
			((IXHJZReportPushMarch) this).reachTimeChange();
		}
		return true;
	}

	@Override
	public Set<IXHJZWorldMarch> getQuarterMarch() {
		if (getMarchStatus() != WorldMarchStatus.MARCH_STATUS_MARCH_QUARTERED_VALUE) {
			return Collections.emptySet();
		}
		List<IXHJZWorldMarch> defMarchList = getParent().getParent().getPointMarches(getTerminalId(), WorldMarchStatus.MARCH_STATUS_MARCH_QUARTERED);
		return new HashSet<>(defMarchList);
	}

	@Override
	public Set<IXHJZWorldMarch> getMassJoinMarchs(boolean b) {
		return Collections.emptySet();
	}
	...
	private void pushMarchEvent(MarchEvent eventType) {
		XHJZWorldScene worldScene = getParent().getParent().getWorldPointService().getWorldScene();
		for (IXHJZPlayer player : getParent().getParent().getPlayerList(XHJZState.GAMEING)) {
			HawkAOIObj aoiObj = worldScene.getObjById(player.getEye().getAoiObjId());
			if (Objects.isNull(aoiObj)) {
				continue;
			}
			if (!isInview(aoiObj.getX(), aoiObj.getY())) {
				continue;
			}
			...
			player.getInviewMarchs().add(getMarchId());
		}

		// 发送主播
		for (IXHJZPlayer anchor : getParent().getParent().getAnchors()) {
			MarchEventSync.Builder builder = createSyncPB(eventType, getRelation(anchor));
			anchor.sendProtocol(HawkProtocol.valueOf(HP.code.WORLD_MARCH_EVENT_SYNC_VALUE, builder));
		}
	}
	 ...
	 * @return
	 */
	public WorldMarchRelation getRelation(IXHJZPlayer tarplayer) {
		...
		// 目标点
		Optional<IXHJZWorldPoint> targetPointOp = getParent().getParent().getWorldPoint(marchEntity.getTerminalId());
		...
		IXHJZWorldPoint targetPoint = targetPointOp.get();
		if (tarplayer.getGuildId().equals(targetPoint.getGuildId()) && !tarplayer.isInSameGuild(parent)) {
	...
	public void sendBattleResultInfo(boolean isWin, List<ArmyInfo> atkArmyList, List<ArmyInfo> defArmyList, boolean isMonsterDead) {
		...
		List<IXHJZPlayer> players = getParent().getParent().getPlayerList(XHJZState.GAMEING);
		for (IXHJZPlayer pla : players) {
	...
	public void remove() {
		try {
			if (this instanceof IXHJZReportPushMarch) {
				((IXHJZReportPushMarch) this).removeAttackReport();
			}
	...
	public void updateDefMarchAfterWar(List<IXHJZWorldMarch> helpMarchList, Map<String, List<ArmyInfo>> defArmyMap, Map<String, List<ArmyInfo>> battlemap) {
		int battleHeal = getParent().getParent().getCfg().getBattleHeal();
		// 更新援助防御玩家行军的部队
		for (IXHJZWorldMarch tmpMarch : helpMarchList) {
			List<ArmyInfo> leftList = defArmyMap.get(tmpMarch.getPlayerId());
			List<ArmyInfo> batlList = battlemap.get(tmpMarch.getPlayerId());
			Map<Integer,ArmyInfo> batlMap = new HashMap<>();
			for(ArmyInfo bar : batlList){
				batlMap.put(bar.getArmyId(), bar);
			}
			
			for (ArmyInfo info : leftList) {
				ArmyInfo battleArmy = batlMap.get(info.getArmyId());
				if (battleArmy == null) {
					continue;
				}
				int woud = battleArmy.getWoundedCount();
				int dead = battleArmy.getDeadCount();
				if (woud == 0 && dead == 0) {
					continue;
				}
				info.setWoundedCount((int) (info.getWoundedCount() - Math.ceil(woud * GsConst.EFF_PER * battleHeal)));
				info.setDeadCount((int) (info.getDeadCount() - Math.ceil(dead * GsConst.EFF_PER * battleHeal)));
			}
...
//				IXHJZPlayer assistPlayer = tmpMarch.getParent();
...
//			tmpMarch.onMarchReturn(tmpMarch.getMarchEntity().getTerminalId(), tmpMarch.getMarchEntity().getOrigionId(), leftList);
	...
	public void onMarchCallback() {
		...
		marchEntity.setCallBackTime(HawkTime.getMillisecond());
		marchEntity.setCallBackX(point.getX());
		marchEntity.setCallBackY(point.getY());
		onMarchReturn(marchEntity.getTerminalId(), getParent().getPointId(), marchEntity.getArmys());
	...
	public boolean onMarchReturn(int origionId, int terminalId, List<ArmyInfo> selfArmys) {
		...
		marchEntity.setStartTime(HawkTime.getMillisecond());
	...
	public long getMarchNeedTime() {
		...
		MarchPart part = new MarchPart(origion, terminal, false, true);
		// 正常行军时间
		double time = getPartMarchTime(part.getDistance(), speed, false);
		// // 机器人模式下 不进行黑土地减速
	...
	private double effectToMarchTime(double time, boolean slowDown, double slowDownTime) {
		try {
			...
			if (this.isMassJoinMarch()) {
				IXHJZPlayer leader = getParent().getParent().getMarch(getMarchEntity().getTargetId()).getParent();
	...
	public double getMarchBaseSpeed() {
		...
		if (isMassMarch()) {
			Set<IXHJZWorldMarch> joinMarchs = getMassJoinMarchs(true);
			for (IXHJZWorldMarch tempMarch : joinMarchs) {
	...
	public GuildWarTeamInfo.Builder getGuildWarInitiativeInfo() {
		...
		// 队长id
		Player leader = getParent();
		// 联盟编队
		XHJZGuildFormationObj formationObj = getParent().getParent().getGuildFormation(leader.getGuildId());
		GuildFormationCell formation = formationObj.getGuildFormation(getMarchId());
		// 队长位置
		...
		if (formation != null) {
			for (String fightId : formation.getFightIds()) {
				if (leader.getId().equals(fightId)) {
					continue;
				}
				Optional<IXHJZWorldMarch> findAny = joinMarchs.stream().filter(m -> m.getPlayerId().equals(fightId)).findAny();
				if (findAny.isPresent()) {
					continue;
				}
				Player joinMemeber = getParent().getParent().getPlayer(fightId);
				...
				XHJZGuildFormationModule module = joinMemeber.getModule(XHJZConst.ModuleType.XHJZGuildFormation);
	...
	public GuildWarTeamInfo.Builder getGuildWarPassivityInfo() {
		...
		IXHJZPlayer leader = XHJZRoomManager.getInstance().makesurePlayer(leaderId);
		// 队长位置
		...
		List<IXHJZWorldMarch> assistandMarchs = getParent().getParent().getPointMarches(leader.getPointId(), WorldMarchStatus.MARCH_STATUS_MARCH_ASSIST,
				WorldMarchType.ASSISTANCE);
		for (IXHJZWorldMarch assistandMarch : assistandMarchs) {
	...
	public String toString() {
		try {
			return MoreObjects.toStringHelper(this)
					.add("i", marchEntity.getMarchId())
					.add("h", marchEntity.getHeroIdStr())
					.add("a", marchEntity.getArmyStr())
					.add("f", marchEntity.getOrigionX() + "," + marchEntity.getOrigionY())
					.add("t", marchEntity.getTerminalX() + "," + marchEntity.getTerminalY())
					.toString();
	...
	public List<Position> getNextPoints() {
		return nextPoints;
	}

	public void setNextPoints(List<Position> nextPoints) {
		this.nextPoints = nextPoints;
	}

	public List<Position> getMarchPoints() {
		return marchPoints;
	}

	public void setMarchPoints(List<Position> marchPoints) {
		this.marchPoints = marchPoints;
	}
	
	public void calFullpath(long startTime,  List<Position.Builder> roadAllPoints) {
		roadAllPoints.get(0).setStartTime(startTime).setReachTime(startTime);
//		double speed = getMarchBaseSpeed() * getParent().getMarchSpeedUp();
		XHJZBattleCfg battlecfg = getParent().getParent().getCfg();
		boolean fuelFree = false;
		int speedUp = 0;
		Position.Builder end = roadAllPoints.get(roadAllPoints.size()-1);
		IXHJZBuilding build = (IXHJZBuilding) getParent().getParent().getWorldPoint(end.getX(), end.getY()).orElse(null);
		if (build.getBuildType() == XHJZBuildType.LIU && build.underGuildControl(getParent().getGuildId())) {
			fuelFree = build.getBuildTypeCfg().getOccupyMarchFree() > 0;
			speedUp = build.getBuildTypeCfg().getOccupyMarchSpeedUp();
		}
		for (int i = 1; i < roadAllPoints.size(); i++) {
			Position.Builder sp = roadAllPoints.get(i - 1);
			Position.Builder ep = roadAllPoints.get(i);
			
			
			int gasoline = fuelFree ? 0 : (int) battlecfg.getFuelUseBase();
			// 行军速度
			// 出发点
			AlgorithmPoint origion = new AlgorithmPoint(sp.getX(), sp.getY());

			// 目标点
			AlgorithmPoint terminal = new AlgorithmPoint(ep.getX(), ep.getY());
			// 根据黑土地进行分段

			// 正常行军时间
			double time = battlecfg.getMarchBaseTime() * (1 - speedUp * 0.01); // getPartMarchTime(part.getDistance(), speed, false) * (1 - speedUp * 0.01);
			if (origion.distanceTo(terminal) < 1) {
				time = 1;
			}
			// 计算作用号效果(向上取整)
//			time = Math.ceil(effectToMarchTime(time, false, 0));
			time = time > 1.0 ? time : 1.0;
			ep.setXhjzGasoline(gasoline).setStartTime(sp.getReachTime()).setReachTime(sp.getReachTime() + (long) (time * 1000)).build();

		}
		
		List<Position> marchPoints = new ArrayList<>();
		List<Position> nextPoints = new LinkedList<>();
		for (int i = 0; i < roadAllPoints.size(); i++) {
			Position.Builder ep = roadAllPoints.get(i);
			marchPoints.add(ep.build());
			if (i > 0) {
				nextPoints.add(ep.build());
			}
		}
		setMarchPoints(marchPoints);
	...
	public long getCreatetime(){
		return getMarchEntity().getCreateTime();
	}

	public WorldMarchPB getLastBuilder() {
		return lastBuilder;
	}

	public void setLastBuilder(WorldMarchPB lastBuilder) {
		this.lastBuilder = lastBuilder;
	}
	
	public Position getTerminalPosition(){
		return marchPoints.get(marchPoints.size()-1);
	}
}
