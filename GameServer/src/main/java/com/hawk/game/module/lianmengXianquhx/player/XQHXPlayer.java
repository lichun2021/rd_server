...
import com.hawk.game.module.lianmengXianquhx.player.module.XQHXArmyModule;
import com.hawk.game.module.lianmengXianquhx.player.module.XQHXMarchModule;
import com.hawk.game.module.lianmengXianquhx.player.module.XQHXWorldModule;
...
public class XQHXPlayer extends IXQHXPlayer {
	...
	public XQHXPlayer(Player player) {
		super(player.getXid());
	...
	public void init() {
		...
		this.setPlayerData(XQHXPlayerData.valueOf(this));
		XQHXPlayerPush playerPush = new XQHXPlayerPush(this);
		this.setPlayerPush(playerPush);
		getParent();
		this.nextLoghonor = getParent().getCurTimeMil() + XQHXConst.MINUTE_MICROS;
	...
	public void initModules() {
		registerModule(XQHXConst.ModuleType.XQHXWorld, new XQHXWorldModule(this));
		registerModule(XQHXConst.ModuleType.XQHXMarch, new XQHXMarchModule(this));
		registerModule(XQHXConst.ModuleType.XQHXArmy, new XQHXArmyModule(this));
		// 联盟编队
		registerModule(XQHXConst.ModuleType.XQHXGuildFormation, new XQHXGuildFormationModule(this));
	}
	...
	public boolean onTick() {
		...
		if (now - lastTickTime > 1000) {
			...
			setXQHXState(XQHXState.GAMEING);
			...
			if (getOnFireEndTime() > now) {
				...
				// .setChatType(ChatType.CHAT_XQHX_SYS)
				// .setKey(NoticeCfgId.XQHX_CITY_DEF_30)
				// .addParms(getName())
				// .build();
		...
		if (now > nextLoghonor) {
			nextLoghonor = now + XQHXConst.MINUTE_MICROS;
		}
	...
	public void checkSuperVipSkinToMassMarch() {
		try {
			...
			List<IXQHXWorldMarch> marchs = getParent().getPlayerMarches(getId());
	...
	public void quitGame() {
		...
		source.setXQHXRoomId("");
		source.setXQHXState(null);
		source.getPush().syncPlayerInfo();

		XQHXRoomManager.getInstance().invalidate(this);
	}

	@Override
	public String getXQHXRoomId() {
		return source.getXQHXRoomId();
	}

	@Override
	public void setXQHXRoomId(String tblyRoomId) {
		source.setXQHXRoomId(tblyRoomId);
	}

	@Override
	public XQHXState getXQHXState() {
		return source.getXQHXState();
	}

	@Override
	public void setXQHXState(XQHXState tblyState) {
		source.setXQHXState(tblyState);
	}
	...
	public boolean sendProtocol(HawkProtocol protocol) {
		if (Objects.equals(getXQHXRoomId(), getParent().getXid().getUUID())) {
