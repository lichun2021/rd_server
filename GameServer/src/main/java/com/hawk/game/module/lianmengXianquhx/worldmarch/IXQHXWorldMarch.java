...
import com.hawk.game.module.lianmengXianquhx.player.module.guildformation.XQHXGuildFormationModule;
...
public abstract class IXQHXWorldMarch implements IWorldMarch {
	private int hashThread;
	private final IXQHXPlayer parent;
	private XQHXMarchEntity marchEntity;

	public IXQHXWorldMarch(IXQHXPlayer parent) {
		this.parent = parent;
	}

	public final IXQHXPlayer getParent() {
		return parent;
	}

	@Override
	public XQHXMarchEntity getMarchEntity() {
		return marchEntity;
	}

	public void setMarchEntity(XQHXMarchEntity marchEntity) {
	...
	public WorldMarchStatus getMassJoinMarchStatus(String playerId) {
		try {
			...
			Set<IXQHXWorldMarch> massJoinMarchs = getMassJoinMarchs(false);
	...
	public GuildWarShowPB getGuildWarShoPb(IXQHXPlayer player) {
		// 行军状态
		...
		try {
			XQHXGuildFormationObj formationObj = getParent().getParent().getGuildFormation(player.getGuildId());
	...
	public WorldMarchPB.Builder toBuilder(WorldMarchRelation relation) {
		...
		XQHXMarchEntity entity = getMarchEntity();
	...
	public boolean speedUp(int timeReducePercent, int speedUpTimes) {
		...
		if (this.isMassMarch()) {
			Set<IXQHXWorldMarch> joinMarchs = getMassJoinMarchs(false);
			for (IXQHXWorldMarch joinMarch : joinMarchs) {
		...
		if (this instanceof IXQHXReportPushMarch) {
			((IXQHXReportPushMarch) this).reachTimeChange();
		}
		return true;
	}

	@Override
	public Set<IXQHXWorldMarch> getQuarterMarch() {
		if (getMarchStatus() != WorldMarchStatus.MARCH_STATUS_MARCH_QUARTERED_VALUE) {
			return Collections.emptySet();
		}
		List<IXQHXWorldMarch> defMarchList = getParent().getParent().getPointMarches(getTerminalId(), WorldMarchStatus.MARCH_STATUS_MARCH_QUARTERED);
		return new HashSet<>(defMarchList);
	}

	@Override
	public Set<IXQHXWorldMarch> getMassJoinMarchs(boolean b) {
		return Collections.emptySet();
	}
	...
	public void pushMarchEvent(MarchEvent eventType) {
		for (IXQHXPlayer player : getParent().getParent().getPlayerList(XQHXState.GAMEING)) {
		...
		// 发送主播
		for (IXQHXPlayer anchor : getParent().getParent().getAnchors()) {
	 ...
	 * @return
	 */
	public WorldMarchRelation getRelation(IXQHXPlayer tarplayer) {
		...
		// 目标点
		Optional<IXQHXWorldPoint> targetPointOp = getParent().getParent().getWorldPoint(marchEntity.getTerminalId());
		...
		IXQHXWorldPoint targetPoint = targetPointOp.get();
	...
	public void sendBattleResultInfo(boolean isWin, List<ArmyInfo> atkArmyList, List<ArmyInfo> defArmyList, boolean isMonsterDead) {
		...
		List<IXQHXPlayer> players = getParent().getParent().getPlayerList(XQHXState.GAMEING);
		for (IXQHXPlayer pla : players) {
	...
	public void remove() {
		try {
			if (this instanceof IXQHXReportPushMarch) {
				((IXQHXReportPushMarch) this).removeAttackReport();
			}
	...
	public void updateDefMarchAfterWar(List<IXQHXWorldMarch> helpMarchList, Map<String, List<ArmyInfo>> defArmyMap) {
		// 更新援助防御玩家行军的部队
		for (IXQHXWorldMarch tmpMarch : helpMarchList) {
			...
			if (tmpMarch.getMarchType() == WorldMarchType.ASSISTANCE) {
				...
				IXQHXPlayer assistPlayer = tmpMarch.getParent();
	...
	private double effectToMarchTime(double time, boolean slowDown, double slowDownTime) {
		...
		if (this.isMassJoinMarch()) {
			Optional<IXQHXWorldPoint> targetPoint = getParent().getParent().getWorldPoint(marchEntity.getTerminalId());
			if (targetPoint.isPresent()) {
				IXQHXPlayer leader = (IXQHXPlayer) targetPoint.get();
	...
	public double getMarchBaseSpeed() {
		...
		if (isMassMarch()) {
			Set<IXQHXWorldMarch> joinMarchs = getMassJoinMarchs(true);
			for (IXQHXWorldMarch tempMarch : joinMarchs) {
	...
	public GuildWarTeamInfo.Builder getGuildWarInitiativeInfo() {
		...
		// 联盟编队
		XQHXGuildFormationObj formationObj = getParent().getParent().getGuildFormation(leader.getGuildId());
		GuildFormationCell formation = formationObj.getGuildFormation(getMarchId());
		// 队长位置
		...
		// 加入集结的行军
		Set<IXQHXWorldMarch> joinMarchs = this.getMassJoinMarchs(false);
		for (IXQHXWorldMarch joinMarch : joinMarchs) {
		...
		if (formation != null) {
			for (String fightId : formation.getFightIds()) {
				...
				Optional<IXQHXWorldMarch> findAny = joinMarchs.stream().filter(m -> m.getPlayerId().equals(fightId)).findAny();
				...
				XQHXGuildFormationModule module = joinMemeber.getModule(XQHXConst.ModuleType.XQHXGuildFormation);
	...
	public GuildWarTeamInfo.Builder getGuildWarPassivityInfo() {
		...
		IXQHXPlayer leader = XQHXRoomManager.getInstance().makesurePlayer(leaderId);
		// 队长位置
		...
		List<IXQHXWorldMarch> assistandMarchs = getParent().getParent().getPointMarches(leader.getPointId(), WorldMarchStatus.MARCH_STATUS_MARCH_ASSIST,
				WorldMarchType.ASSISTANCE);
		for (IXQHXWorldMarch assistandMarch : assistandMarchs) {
