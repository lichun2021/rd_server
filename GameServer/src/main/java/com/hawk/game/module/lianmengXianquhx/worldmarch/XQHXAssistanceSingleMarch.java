...
public class XQHXAssistanceSingleMarch extends IXQHXWorldMarch implements IXQHXPassiveAlarmTriggerMarch, IXQHXReportPushMarch {

	public XQHXAssistanceSingleMarch(IXQHXPlayer parent) {
		super(parent);
	}
	...
	public void onMarchReach(Player parent) {
		...
		// 路点为空
		if (point == null || !(point instanceof IXQHXPlayer)) {
		...
		if (backList.size() > 0) {
			...
			IXQHXWorldMarch back = getParent().getParent().startMarch(getParent(), point, getParent(), WorldMarchType.ASSISTANCE, "",0, effParams);
