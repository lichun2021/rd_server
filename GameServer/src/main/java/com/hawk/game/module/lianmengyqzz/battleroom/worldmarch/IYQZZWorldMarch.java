...
import com.hawk.game.module.lianmengyqzz.battleroom.YQZZWorldScene;
...
public abstract class IYQZZWorldMarch implements IWorldMarch {
	...
	private YQZZMarchEntity marchEntity;
	private int hashThread;
	
	public IYQZZWorldMarch(IYQZZPlayer parent) {
		this.parent = parent;
	}

	public final IYQZZPlayer getParent() {
		return parent;
	}

	@Override
	public YQZZMarchEntity getMarchEntity() {
		return marchEntity;
	}

	public void setMarchEntity(YQZZMarchEntity marchEntity) {
	...
	public WorldMarchStatus getMassJoinMarchStatus(String playerId) {
		try {
			...
			Set<IYQZZWorldMarch> massJoinMarchs = getMassJoinMarchs(false);
	...
	public GuildWarShowPB getGuildWarShoPb(IYQZZPlayer viewer) {
		// 行军状态
		...
		try {
			YQZZGuildFormationObj formationObj = getParent().getParent().getGuildFormation(viewer.getGuildId());
	...
	public WorldMarchPB.Builder toBuilder(WorldMarchRelation relation) {
		...
		YQZZMarchEntity entity = getMarchEntity();
...
//				builder.addHeroLvl(hero.getLevel());
	...
	public boolean speedUp(int timeReducePercent, int speedUpTimes) {
		...
		if (this.isMassMarch()) {
			Set<IYQZZWorldMarch> joinMarchs = getMassJoinMarchs(false);
			for (IYQZZWorldMarch joinMarch : joinMarchs) {
		...
		if (this instanceof IYQZZReportPushMarch) {
			((IYQZZReportPushMarch) this).reachTimeChange();
		}
		return true;
	}
	
	@Override
	public Set<IYQZZWorldMarch> getQuarterMarch() {
		if (getMarchStatus() != WorldMarchStatus.MARCH_STATUS_MARCH_QUARTERED_VALUE) {
			return Collections.emptySet();
		}
		List<IYQZZWorldMarch> defMarchList = getParent().getParent().getPointMarches(getTerminalId(), WorldMarchStatus.MARCH_STATUS_MARCH_QUARTERED);
		return new HashSet<>(defMarchList);
	}

	@Override
	public Set<IYQZZWorldMarch> getMassJoinMarchs(boolean b) {
		return Collections.emptySet();
	}
...
//		}
	...
	public void pushMarchEvent(MarchEvent eventType) {
		...
		markUnchanged();
		YQZZWorldScene worldScene = getParent().getParent().getWorldPointService().getWorldScene();
		for (IYQZZPlayer player : getParent().getParent().getPlayerList(YQZZState.GAMEING)) {
	 ...
	 * @return
	 */
	public WorldMarchRelation getRelation(IYQZZPlayer tarplayer) {
		...
		// 目标点
		Optional<IYQZZWorldPoint> targetPointOp = getParent().getParent().getWorldPoint(marchEntity.getTerminalId());
		...
		IYQZZWorldPoint targetPoint = targetPointOp.get();
	...
	public void sendBattleResultInfo(boolean isWin, List<ArmyInfo> atkArmyList, List<ArmyInfo> defArmyList, boolean isMonsterDead) {
		...
		List<IYQZZPlayer> players = getParent().getParent().getPlayerList(YQZZState.GAMEING);
		for (IYQZZPlayer pla : players) {
	...
	public synchronized void remove() {
		try {
			if (this instanceof IYQZZReportPushMarch) {
				((IYQZZReportPushMarch) this).removeAttackReport();
			}
	...
	public void updateDefMarchAfterWar(List<IYQZZWorldMarch> helpMarchList, Map<String, List<ArmyInfo>> defArmyMap) {
		// 更新援助防御玩家行军的部队
		for (IYQZZWorldMarch tmpMarch : helpMarchList) {
			...
			if (tmpMarch.getMarchType() == WorldMarchType.ASSISTANCE) {
				...
				IYQZZPlayer assistPlayer = tmpMarch.getParent();
...
//		List<MarchPart> parts = WorldUtil.getMarchParts(origion, terminal);
	...
	private double effectToMarchTime(double time, boolean slowDown, double slowDownTime) {
		...
		speedUpEffectVal += player.getEffect().getEffVal(EffType.YQZZ_633);
		...
		if (this.isMassJoinMarch()) {
			Optional<IYQZZWorldPoint> targetPoint = getParent().getParent().getWorldPoint(marchEntity.getTerminalId());
			IYQZZWorldPoint iswWorldPoint = targetPoint.orElse(null);
			if (iswWorldPoint instanceof IYQZZPlayer) {
				IYQZZPlayer leader = (IYQZZPlayer) iswWorldPoint;
		...
		// // 作用值/10000）
		// IJBYQZZorldPoint terminalPoint;
	...
	public double getMarchBaseSpeed() {
		...
		if (isMassMarch()) {
			Set<IYQZZWorldMarch> joinMarchs = getMassJoinMarchs(true);
			for (IYQZZWorldMarch tempMarch : joinMarchs) {
	...
	public GuildWarTeamInfo.Builder getGuildWarInitiativeInfo() {
		...
		// 联盟编队
		YQZZGuildFormationObj formationObj = getParent().getParent().getGuildFormation(leader.getGuildId());
		GuildFormationCell formation = formationObj.getGuildFormation(getMarchId());
		// 队长位置
		...
		// 加入集结的行军
		Set<IYQZZWorldMarch> joinMarchs = this.getMassJoinMarchs(false);
		for (IYQZZWorldMarch joinMarch : joinMarchs) {
		...
		if (formation != null) {
			for (String fightId : formation.getFightIds()) {
				...
				Optional<IYQZZWorldMarch> findAny = joinMarchs.stream().filter(m -> m.getPlayerId().equals(fightId)).findAny();
				...
				YQZZGuildFormationModule module = joinMemeber.getModule(YQZZConst.ModuleType.YQZZGuildFormation);
	...
	public GuildWarTeamInfo.Builder getGuildWarPassivityInfo() {
		...
		List<IYQZZWorldMarch> assistandMarchs = getParent().getParent().getPointMarches(leader.getPointId(),
	...
	public <T extends IYQZZWorldPoint> T getTerminalWorldPoint() {
