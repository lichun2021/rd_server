...
import com.hawk.game.module.lianmengXianquhx.invoker.XQHXMoveCityMsgInvoker;
...
public class XQHXWorldModule extends PlayerModule {
	private IXQHXPlayer player;
	private int moveCnt;
	public XQHXWorldModule(IXQHXPlayer player) {
		super(player);
		this.player = player;
	}
...
//	 * 核弹发射
//	 */
//	@ProtocolHandler(code = HP.code.XQHX_NUCLEAR_SEND_C_VALUE)
//	private void onNuclearStart(HawkProtocol protocol) {
//		
//		PBXQHXNuclearStartReq req = protocol.parseProtocol(PBXQHXNuclearStartReq.getDefaultInstance());
//
...
//			player.sendError(protocol.getType(), Status.Error.XQHX_DAODAN_BAN);
//			return;
//		}
//		XQHXNuclearMissileSilo w = player.getParent().getXQHXBuildingByClass(XQHXNuclearMissileSilo.class).get(0);
...
//	 * 现在是双倍积分
//	 */
//	@ProtocolHandler(code = HP.code.XQHX_CHRONO_SEND_REQ_VALUE)
//	private void onChronoStart(HawkProtocol protocol) {
//		
//		PBXQHXChronoStartReq req = protocol.parseProtocol(PBXQHXChronoStartReq.getDefaultInstance());
//
...
//
//		IXQHXWorldPoint worldPoint = player.getParent().getWorldPoint(req.getX(), req.getY()).orElse(null);
//		if (worldPoint == null || !(worldPoint instanceof IXQHXBuilding)) {
//			return;
//		}
//		IXQHXBuilding tarBuild = (IXQHXBuilding) worldPoint;
//		if (tarBuild.getState() != XQHXBuildState.ZHAN_LING || tarBuild instanceof XQHXHeadQuarters) { // 已占不用了
//			return;
//		}
...
//		}
//
//		XQHXChronoSphere w = player.getParent().getXQHXBuildingByClass(XQHXChronoSphere.class).get(0);
//		if (w.underGuildControl(player.getGuildId())) {
...
//	 * 主播查看建筑战斗邮件
//	 */
//	@ProtocolHandler(code = HP.code.XQHX_ANCHOR_LIST_MAIL_C_VALUE)
//	private boolean onListBuildMail(HawkProtocol protocol) {
//		
//		if (!player.isAnchor()) {
//			return false;
//		}
//		PBXQHXAnchorListMailReq req = protocol.parseProtocol(PBXQHXAnchorListMailReq.getDefaultInstance());
//		int posX = req.getX();
//		int posY = req.getY();
//		int type = req.getType();
//		IXQHXBuilding build = (IXQHXBuilding) player.getParent().getWorldPoint(posX, posY).orElse(null);
...
//	}
	...
	private boolean onShareCoordinate(HawkProtocol protocol) {
		...
		ChatParames parames = ChatParames.newBuilder().setChatType(Const.ChatType.CHAT_FUBEN_TEAM).setKey(Const.NoticeCfgId.XQHX_SHARECOORDINATE).setPlayer(player)
	...
	@ProtocolHandler(code = HP.code2.XQHX_SECOND_MAP_C_VALUE)
	...
	private boolean onGetGuildAssistenceInfo(HawkProtocol protocol) {
		...
		IXQHXPlayer tarPlayer = player.getParent().getPlayer(req.getPlayerId());
		if (tarPlayer == null) {
			return false;
		}
		...
		List<WorldMarch> helpMarchList = player.getParent().getPointMarches(tarPlayer.getPointId(),
				...
				WorldMarchType.ASSISTANCE).stream().map(IXQHXWorldMarch::getMarchEntity).collect(Collectors.toList());
	...
	private void getPresidentTowerQuarterInfo(HawkProtocol protocol) {
		...
		IXQHXBuilding weapon = (IXQHXBuilding) player.getParent().getWorldPoint(req.getPosX(), req.getPosY()).orElse(null);
		if (weapon == null) {
			return;
		}
	...
	@ProtocolHandler(code = HP.code2.XQHX_GAME_SYNC_C_VALUE)
	...
	private boolean onPlayerMoveCity(HawkProtocol protocol) {
		...
		if (player.getParent().getStartTime() > HawkTime.getMillisecond()) {
			player.sendError(protocol.getType(), Status.XQHXError.XQHX_PRETIME_CANNOT_MOVE);
			return false;
		}
		...
		// 迁移前城点
		// IXQHXWorldPoint beforePoint = player;
		...
		if (WorldUtil.isRandomMoveCity(moveCityType)) {
			// 随机迁城
			int[] xy = player.getParent().randomFreePoint(player.getParent().randomPoint(), player.getGridCnt());
			// 迁城失败
			if (xy == null) {
		...
		if (player.getCostCityMoveCount() > 0) {
			// 投递回玩家线程：消耗道具
			player.dealMsg(MsgId.MOVE_CITY, new XQHXMoveCityMsgInvoker(player, consumeItems, moveCityType));
		}
	...
	private boolean moveCityCheck(IXQHXPlayer player, int hp, WorldMoveCityReq req, ConsumeItems consumeItems) {
		if (player.isAnchor()) {
			return false;
		}
	 ...
	 * @return
	 */
	private boolean checkMoveCityConsume(IXQHXPlayer player, int hp, int type, ConsumeItems consumeItems, boolean forceMove) {
		if (player.getParent().getCurTimeMil() > player.getNextCityMoveTime()) {
	 ...
	 * @return
	 */
	private boolean randomMoveCityCheck(IXQHXPlayer player, boolean forceMove) {
		// 自己有出征队伍
		...
		// 是否被攻击
		List<IXQHXWorldMarch> marchList = player.getParent().getPointMarches(player.getPointId(), WorldMarchStatus.MARCH_STATUS_MARCH, WorldMarchType.ATTACK_PLAYER);
	 ...
	 * @return
	 */
	private boolean selectMoveCityCheck(IXQHXPlayer player, int hp, WorldMoveCityReq req) {
	 ...
	 * @return
	 */
	private boolean guildMoveCityCheck(IXQHXPlayer player) {
		return false;
	}
	...
	private void onPlayerEnterWorld(HawkProtocol protocol) {
		...
		
		{
			...
			for (IXQHXWorldPoint point : player.getParent().getViewPoints()) {
		...
		if (player.isAnchor()) {// 主播全量
			...
			for (IXQHXWorldMarch march : player.getParent().getWorldMarchList()) {
	 ...
	 * @param player
	 */
	public void onPlayerMove(IXQHXPlayer player, int x, int y) {
		// 当前的
	...
	// /** 手动进入房间 */
	// @ProtocolHandler(code = HP.code.XQHX_JOIN_ROOM_REQ_VALUE)
	// private void onPlayerrJoinRoom(HawkProtocol protocol) {
	// 
	// XQHXJoinRoomMsg msg = XQHXJoinRoomMsg.valueOf(player.player.getParent(),
	// player);
	...
	/** 主动退出 */
	@ProtocolHandler(code = HP.code2.XQHX_QUIT_ROOM_REQ_VALUE)
	private void onPlayerQuitRoom(HawkProtocol protocol) {
		
		player.getParent().quitWorld(player, XQHXQuitReason.LEAVE);
	}
	...
	private boolean onGetOtherPlayerEffect(HawkProtocol protocol) {
		...
		Player otherPlayer = XQHXRoomManager.getInstance().makesurePlayer(playerId);
		if (otherPlayer == null) {
	...
	private boolean onPlayerDetailOtherRequest(HawkProtocol protocol) {
		...
		String playerId = req.getPlayerId();
		// 快照数据
		Player snapshot = XQHXRoomManager.getInstance().makesurePlayer(playerId);
		if (snapshot == null) {
			player.sendError(protocol.getType(), Status.SysError.ACCOUNT_NOT_EXIST);
			return false;
		}
	...
	private void onReqWorldPointDetail(HawkProtocol protocol) {
		...
		IXQHXWorldPoint worldPoint = player.getParent().getWorldPoint(req.getPointX(), req.getPointY()).orElse(null);
		if (worldPoint == null) {
			// 删除点
	...
	private HPGetGuildWarInfoResp.Builder buildGuildWarInfo(IXQHXPlayer player) {
		// 回复协议
		HPGetGuildWarInfoResp.Builder resp = HPGetGuildWarInfoResp.newBuilder();
		// 联盟行军
		List<IXQHXWorldMarch> guildMarchs = player.getParent().getGuildWarMarch(player.getGuildId());
		for (IXQHXWorldMarch guildMarch : guildMarchs) {
			try {
	...
	private HPGetGuildWarInfoResp.Builder buildGuildWarCellInfo(final String marchId, IXQHXPlayer player) {
		HPGetGuildWarInfoResp.Builder resp = HPGetGuildWarInfoResp.newBuilder();
		IXQHXWorldMarch guildMarch = player.getParent().getMarch(marchId);
		if (guildMarch != null) {
	...
	private boolean onGetGuildAssistantInfo(HawkProtocol protocol) {
		...
		List<IXQHXWorldMarch> marchList = player.assisReachMarches();
		...
		if (marchList != null) {
			...
			for (IXQHXWorldMarch march : marchList) {
				...
				IXQHXPlayer assistPlayer = march.getParent();
