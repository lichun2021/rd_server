...
import com.hawk.game.module.lianmenxhjz.battleroom.player.module.XHJZArmyModule;
import com.hawk.game.module.lianmenxhjz.battleroom.player.module.XHJZMarchModule;
import com.hawk.game.module.lianmenxhjz.battleroom.player.module.XHJZWorldModule;
...
public class XHJZPlayer extends IXHJZPlayer {
	private long lastTickTime;
	private Player source;

	private boolean inited;
	private long nextLoghonor;
	private int lastSyncFuel;
	
	public XHJZPlayer(Player player) {
		super(player.getXid());
	...
	public void init() {
		...
		setEye(new XHJZPlayerEye(this));
		int maxCityDef = source.getData().getRealMaxCityDef();
		this.setMaxCityDef(maxCityDef);
		this.setCityDefVal(maxCityDef);
		this.setPlayerData(XHJZPlayerData.valueOf(this));
		XHJZPlayerPush playerPush = new XHJZPlayerPush(this);
		this.setPlayerPush(playerPush);
		getParent();
		this.nextLoghonor = getParent().getCurTimeMil() + XHJZConst.MINUTE_MICROS;
	...
	public void initModules() {
		registerModule(XHJZConst.ModuleType.XHJZWorld, new XHJZWorldModule(this));
		registerModule(XHJZConst.ModuleType.XHJZMarch, new XHJZMarchModule(this));
		registerModule(XHJZConst.ModuleType.XHJZArmy, new XHJZArmyModule(this));
		// 联盟编队
		registerModule(XHJZConst.ModuleType.XHJZGuildFormation, new XHJZGuildFormationModule(this));
	}
	...
	public boolean onTick() {
		...
		if (now - lastTickTime > 1000) {
			...
			setXhjzState(XHJZState.GAMEING);
			...
			XHJZGuildBaseInfo baseInfo = getParent().getCampBase(getCamp());
			double afterfuel = Math.min(getGasoline() + baseInfo.getPlayerFuelAdd(), getMaxGasoline());
			setGasoline(afterfuel);

			int gasoline = (int) getGasoline();
			if (lastSyncFuel != gasoline) {
				getPush().syncXHJZPlayerInfo();
				lastSyncFuel = gasoline;
			}
			...
			if (getOnFireEndTime() > now) {
				...
				// .setChatType(ChatType.CHAT_XHJZ_SYS)
				// .setKey(NoticeCfgId.XHJZ_CITY_DEF_30)
				// .addParms(getName())
				// .build();
		...
		if (now > nextLoghonor) {
			nextLoghonor = now + XHJZConst.MINUTE_MICROS;
		}
	...
	public void checkSuperVipSkinToMassMarch() {
		try {
			...
			List<IXHJZWorldMarch> marchs = getParent().getPlayerMarches(getId());
...
//		newBuilder.setIsXHJZAnchor(false);
		sendProtocol(HawkProtocol.valueOf(HP.code.PLAYER_INFO_SYNC_S, HPPlayerInfoSync.newBuilder().setPlayerInfo(newBuilder)));

		source.setXhjzRoomId("");
		source.setXhjzState(null);
		source.getPush().syncPlayerInfo();

		XHJZRoomManager.getInstance().invalidate(this);
	}

	@Override
	public String getXhjzRoomId() {
		return source.getXhjzRoomId();
	}

	@Override
	public void setXhjzRoomId(String tblyRoomId) {
		source.setXhjzRoomId(tblyRoomId);
	}

	@Override
	public XHJZState getXhjzState() {
		return source.getXhjzState();
	}

	@Override
	public void setXhjzState(XHJZState tblyState) {
		source.setXhjzState(tblyState);
	}
	...
	public boolean sendProtocol(HawkProtocol protocol) {
		if (Objects.equals(getXhjzRoomId(), getParent().getXid().getUUID())) {
