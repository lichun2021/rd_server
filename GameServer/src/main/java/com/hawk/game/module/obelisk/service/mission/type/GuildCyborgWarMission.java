 ...
 * @author hf
 */
@ObeliskMission(missionType = ObeliskMissionType.GUIlD_CYBORG_WAR)
public class GuildCyborgWarMission implements IObeliskMission {

	@Override
	public Class<? extends MissionEvent> getEventClassType() {
		return EventCyborgWar.class;
	}
	...
	public void refreshMission(Player player, ObeliskMissionItem missionItem, MissionEvent missionEvent) {
		...
		HawkLog.logPrintln("GuildCyborgWarMission refreshMission playerId:{}, guildId:{}", player.getId(), guildId);
	...
	public void calculateContribution(Player player, ObeliskCfg cfg) {
		...
		HawkLog.logPrintln("GuildCyborgWarMission calculateContribution playerId:{},afterContribution:{}", player.getId(), obeliskEntity.getContribution());
	}
	...
	public Builder buildPbToPlayer(Player player, ObeliskMissionItem missionItem) {
		...
		if (missionItem.getState() == Obelisk.PBObeliskMissionState.NOTOPEN) {
			...
			if (obeliskEntity.getState().getNumber() < Obelisk.PBObeliskPlayerState.FAILED.getNumber()) {
				...
				HawkLog.logPrintln("GuildCyborgWarMission buildPbToPlayer end playerId:{},state:{}", player.getId(), obeliskEntity.getState().getNumber());
			}
