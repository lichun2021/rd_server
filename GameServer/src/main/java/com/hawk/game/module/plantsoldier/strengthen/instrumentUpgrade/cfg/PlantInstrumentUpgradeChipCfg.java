...
@HawkConfigManager.XmlResource(file = "xml/plant_instrument_upgrade_chip.xml")
public class PlantInstrumentUpgradeChipCfg extends HawkConfigBase {
	...
	public PlantInstrumentUpgradeChipCfg() {
		id = 0;
		level = 0;
		effect = "";
		frontStage = 0;
	...
	protected boolean checkValid() {
		if (frontStage != 0) {
			PlantInstrumentUpgradeChipCfg cfg = HawkConfigManager.getInstance().getConfigByKey(PlantInstrumentUpgradeChipCfg.class, frontStage);
			if (cfg == null) {
		...
		if (postStage != 0) {
			PlantInstrumentUpgradeChipCfg cfg = HawkConfigManager.getInstance().getConfigByKey(PlantInstrumentUpgradeChipCfg.class, postStage);
			if (cfg == null) {
