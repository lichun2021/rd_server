 ...
 * @author hf
 */
@ObeliskMission(missionType = ObeliskMissionType.GUIlD_PRESIDENT)
public class GuildPresidentMission implements IObeliskMission{

	@Override
	public Class<? extends MissionEvent> getEventClassType(){
		return EventControlPresident.class;
	}
	...
	public void onTick(ObeliskMissionItem missionItem) {
		...
		if (missionItem.getState() == Obelisk.PBObeliskMissionState.OPEN) {
			String guildId = PresidentFightService.getInstance().getPresidentGuildId();
			if (StringUtils.isNotEmpty(guildId) && missionItem.getGuildValue(guildId) == 0) {
				// 满足条件数量加一
	...
	public void refreshMission(Player player, ObeliskMissionItem missionItem, MissionEvent missionEvent) {
		...
		HawkLog.logPrintln("GuildPresidentMission refreshMission playerId:{}, guildId:{}", player.getId(), guildId);
	...
	public Builder buildPbToPlayer(Player player, ObeliskMissionItem missionItem) {
		...
		if (missionItem.getState() == Obelisk.PBObeliskMissionState.NOTOPEN) {
			...
			if (obeliskEntity.getState().getNumber() < Obelisk.PBObeliskPlayerState.FAILED.getNumber()) {
				...
				HawkLog.logPrintln("GuildPresidentMission buildPbToPlayer end playerId:{},state:{}", player.getId(), obeliskEntity.getState().getNumber());
			}
