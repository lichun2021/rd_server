...
import com.hawk.game.protocol.Mail.PBDetectXQHXBuilding;
...
public class XQHXSpyMarch extends IXQHXWorldMarch implements IXQHXReportPushMarch {

	public XQHXSpyMarch(IXQHXPlayer parent) {
		super(parent);
	}
	...
	public void onMarchReach(Player parent) {
		...
		else if (point instanceof XQHXPylon) {
			doPlyonPointSpy((XQHXPylon) point);
		}
		onMarchReturn(getMarchEntity().getTerminalId(), getMarchEntity().getOrigionId(), Collections.emptyList());
	}
	...
	/** 侦查能量塔行军 */
	private void doPlyonPointSpy(XQHXPylon point) {
		XQHXPylonMarch oldMarch = point.getCollectMarch();
		if (oldMarch == null) {
			return;
		}
		...
		MailParames.Builder playerParamesBuilder = MailParames.newBuilder().setPlayerId(player.getId())
				.setMailId(MailId.DETECT_XQHX_PLYON_SUCCESS)
				.addSubTitles(point.getX(), point.getY())
				.addTips(getParent().getName())
				.addContents(DetectMail.newBuilder().setDetectTHRes(builder)).setDuntype(DungeonMailType.XQHX);
		...
		for (IWorldMarch tempMarch : quarteredMarchs) {
			FightMailService.getInstance().sendMail(MailParames.newBuilder()
					...
					.setMailId(MailId.DETECT_XQHX_PLYON_BE)
					.addSubTitles(point.getX(), point.getY())
	...
	private void doBuildSpy(IXQHXWorldPoint point) {
		// XQHX_SPY_BUILD_SUCCESS = 2020009;//泰伯利亚建筑侦察成功 2020009
		// XQHX_SPY_BUILD_FAIL = 2020010;//泰伯利亚建筑侦察失败 2020010
		// XQHX_SPY_BUILD_BE = 2020011; //泰伯利亚建筑被侦察 2020011
		IXQHXBuilding build = (IXQHXBuilding) point;
		List<IXQHXWorldMarch> quarteredMarchs = getParent().getParent().getPointMarches(getMarchEntity().getTerminalId(), WorldMarchStatus.MARCH_STATUS_MARCH_QUARTERED);
		...
		if (quarteredMarchs.get(0).getParent().getCamp() == getParent().getCamp()) {
			...
			// .setMailId(MailId.XQHX_SPY_BUILD_FAIL)
			// .build());
			return;
		}
		...
		PBDetectXQHXBuilding.Builder builder = PBDetectXQHXBuilding.newBuilder().setLevel(radarLevel).setPointX(point.getX()).setPointY(point.getY());
		...
		// 侦查邮件
		MailParames.Builder playerParamesBuilder = MailParames.newBuilder().setPlayerId(getParent().getId()).setMailId(MailId.XQHX_SPY_BUILD_SUCCESS)
				.addSubTitles(point.getX(), point.getY())
				.setDuntype(DungeonMailType.XQHX)
				.addContents(DetectMail.newBuilder().setXqhxBuild(builder)).addTips(point.getX()).addTips(point.getY());
		...
		for (IWorldMarch tempMarch : quarteredMarchs) {
			FightMailService.getInstance()
					.sendMail(MailParames.newBuilder().setPlayerId(tempMarch.getPlayerId()).setMailId(MailId.XQHX_SPY_BUILD_BE).addSubTitles(point.getX(), point.getY())
	...
	private void doPlayerSpy(IXQHXWorldPoint point) {
		IXQHXPlayer tarPlayer = (IXQHXPlayer) point;
	 ...
	 * @return
	 */
	public boolean sendDetectMail(Player player, Player targetPlayer, IXQHXWorldPoint point) {
		...
		MailParames.Builder playerParamesBuilder = MailParames.newBuilder().setPlayerId(player.getId()).addSubTitles(subTitle).addContents(builder)
				...
				.addTips(targetPlayer.getName()).setDuntype(DungeonMailType.XQHX)
		...
		FightMailService.getInstance().sendMail(playerParamesBuilder.setDuntype(DungeonMailType.XQHX).build());

		FightMailService.getInstance().sendMail(targetParamesBuilder.setDuntype(DungeonMailType.XQHX).build());
		return true;
	}

	public DetectMail.Builder createDetectMail(final int radarLevel, Player player, Player targetPlayer, IXQHXWorldPoint point, boolean isMirror) {
		...
		if (pointType == WorldPointType.RESOURCE_VALUE || pointType == WorldPointType.QUARTERED_VALUE) {
			// 资源点 驻扎点
			List<IXQHXWorldMarch> marchs = getParent().getParent().getPointMarches(point.getPointId(), WorldMarchStatus.MARCH_STATUS_MARCH_QUARTERED);
			if (!marchs.isEmpty()) {
				leaderMarch = marchs.get(0);
			}
		} else if (pointType == WorldPointType.PLAYER_VALUE) {
			// 玩家城堡---基地
			List<IXQHXWorldMarch> massMarches = getParent().getParent().getPlayerMarches(targetPlayer.getId(), WorldMarchStatus.MARCH_STATUS_WAITING);
			if (!massMarches.isEmpty()) {
				leaderMarch = massMarches.get(0);
			}
			List<IXQHXWorldMarch> marchs = getParent().getParent().getPointMarches(point.getPointId(), WorldMarchStatus.MARCH_STATUS_MARCH_ASSIST);
		...
		if (pointType == WorldPointType.PLAYER_VALUE && !targetPlayer.getData().getArmyEntities().isEmpty()) {
			...
			if (radarLevel >= GsConst.RadarLevel.LV21 && leaderMarch != null) {
				List<IXQHXWorldMarch> massMarches = getParent().getParent().getPointMarches(point.getPointId(), WorldMarchStatus.MARCH_STATUS_MARCH,
		...
		if (pointType == WorldPointType.PLAYER_VALUE) {
			...
			// 藏兵洞
			List<IXQHXWorldMarch> hiddenMarchs = getParent().getParent().getPointMarches(point.getPointId(), WorldMarchType.HIDDEN_MARCH);
			if (radarLevel >= GsConst.RadarLevel.LV10) {
	...
	public Set<String> attackReportRecipients() {
		// 防守方援军
		List<IXQHXWorldMarch> helpMarchList = getParent().getParent().getPointMarches(getMarchEntity().getTerminalId(), WorldMarchStatus.MARCH_STATUS_MARCH_ASSIST,
