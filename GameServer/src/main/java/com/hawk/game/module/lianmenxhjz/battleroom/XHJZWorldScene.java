...
import com.hawk.game.module.lianmenxhjz.battleroom.player.XHJZPlayerEye;
 ...
 * @date 2020年9月14日
 */
public class XHJZWorldScene extends HawkAOIScene {
	 ...
	 * @param maxRadius
	 */
	public XHJZWorldScene(int width, int height, int searchRadius) {
	...
	protected void notifyObjEnter(HawkAOIObj aoiObj, Set<HawkAOIObj> targetObjs) {
		...
		IXHJZWorldPoint worldPoint = (IXHJZWorldPoint) aoiObj.getUserData();
		if (worldPoint == null) {
			return;
		}
		...
		for (HawkAOIObj targetObj : targetObjs) {
			if (targetObj.getType() == WorldObjType.PLAYER) {
				XHJZPlayerEye playerEye = (XHJZPlayerEye) targetObj.getUserData();
				if (playerEye == null) {
					continue;
				}
				builder.clearPoints();
	...
	protected void syncSceneObjs(HawkAOIObj aoiObj, Set<HawkAOIObj> oldInviewObjs, Set<HawkAOIObj> nowInviewObjs, float moveSpeed) {
		if (aoiObj.getType() == GsConst.WorldObjType.PLAYER && GsApp.getInstance().isRunning()) {
			XHJZPlayerEye playerEye = (XHJZPlayerEye) aoiObj.getUserData();
			if (playerEye == null) {
				return;
			}
			...
			for (HawkAOIObj inviewObj : inviewObjs) {
				try {
					...
					IXHJZWorldPoint worldPoint = (IXHJZWorldPoint) inviewObj.getUserData();
					if (worldPoint == null) {
						continue;
					}
	 ...
	 * @return
	 */
	public List<WorldPointPB.Builder> getPlayerViewObjs(XHJZPlayerEye viewer, int x, int y, float moveSpeed) {
	 ...
	 * @return
	 */
	private int fillWorldPoints(XHJZPlayerEye viewer, Set<HawkAOIObj> inviewObjs, float moveSpeed, List<WorldPointPB.Builder> builderList) {
		float randFactor = 1.0f;
		if (!HawkOSOperator.isZero(GameConstCfg.getInstance().getMoveSyncFactor())) {
			randFactor = (float) Math.pow(GameConstCfg.getInstance().getMoveSyncFactor() - moveSpeed, 2);
		}
