 ...
 * @date 2023年8月14日
 */
public class TBLYOrder10003 extends TBLYOrder {
	...
	public void onTick() {
		...
		if (target != null) {
			if (!inEffect() || target.getGuildCamp() != getParent().getCamp()) {
				target.getShowOrder().remove(TBLYOrderCollection.Order10003);
	...
	public void startOrder(PBTBLYOrderUseReq req) {
		...
		PBTBLYBuildSkill buff = PBTBLYBuildSkill.newBuilder()
				.setSkillId(TBLYOrderCollection.Order10003)
				...
				.build();
		target.getShowOrder().put(TBLYOrderCollection.Order10003, buff);
		getParent().getParent().worldPointUpdate(target);
		ChatParames parames = ChatParames.newBuilder().setChatType(ChatType.CHAT_FUBEN_SPECIAL_BROADCAST).setKey(NoticeCfgId.TBLY_CHIYUAN)
