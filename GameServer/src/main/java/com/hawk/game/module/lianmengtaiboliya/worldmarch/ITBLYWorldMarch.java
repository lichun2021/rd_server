...
import com.hawk.game.module.lianmengtaiboliya.order.TBLYOrder10002;
...
public abstract class ITBLYWorldMarch implements IWorldMarch {

	private final ITBLYPlayer parent;
	private TBLYMarchEntity marchEntity;
	private int hashThread;
	public ITBLYWorldMarch(ITBLYPlayer parent) {
		this.parent = parent;
	}

	public final ITBLYPlayer getParent() {
		return parent;
	}

	@Override
	public TBLYMarchEntity getMarchEntity() {
		return marchEntity;
	}

	public void setMarchEntity(TBLYMarchEntity marchEntity) {
	...
	public WorldMarchStatus getMassJoinMarchStatus(String playerId) {
		try {
			...
			Set<ITBLYWorldMarch> massJoinMarchs = getMassJoinMarchs(false);
	...
	public GuildWarShowPB getGuildWarShoPb(ITBLYPlayer player) {
		// 行军状态
		...
		try {
			TBLYGuildFormationObj formationObj = getParent().getParent().getGuildFormation(player.getGuildId());
	...
	public WorldMarchPB.Builder toBuilder(WorldMarchRelation relation) {
		...
		TBLYMarchEntity entity = getMarchEntity();
	...
	public boolean speedUp(int timeReducePercent, int speedUpTimes) {
		...
		if (this instanceof ITBLYReportPushMarch) {
			((ITBLYReportPushMarch) this).reachTimeChange();
		}
		return true;
	}

	@Override
	public Set<ITBLYWorldMarch> getQuarterMarch() {
		if (getMarchStatus() != WorldMarchStatus.MARCH_STATUS_MARCH_QUARTERED_VALUE) {
			return Collections.emptySet();
		}
		List<ITBLYWorldMarch> defMarchList = getParent().getParent().getPointMarches(getTerminalId(), WorldMarchStatus.MARCH_STATUS_MARCH_QUARTERED);
	...
	public void pushMarchEvent(MarchEvent eventType) {
		for (ITBLYPlayer player : getParent().getParent().getPlayerList(TBLYState.GAMEING)) {
			int[] aoiObj = player.getWorldMovePos();
			if (Objects.isNull(aoiObj)) {
				continue;
			}
			if (!isInview(aoiObj[0], aoiObj[1])) {
				continue;
			}
		...
		// 发送主播
		for (ITBLYPlayer anchor : getParent().getParent().getAnchors()) {
	 ...
	 * @return
	 */
	public WorldMarchRelation getRelation(ITBLYPlayer tarplayer) {
		...
		// 目标点
		Optional<ITBLYWorldPoint> targetPointOp = getParent().getParent().getWorldPoint(marchEntity.getTerminalId());
		...
		ITBLYWorldPoint targetPoint = targetPointOp.get();
	...
	public void sendBattleResultInfo(boolean isWin, List<ArmyInfo> atkArmyList, List<ArmyInfo> defArmyList, boolean isMonsterDead) {
		...
		List<ITBLYPlayer> players = getParent().getParent().getPlayerList(TBLYState.GAMEING);
	...
	public void updateDefMarchAfterWar(List<ITBLYWorldMarch> helpMarchList, Map<String, List<ArmyInfo>> defArmyMap) {
		// 更新援助防御玩家行军的部队
		for (ITBLYWorldMarch tmpMarch : helpMarchList) {
			...
			if (tmpMarch.getMarchType() == WorldMarchType.ASSISTANCE) {
				...
				ITBLYPlayer assistPlayer = tmpMarch.getParent();
	...
	public boolean onMarchReturn(int origionId, int terminalId, List<ArmyInfo> selfArmys) {
		...
		TBLYOrderCollection orderCollection = getParent().getParent().getTBLYOrderCollection(getParent().getCamp());
		TBLYOrder10002 order10002 = orderCollection.getOrderById(TBLYOrderCollection.Order10002);
		if (order10002 != null && order10002.inEffect() && getParent().getParent().getWorldPoint(origionId).orElse(null) instanceof ITBLYBuilding) {
			marchTime = 3000;
		}
	...
	private double effectToMarchTime(double time, boolean slowDown, double slowDownTime) {
		...
		if (this.isMassJoinMarch()) {
			Optional<ITBLYWorldPoint> targetPoint = getParent().getParent().getWorldPoint(marchEntity.getTerminalId());
			if (targetPoint.isPresent()) {
				ITBLYPlayer leader = (ITBLYPlayer) targetPoint.get();
	...
	public double getMarchBaseSpeed() {
		...
		if (isMassMarch()) {
			Set<ITBLYWorldMarch> joinMarchs = getMassJoinMarchs(true);
			for (ITBLYWorldMarch tempMarch : joinMarchs) {
	...
	public GuildWarTeamInfo.Builder getGuildWarInitiativeInfo() {
		...
		// 联盟编队
		TBLYGuildFormationObj formationObj = getParent().getParent().getGuildFormation(leader.getGuildId());
		GuildFormationCell formation = formationObj.getGuildFormation(getMarchId());
		// 队长位置
		...
		// 加入集结的行军
		Set<ITBLYWorldMarch> joinMarchs = this.getMassJoinMarchs(false);
		for (ITBLYWorldMarch joinMarch : joinMarchs) {
		...
		if (formation != null) {
			for (String fightId : formation.getFightIds()) {
				...
				Optional<ITBLYWorldMarch> findAny = joinMarchs.stream().filter(m -> m.getPlayerId().equals(fightId)).findAny();
				...
				TBLYGuildFormationModule module = joinMemeber.getModule(TBLYConst.ModuleType.TBLYGuildFormation);
	...
	public GuildWarTeamInfo.Builder getGuildWarPassivityInfo() {
		...
		ITBLYPlayer leader = TBLYRoomManager.getInstance().makesurePlayer(leaderId);
		// 队长位置
		...
		List<ITBLYWorldMarch> assistandMarchs = getParent().getParent().getPointMarches(leader.getPointId(), WorldMarchStatus.MARCH_STATUS_MARCH_ASSIST,
				WorldMarchType.ASSISTANCE);
		for (ITBLYWorldMarch assistandMarch : assistandMarchs) {
...
//		int tpointId = getMarchStatus() == WorldMarchStatus.MARCH_STATUS_RETURN_BACK_VALUE ? getOrigionId() : getTerminalId();
//		ITBLYWorldPoint tar = getParent().getParent().getWorldPoint(tpointId).orElse(null);
//		if (tar != null) {
//			return tar.getHashThread(threadNum);
//		}
//
//		return Math.abs(tpointId % threadNum);
		return hashThread;
	}
