...
public class XQHXAttackPlayerMarch extends IXQHXWorldMarch implements IXQHXReportPushMarch {

	public XQHXAttackPlayerMarch(IXQHXPlayer parent) {
		super(parent);
	}
	...
	public void onMarchReach(Player parent) {
		...
		IXQHXPlayer defplayer = (IXQHXPlayer) point;
		...
		atkMarchs.add(this);
		// 防守方援军
		List<IXQHXWorldMarch> helpMarchList = helpMarches();
		// 防守方行军
		...
		for (Player defer : defPlayers) {
			((IXQHXPlayer) defer).setLastAttacker(getParent());
		}
	}
	...
	private List<IXQHXWorldMarch> helpMarches() {
