...
public class YQZZPylonRefesh {

	private List<HawkTuple2<Integer, YQZZPylonCfg>> fuelBankRefreshTimes = new LinkedList<>();
	private YQZZBattleRoom parent;

	private boolean yugaoNotSend = true;

	private List<YQZZPylonCreater> createrList = new CopyOnWriteArrayList<>();
	private int refreshTurn; // 已经刷新的波数

	private YQZZPylonRefesh() {
	}

	public static YQZZPylonRefesh create(YQZZBattleRoom parent) {
		YQZZPylonRefesh result = new YQZZPylonRefesh();
		result.parent = parent;

		ConfigIterator<YQZZPylonCfg> cfgs = HawkConfigManager.getInstance().getConfigIterator(YQZZPylonCfg.class);
		for (YQZZPylonCfg cfg : cfgs) {
	...
	public void onTick() {
		createrList.forEach(YQZZPylonCreater::onTick);
		...
		YQZZPylonCfg mcfg = fuelBankRefreshTimes.remove(0).second;
		createrList.add(YQZZPylonCreater.create(this, mcfg));
		refreshTurn++;
		// if (!fuelBankRefreshTimes.isEmpty()) {
