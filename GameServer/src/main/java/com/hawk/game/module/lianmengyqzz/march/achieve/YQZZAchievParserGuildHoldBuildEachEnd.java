...
public class YQZZAchievParserGuildHoldBuildEachEnd extends IYQZZAchievParser{

	@Override
	public YQZZAchieveType getYQZZAchieveType() {
		return YQZZAchieveType.GUILD_HOLD_BUILDING_EACH;
	}
	...
	public boolean updateAchieveOnUpdate(YQZZAchievecomponent achieve,List<Long> conditionValueList,long targetValue) {
		...
		for(long btype : conditionValueList){
			int bcount = data.getGuildHoldBuildsById(guildId,(int)btype);
			if(curVal < 0){
				curVal = bcount;
			}
			if(curVal > bcount){
