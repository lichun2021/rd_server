...
import com.hawk.game.module.lianmengfgyl.battleroom.player.module.FGYLArmyModule;
import com.hawk.game.module.lianmengfgyl.battleroom.player.module.FGYLMarchModule;
import com.hawk.game.module.lianmengfgyl.battleroom.player.module.FGYLWorldModule;
...
public class FGYLPlayer extends IFGYLPlayer {
	...
	public FGYLPlayer(Player player) {
		super(player.getXid());
	...
	public void init() {
		...
		setEye(new FGYLPlayerEye(this));
		int maxCityDef = source.getData().getRealMaxCityDef();
		this.setMaxCityDef(maxCityDef);
		this.setCityDefVal(maxCityDef);
		this.setPlayerData(FGYLPlayerData.valueOf(this));
		FGYLPlayerPush playerPush = new FGYLPlayerPush(this);
		this.setPlayerPush(playerPush);
		getParent();
		this.nextLoghonor = getParent().getCurTimeMil() + FGYLConst.MINUTE_MICROS;
		this.checkEquipResearchUnlock();
		try {
			PlantSoldierSchool plantSchoolObj = getData().getPlantSoldierSchoolEntity().getPlantSchoolObj();
			this.setPlantMilitaryLevel(plantSchoolObj.getMaxSoldierPlantMilitaryLevel());	
		} catch (Exception e) {
	...
	public void initModules() {
		registerModule(FGYLConst.ModuleType.FGYLWorld, new FGYLWorldModule(this));
		registerModule(FGYLConst.ModuleType.FGYLMarch, new FGYLMarchModule(this));
		registerModule(FGYLConst.ModuleType.FGYLArmy, new FGYLArmyModule(this));
		// 联盟编队
		registerModule(FGYLConst.ModuleType.FGYLGuildFormation, new FGYLGuildFormationModule(this));
	}
	...
	public boolean onTick() {
		...
		if (now - lastTickTime > 1000) {
			...
			setFgylState(FGYLState.GAMEING);
			...
			if (getOnFireEndTime() > now) {
				...
				// .setChatType(ChatType.CHAT_FGYL_SYS)
				// .setKey(NoticeCfgId.FGYL_CITY_DEF_30)
				// .addParms(getName())
				// .build();
		...
		if (now > nextLoghonor) {
			nextLoghonor = now + FGYLConst.MINUTE_MICROS;
		}
	...
	public void checkSuperVipSkinToMassMarch() {
		try {
			...
			OptionalLong optional = getParent().getPlayerMarches(getId(), WorldMarchStatus.MARCH_STATUS_WAITING).stream()
	...
	public void quitGame() {
		...
		// newBuilder.setIsFGYLAnchor(false);
		sendProtocol(HawkProtocol.valueOf(HP.code.PLAYER_INFO_SYNC_S, HPPlayerInfoSync.newBuilder().setPlayerInfo(newBuilder)));

		source.setFgylRoomId("");
		source.setFgylState(null);
		source.getPush().syncPlayerInfo();

		FGYLRoomManager.getInstance().invalidate(this);
	}

	@Override
	public String getFgylRoomId() {
		return source.getFgylRoomId();
	}

	@Override
	public void setFgylRoomId(String tblyRoomId) {
		source.setFgylRoomId(tblyRoomId);
	}

	@Override
	public FGYLState getFgylState() {
		return source.getFgylState();
	}

	@Override
	public void setFgylState(FGYLState tblyState) {
		source.setFgylState(tblyState);
	}
	...
	public boolean sendProtocol(HawkProtocol protocol) {
		if (Objects.equals(getFgylRoomId(), getParent().getXid().getUUID())) {
