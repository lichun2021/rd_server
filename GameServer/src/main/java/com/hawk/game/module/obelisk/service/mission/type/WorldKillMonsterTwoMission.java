 ...
 * @author hf
 */
@ObeliskMission(missionType = ObeliskMissionType.WORLD_KILL_LEVEL_MONSTER_TWO)
public class WorldKillMonsterTwoMission implements IObeliskMission {
	...
	public void refreshMission(Player player,ObeliskMissionItem missionItem, MissionEvent missionEvent){
		...
		HawkLog.logPrintln("WorldKillMonsterTwoMission refreshMission playerId:{}, monsterLv:{}", player.getId(), eventMonsterAttack.getLevel());
	}
	...
	public void calculateContribution(Player player,ObeliskCfg cfg) {
		...
		HawkLog.logPrintln("WorldKillMonsterTwoMission calculateContribution playerId:{},afterContribution:{}", player.getId(), obeliskEntity.getContribution());
	}
	...
	public PBObelisk.Builder buildPbToPlayer(Player player, ObeliskMissionItem missionItem) {
		...
		if (missionItem.getState() == PBObeliskMissionState.NOTOPEN){
			...
			if (obeliskEntity.getState().getNumber() < PBObeliskPlayerState.FAILED_VALUE){
				...
				HawkLog.logPrintln("WorldKillMonsterTwoMission buildPbToPlayer end change state playerId:{},obeliskNum:{},state:{}", player.getId(), obeliskNum,obeliskEntity.getState().getNumber());
			}
