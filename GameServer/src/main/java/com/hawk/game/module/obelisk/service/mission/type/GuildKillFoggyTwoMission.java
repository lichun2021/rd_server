 ...
 * @author hf
 */
@ObeliskMission(missionType = ObeliskMissionType.GUIlD_KILL_FOGGY2)
public class GuildKillFoggyTwoMission implements IObeliskMission {
	...
	public void refreshMission(Player player, ObeliskMissionItem missionItem, MissionEvent missionEvent) {
		...
		HawkLog.logPrintln("GuildKillFoggyTwoMission refreshMission playerId:{}, guildId:{}, foggyLv:{}", player.getId(), guildId, foggyLv);
	}
	...
	public void calculateContribution(Player player, ObeliskCfg cfg) {
		...
		HawkLog.logPrintln("GuildKillFoggyTwoMission calculateContribution playerId:{},afterContribution:{}", player.getId(), obeliskEntity.getContribution());
	}
	...
	public Builder buildPbToPlayer(Player player, ObeliskMissionItem missionItem) {
		...
		if (missionItem.getState() == Obelisk.PBObeliskMissionState.NOTOPEN) {
			...
			if (obeliskEntity.getState().getNumber() < Obelisk.PBObeliskPlayerState.FAILED.getNumber()) {
				...
				HawkLog.logPrintln("GuildKillFoggyTwoMission buildPbToPlayer end playerId:{},state:{}", player.getId(), obeliskEntity.getState().getNumber());
			}
