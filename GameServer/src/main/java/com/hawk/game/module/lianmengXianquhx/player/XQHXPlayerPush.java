...
public class XQHXPlayerPush extends IXQHXPlayerPush {
	...
	public XQHXPlayerPush(Player player) {
		super(player);
	}
	...
	public void syncPlayerInfo() {
		...
		newBuilder.setXqhxCamp(getPlayer().getCamp().intValue());
	...
	public void pushJoinGame() {
		// XQHXBattleRoom battleRoom = getPlayer().getParent();
		XQHXPlayerData playerData = (XQHXPlayerData) getData();
//		playerData.unLockOriginalData();
		// 隐藏真实数据
		getPlayer().sendProtocol(HawkProtocol.valueOf(HP.code2.XQHX_CLEAN_CLAIENT_DATA_VALUE));
		...
		getPlayer().refreshPowerElectric(PowerChangeReason.XQHX_INIT);
		// // 世办点开罩子
	...
	public void pushGameOver() {
		...
		// XQHXBattleRoom battleRoom = getPlayer().getParent();
		source.refreshPowerElectric(PowerChangeReason.XQHX_INIT);
		getPlayer().quitGame();
		source.sendProtocol(HawkProtocol.valueOf(HP.code2.XQHX_CLEAN_CLAIENT_DATA_VALUE));
		// 清除军演数据
		...
		// }
		// }
		XQHXQuitRoomMsg msg = XQHXQuitRoomMsg.valueOf(getPlayer().getQuitReason());
		...
		DungeonRedisLog.log(getPlayer().getId(), "{} kill {} loseTank {} collect {} honor {}", 
				...
				getPlayer().getHurtTankHonor(),
				getPlayer().getMonsterHonor(),
				getPlayer().getHonor());
	}

	public XQHXPlayer getPlayer() {
		return (XQHXPlayer) player;
	}

	@Override
	public IXQHXPlayerData getData() {
		return (IXQHXPlayerData) player.getData();
	}
