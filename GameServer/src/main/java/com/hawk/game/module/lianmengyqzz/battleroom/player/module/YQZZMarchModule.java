...
public class YQZZMarchModule extends PlayerModule {
	private IYQZZPlayer player;

	public YQZZMarchModule(IYQZZPlayer player) {
		super(player);
		this.player = player;
	}
	...
	private boolean onWorldPylonMarchStart(HawkProtocol protocol) {
		...
		// 目标点
		IYQZZWorldPoint ipoint = player.getParent().getWorldPoint(req.getPosX(), req.getPosY()).orElse(null);

		// 坐标点检查
		if (ipoint == null || ipoint.getPointType() != WorldPointType.PYLON) {
			sendError(protocol.getType(), Status.Error.WORLD_POINT_NOT_EXIST);
			return false;
		}
		List<IYQZZWorldMarch> playerMarches = player.getParent().getPlayerMarches(player.getId(), WorldMarchType.PYLON_MARCH);
		if (!playerMarches.isEmpty() && playerMarches.get(0).getMarchStatus() != WorldMarchStatus.MARCH_STATUS_RETURN_BACK_VALUE) {
			sendError(protocol.getType(), Status.YQZZError.YQZZ_PYLON_MARCH_LIMIT);
			return false;
		}
		
		YQZZPylon point = (YQZZPylon) ipoint;
		// 所需体力
		...
		// 生成行军
		IYQZZWorldMarch worldMatch = player.getParent().startMarch(player, player, point, WorldMarchType.PYLON_MARCH, point.getResourceId()+"", 0, new EffectParams(req, armyList));
		
		((YQZZPylonMarch)worldMatch).setVitBack(cfg.getStrongpointCost());
		...
		YQZZPylonHonor mhonor = player.getPylonHonorStat(cfg.getId());
		mhonor.setPylonMarchCount(mhonor.getPylonMarchCount() + 1);
	...
	private boolean onWorldAttackMonsterStart(HawkProtocol protocol) {
		...
		// 目标点
		IYQZZWorldPoint targetPoint = player.getParent().getWorldPoint(posX, posY).orElse(null);
		...
		IYQZZWorldMarch worldMatch = player.getParent().startMarch(player, player, targetPoint, WorldMarchType.ATTACK_MONSTER, targetId, 0, new EffectParams(req, armyList));
		if (worldMatch == null) {
			return false;
		}
	...
	public boolean marchEmoticonUse(HawkProtocol protocol, int emoticonId, String marchId) {
		IYQZZWorldMarch march = player.getParent().getMarch(marchId);
		if (march == null) {
		...
		if (march.getMarchStatus() != WorldMarchStatus.MARCH_STATUS_MARCH_VALUE
			...
			IYQZZWorldPoint worldPoint = player.getParent().getWorldPoint(march.getTerminalId()).get();
			if (worldPoint instanceof IYQZZBuilding) {
				WorldMarch firstMarch = ((IYQZZBuilding) worldPoint).getLeaderMarch().getMarchEntity();
	...
	private boolean onInviteMass(HawkProtocol protocol) {
		...
		IYQZZWorldMarch march = player.getParent().getMarch(req.getMarchId());
		if (march == null) {
		...
		if (HawkTime.getMillisecond() - player.getData().getLastInviteMassTime() < WorldMarchConstProperty.getInstance()
				.getInviteMassCD()) {
			player.sendError(protocol.getType(), Status.Error.INVITE_MASS_CD);
			return false;
		}

		Optional<IYQZZWorldPoint> worldPoint = player.getParent().getWorldPoint(march.getMarchEntity().getTerminalId());
		if (worldPoint.isPresent()) {
			YQZZGuildFormationObj formationObj = player.getParent().getGuildFormation(player.getGuildId());
	 ...
	 * 发送公告
	 */
	public void sendNotice(IYQZZWorldPoint point, IYQZZWorldMarch march, int formationIndex, String formationName) {
		IYQZZPlayer player = march.getParent();
		if (point instanceof IYQZZPlayer) {
			ChatParames parames = ChatParames.newBuilder().setPlayer(player).setChatType(ChatType.CHAT_FUBEN_TEAM)
					.setKey(NoticeCfgId.YQZZ_MASS_START_PLAYER).addParms(((IYQZZPlayer) point).getName())
					.addParms(march.getMarchId()).addParms(player.getX()).addParms(player.getY()).addParms(formationIndex).addParms(formationName).build();
			player.getParent().addWorldBroadcastMsg(parames);
		} else if (point instanceof IYQZZBuilding) {
			ChatParames parames = ChatParames.newBuilder().setPlayer(player).setChatType(ChatType.CHAT_FUBEN_TEAM)
					.setKey(NoticeCfgId.YQZZ_MASS_START_BUILD).addParms(point.getX()).addParms(point.getY())
					.addParms(march.getMarchId()).addParms(player.getX()).addParms(player.getY()).addParms(formationIndex).addParms(formationName)
					.build();
			player.getParent().addWorldBroadcastMsg(parames);
		} else if (point.getPointType() == WorldPointType.FOGGY_FORTRESS) {
			YQZZFoggyFortress foggy = (YQZZFoggyFortress) point;
			ChatParames parames = ChatParames.newBuilder().setPlayer(player).setChatType(ChatType.CHAT_FUBEN_TEAM)
					.setKey(NoticeCfgId.YQZZ_FOGGY_MASS).addParms(foggy.getFoggyFortressId())
					.addParms(march.getMarchId()).addParms(player.getX()).addParms(player.getY()).addParms(formationIndex).addParms(formationName)
					.build();
			player.getParent().addWorldBroadcastMsg(parames);
		}
	...
	private void onYuriMonsterQueryHelp(HawkProtocol protocol) {
		...
		// 行军已不存在
		IYQZZWorldMarch worldMarch = player.getParent().getMarch(req.getMarhcId());
		if (worldMarch == null) {
			return;
		}

		Optional<IYQZZWorldPoint> point = player.getParent().getWorldPoint(worldMarch.getTerminalId());
		int pointType = point.isPresent() ? point.get().getPointType().getNumber() : 0;
		ChatParames parames = ChatParames.newBuilder().setPlayer(player).setChatType(ChatType.CHAT_FUBEN_TEAM)
				.setKey(NoticeCfgId.YQZZ_MARCH_QUERY_HELP).addParms(worldMarch.getTerminalX())
				.addParms(worldMarch.getTerminalY()).addParms(pointType).build();
		player.getParent().addWorldBroadcastMsg(parames);
	...
	private boolean onRepatriateManormarch(HawkProtocol protocol) {
		...
		// 世界点
		IYQZZWorldPoint worldPoint = player.getParent().getWorldPoint(req.getX(), req.getY()).orElse(null);
		if (worldPoint == null) {
		...
		// 战区
		if (worldPoint instanceof IYQZZBuilding) {
			IYQZZBuilding building = (IYQZZBuilding) worldPoint;
	...
	protected boolean onWorldMassDissolve(HawkProtocol protocol) {
		...
		// check
		IYQZZWorldMarch worldMarch = player.getParent().getMarch(req.getMarchId());
		if (worldMarch == null) {
			player.sendError(protocol.getType(), Status.Error.WORLD_MARCH_NOT_EXISIT);
			return false;
		}
		...
		for (IYQZZWorldMarch jm : worldMarch.getMassJoinMarchs(true)) {
			jm.onMarchCallback();
			// // 发送邮件---集结失败：发起者解散
			// GuildMailService.getInstance().sendMail(
			// MailParames.newBuilder().setPlayerId(jm.getPlayerId()).setMailId(MailId.MASS_FAILED_ORGANIZER_DISSOLVE).addContents(terminalX,
			// terminalY).setIcon(player.getGuildFlag()).build());
		}
	...
	protected boolean onWorldMassRepatriate(HawkProtocol protocol) {
		...
		IYQZZWorldMarch leaderMarch = player.getParent().getMarch(march.getTargetId());
		if (leaderMarch == null) {
			player.sendError(protocol.getType(), Status.Error.WORLD_MARCH_NOT_LEADER);
			return false;
		}
	...
	/**
	 * YQZZ_SINGLE_MARCH = 487; // 副本内特有建筑单人行军
	 */
	@ProtocolHandler(code = HP.code2.YQZZ_SINGLE_MARCH_VALUE)
	private boolean onYQZZ_SINGLE_MARCHStart(HawkProtocol protocol) {
		WorldMarchReq req = protocol.parseProtocol(WorldMarchReq.getDefaultInstance());
		IYQZZWorldPoint point = player.getParent().getWorldPoint(req.getPosX(), req.getPosY()).orElse(null);
		// 路点为空
		if (point == null) {
			player.sendError(protocol.getType(), Status.Error.WORLD_POINT_NOT_EXIST);
			return false;
		...
		case YQZZ_BUILDING:// 指挥部
			marchType = WorldMarchType.YQZZ_BUILDING_SINGLE;
			break;
		default:
		...
		WorldMarchResp.Builder builder = WorldMarchResp.newBuilder();
		builder.setSuccess(true);
		protocol.response(HawkProtocol.valueOf(HP.code2.YQZZ_SINGLE_MARCH_S, builder));
		return true;
	...
	private boolean onMarchArmyDetailInfo(HawkProtocol protocol) {
		...
		Set<IYQZZWorldMarch> allMarch = new HashSet<>();
		allMarch.add(march);
		...
		// 是否有自己的行军
		for (IYQZZWorldMarch thisMarch : allMarch) {
		...
		for (IYQZZWorldMarch thisMarch : allMarch) {
			...
			IYQZZPlayer mp = thisMarch.getParent();
	...
	private boolean onWorldAttackPlayerStart(HawkProtocol protocol) {
		...
		// 目标玩家和自己同盟
		IYQZZPlayer target = (IYQZZPlayer) point;
		...
		player.getParent().startMarch(player, player, target, WorldMarchType.ATTACK_PLAYER, targetId, 0,
				new EffectParams(req, armyList));
	...
	private boolean onMarchSpeedUp(HawkProtocol protocol) {
		...
		// 加入集结出征，给队长行军加速
		if (march.isMassJoinMarch()
				&& march.getMarchEntity().getMarchStatus() == WorldMarchStatus.MARCH_STATUS_JOIN_MARCH_VALUE) {
		...
		if (itemCfg.getMarchSpeedMultiple() <= 0) {
			player.sendError(protocol.getType(), Status.Error.WORLD_MARCH_NOT_EXISIT);
			return false;
		}
	...
	private boolean onMarchCallBack(HawkProtocol protocol) {
		...
		// 检测行军是否存在
		IYQZZWorldMarch march = player.getParent().getPlayerMarch(player.getId(), req.getMarchId());
		if (march == null) {
	...
	private boolean onPointMarchCallBack(HawkProtocol protocol) {
		...
		if (worldPoint.getPointType() == WorldPointType.PLAYER) {
			// 获取是否有驻军信息
			IYQZZWorldMarch march = player.getParent()
					.getPointMarches(GameUtil.combineXAndY(req.getX(), req.getY()),
							WorldMarchStatus.MARCH_STATUS_MARCH_ASSIST, WorldMarchType.ASSISTANCE)
					.stream().filter(ma -> Objects.equals(ma.getPlayerId(), player.getId())).findFirst().orElse(null);
			if (march == null) {
			...
			// 召回
			march.onMarchCallback();
		} else if (worldPoint instanceof IYQZZBuilding) {
			// 获取是否有驻军信息
			IYQZZWorldMarch march = player.getParent()
					.getPointMarches(GameUtil.combineXAndY(req.getX(), req.getY()),
			...
			// 召回
			march.onMarchCallback();
		} else if (worldPoint.getPointType() == WorldPointType.RESOURCE) {
			// 获取是否有驻军信息
			IYQZZWorldMarch march = player.getParent()
			...
			// 召回
			march.onMarchCallback();
		} else if (worldPoint.getPointType() == WorldPointType.PYLON) {
			// 获取是否有驻军信息
			IYQZZWorldMarch march = player.getParent()
					.getPointMarches(GameUtil.combineXAndY(req.getX(), req.getY()), WorldMarchStatus.MARCH_STATUS_MARCH_QUARTERED, WorldMarchType.PYLON_MARCH).stream()
					.filter(ma -> Objects.equals(ma.getPlayerId(), player.getId())).findFirst().orElse(null);
			if (march == null) {
				player.sendError(protocol.getType(), Status.Error.POINT_NO_MARCH_CALL_BACK);// 提示目标点没有援助行军
				return false;
	...
	private boolean onWorldSpyStart(HawkProtocol protocol) {
		...
		String targetId = "";
		if (point instanceof YQZZPlayer) {
			IYQZZPlayer targetPlayer = (IYQZZPlayer) point;
			// 普通玩家基地，放玩家的ID
	...
	private boolean onWorldAssistanceStart(HawkProtocol protocol) {
		...
		// 获取目标玩家
		final IYQZZPlayer tarPlayer = (IYQZZPlayer) point;
		...
		long size = player.getParent().getPointMarches(point.getPointId(), req.getType()).stream()
				.filter(m -> m.getPlayerId().equals(player.getId())).count();
		if (size > 0) {
		...
		player.getParent().startMarch(player, player, point, WorldMarchType.ASSISTANCE, tarPlayer.getId(), 0,
				new EffectParams(req, armyList));
	 ...
	 * @return
	 */
	public boolean checkMarchReq(IYQZZPlayer player, WorldMarchReq req, int hpCode, List<ArmyInfo> armyList,
			IYQZZWorldPoint worldPoint, boolean checkFreeMarch) {
		if (req.getType() == WorldMarchType.FOGGY_FORTRESS_MASS && isFoggyLimit()) {
			player.sendError(hpCode, Status.YQZZError.YQZZ_FOGGY_MASS_LIMIT_VALUE, 0);
			return false;
		}
		if (req.getType() == WorldMarchType.FOGGY_FORTRESS_MASS_JOIN && isJoinFoggyLimit()) {
			player.sendError(hpCode, Status.YQZZError.YQZZ_FOGGY_JOIN_LIMIT_VALUE, 0);
			return false;
		}
		
		if (worldPoint.getPointType() == WorldPointType.MONSTER) {
			if (isMonsterLimit()) {
				player.sendError(hpCode, Status.YQZZError.YQZZ_MONSTER_LIMIT_VALUE, 0);
				return false;
			}
		}
		if (worldPoint.getPointType() == WorldPointType.PYLON) {
			if (isPylonLimit()) {
				player.sendError(hpCode, Status.YQZZError.YQZZ_PYLON_LIMIT_VALUE, 0);
				return false;
			}
		}

		if (worldPoint.getClass() != YQZZBarrier.class) {
			int tarScirl = YQZZMapBlock.getInstance().circleNumber(worldPoint.getPointId());
			...
			if (tarScirl != circle) { // 不同圈层
				...
				if (!baseByCamp.controlBuildTypes.contains(YQZZBuildType.JIU)) {
					player.sendError(hpCode, Status.YQZZError.YQZZ_NO_TYPE9_BUILD);
					return false;
				}
			}
		}
		
		if (worldPoint instanceof YQZZPylon) {
			YQZZPylon build = (YQZZPylon) worldPoint;
			if (build.underNationControl(player.getGuildId())) { // 国家控制
				player.sendError(hpCode, Status.YQZZError.YQZZ_SAME_NATION);
				return false;
			}
		}
		
		if(worldPoint instanceof IYQZZPlayer){
			IYQZZPlayer build = (IYQZZPlayer) worldPoint;
			if (!build.isInSameGuild(player) && build.underNationControl(player.getGuildId())) { // 非本盟控制, 但国家控制
				player.sendError(hpCode, Status.YQZZError.YQZZ_SAME_NATION);
				return false;
			}
		}
		...
		if (worldPoint instanceof IYQZZBuilding) {
			...
			if (!build.underGuildControl(player.getGuildId()) && build.underNationControl(player.getGuildId())) { // 非本盟控制, 但国家控制
				player.sendError(hpCode, Status.YQZZError.YQZZ_SAME_NATION);
				return false;
			}
			
			if (!build.underGuildControl(player.getGuildId())) {
				Optional<YQZZDeclareWar> dclop = build.getDeclareWarRecord(player.getGuildId());
				if (!dclop.isPresent()) {
					player.sendError(hpCode, Status.YQZZError.YQZZ_NOT_DECLEAR_WAR);
					return false;
				}
				if (dclop.get().getEndTime() > player.getParent().getCurTimeMil()) {
					player.sendError(hpCode, Status.YQZZError.YQZZ_DECLEAR_WAR_WAIT);
					return false;
				}
				if (!build.canBeAttack(player.getGuildId())) {
					player.sendError(hpCode, Status.YQZZError.YQZZ_BUILD_NOT_LINK);
					return false;
				}
			}
		}
		...
		boolean hasFreeMarch = player.isHasFreeMarch();
		...
		if ((req.getAssistantList() == null || req.getAssistantList().isEmpty())
				&& (reqArmyList == null || reqArmyList.size() == 0)) {
	...
	private boolean onWorldMassStart(HawkProtocol protocol) {
		...
		case YQZZ_BUILDING:// 指挥部
			...
			break;
		case FOGGY_FORTRESS:
			YQZZFoggyFortress fpoint = (YQZZFoggyFortress) point;
			marchType = WorldMarchType.FOGGY_FORTRESS_MASS;
			targetId = String.valueOf(fpoint.getFoggyFortressId());
			break;
		default:
			player.sendError(protocol.getType(), Status.Error.WORLD_MARCH_REQ_TYPE_ERROR);
			return false;
		...
		IYQZZWorldMarch march = player.getParent().startMarch(player, player, point, marchType, targetId, waitTime,
				new EffectParams(req, armyList));
	...
	private boolean onWorldMassJoinStart(HawkProtocol protocol) {
		...
		IYQZZWorldMarch worldMarch = player.getParent().getMarch(massMarchId);
		// 取队长行军
		if (worldMarch == null) {
			player.sendError(protocol.getType(), Status.Error.MASS_ERR_MARCH_NOT_EXIST);
			return false;
		...
		// 出发
		player.getParent().startMarch(player, player, leader, req.getType(), massMarchId, 0, new EffectParams(req, armyList));

		return true;
	}

	public boolean checkArmyAndMarch(int protoType, IYQZZPlayer player, List<ArmyInfo> armyList,
			List<Integer> heroIdList, int superSoldierId) {
	...
	private boolean onWorldFoggySingleMarchStart(HawkProtocol protocol) {
		...
		// 目标点不存在
		if (targetPoint == null) {
			player.sendError(protocol.getType(), Status.Error.TARGET_MONSTER_DISAPPEAR);
			WorldScene.getInstance().move(player.getAoiObjId(), posX, posY, 0.0f);
			return false;
		}
		// 不是迷雾要塞
		if (targetPoint.getPointType() != WorldPointType.FOGGY_FORTRESS) {
			return false;
		}
		// 检测出征的军队信息
		List<ArmyInfo> armyList = new ArrayList<ArmyInfo>(); // 出征的士兵列表
		...
		// 发起行军
		player.getParent().startMarch(player, player, targetPoint, WorldMarchType.FOGGY_SINGLE, "", 0, new EffectParams(req, armyList));
	...
	private boolean onWorldCollectResourceStart(HawkProtocol protocol) {
		WorldMarchReq req = protocol.parseProtocol(WorldMarchReq.getDefaultInstance());
		IYQZZWorldPoint reqPoint = player.getParent().getWorldPoint(req.getPosX(), req.getPosY()).orElse(null);

		// check点数据
		if (reqPoint == null || !(reqPoint instanceof YQZZResource)) {
			player.sendError(protocol.getType(), Status.Error.WORLD_POINT_NOT_RESOURCE);
			return false;
		}
		YQZZResource point = (YQZZResource) reqPoint;
	...
	public boolean checkMarchDressReq(IYQZZPlayer player, List<Integer> marchDressList) {
		try {
	...
	private Activity.ChatMassCardInfo.Builder getChatMassCardInfo(String marchId, IYQZZPlayer viewer) {
		try {
			...
			IYQZZWorldMarch march = viewer.getParent().getMarch(marchId);
	...
	private boolean isFoggyLimit() {
		int attackFoggyWinTimes = player.getKillFoggy();
		if (attackFoggyWinTimes < player.getParent().getCfg().getFoggyStartAssembleLimit()) {
			return false;
		}
		return true;
	}
	
	private boolean isJoinFoggyLimit() {
		int joinAtkWinTimes = player.getJoinKillFoggy();
		if (joinAtkWinTimes < player.getParent().getCfg().getFoggyAssembleLimit()) {
			return false;
		}
		return true;
	}
	...
	private boolean isPylonLimit() {
		int attackFoggyWinTimes = player.getCollPylon();
		if (attackFoggyWinTimes < player.getParent().getCfg().getPylonLimit()) {
			return false;
		}
		return true;
	}
