...
public class YQZZMonsterRefesh {

	private List<HawkTuple2<Integer, YQZZMonsterCfg>> fuelBankRefreshTimes = new LinkedList<>();
	private YQZZBattleRoom parent;

	private boolean yugaoNotSend = true;

	private List<YQZZMonsterCreater> createrList = new CopyOnWriteArrayList<>();
	private int refreshTurn; // 已经刷新的波数
	
	private YQZZMonsterRefesh() {
	}

	public static YQZZMonsterRefesh create(YQZZBattleRoom parent) {
		YQZZMonsterRefesh result = new YQZZMonsterRefesh();
		result.parent = parent;

		ConfigIterator<YQZZMonsterCfg> cfgs = HawkConfigManager.getInstance().getConfigIterator(YQZZMonsterCfg.class);
		for (YQZZMonsterCfg cfg : cfgs) {
	...
	public void onTick() {
		createrList.forEach(YQZZMonsterCreater::onTick);
...
//		}
		...
		YQZZMonsterCfg mcfg = fuelBankRefreshTimes.remove(0).second;
		createrList.add(YQZZMonsterCreater.create(this, mcfg));
		refreshTurn ++;
//		if (!fuelBankRefreshTimes.isEmpty()) {
