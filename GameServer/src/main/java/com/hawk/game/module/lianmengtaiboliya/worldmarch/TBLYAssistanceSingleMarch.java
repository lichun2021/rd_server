...
public class TBLYAssistanceSingleMarch extends ITBLYWorldMarch implements ITBLYPassiveAlarmTriggerMarch, ITBLYReportPushMarch {

	public TBLYAssistanceSingleMarch(ITBLYPlayer parent) {
		super(parent);
	}
	...
	public void onMarchReach(Player parent) {
		...
		// 路点为空
		if (point == null || !(point instanceof ITBLYPlayer)) {
		...
		if (backList.size() > 0) {
			...
			ITBLYWorldMarch back = getParent().getParent().startMarch(getParent(), point, getParent(), WorldMarchType.ASSISTANCE, "",0, effParams);
