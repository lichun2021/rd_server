 ...
 * @author julia
 */
public class DYZZMapBlock {
	/**
	 * 阻挡点id列表
	 */
	private Set<Integer> stops = new TreeSet<Integer>();

	private static DYZZMapBlock instance = null;

	public static DYZZMapBlock getInstance() {
		if (instance == null) {
			instance = new DYZZMapBlock();
		}
		return instance;
	}
	...
	private boolean loadMapData() {
		...
		String filePath = HawkOSOperator.getWorkPath() + "xml/dyzz_tmx_block.dat";
		try {
			// 读取配置文件内容
