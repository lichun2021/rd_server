...
public class SFHero extends PlayerHero {
	private SFPlayer parent = SFPlayer.DEFAULT_INSTANCE;
	private PBHeroState state = PBHeroState.HERO_STATE_FREE;

	private SFHero(HeroEntity heroEntity) {
		super(heroEntity);
	}

	public static SFHero create(int heroId) {
	...
	public HeroCfg getConfig() {
		return HawkConfigManager.getInstance().getConfigByKey(HeroCfg.class, getCfgId());
	}
	...
	@Override
	public int getCfgId() {
		return getHeroEntity().getHeroId();
	}
