...
public class TBLYAttackPlayerMarch extends ITBLYWorldMarch implements ITBLYReportPushMarch {

	public TBLYAttackPlayerMarch(ITBLYPlayer parent) {
		super(parent);
	}
	...
	public void onMarchReach(Player parent) {
		...
		atkMarchs.add(this);
		// 防守方援军
		List<ITBLYWorldMarch> helpMarchList = helpMarches();
		// 防守方行军
		...
		for (Player defer : defPlayers) {
			((ITBLYPlayer) defer).setLastAttacker(getParent());
		}
	}
	...
	private List<ITBLYWorldMarch> helpMarches() {
