...
import com.hawk.game.protocol.Mail.PBDetectTBLYBuilding;
...
public class TBLYSpyMarch extends ITBLYWorldMarch implements ITBLYReportPushMarch {

	public TBLYSpyMarch(ITBLYPlayer parent) {
		super(parent);
	}
	...
	public void onMarchReach(Player parent) {
		...
		if (point instanceof TBLYFuelBank) {
			doFuelBankSpy(point);
			onMarchReturn(getMarchEntity().getTerminalId(), getMarchEntity().getOrigionId(), Collections.emptyList());
			return;
		}
	...
	private void doBuildSpy(ITBLYWorldPoint point) {
		// TBLY_SPY_BUILD_SUCCESS = 2020009;//泰伯利亚建筑侦察成功 2020009
		// TBLY_SPY_BUILD_FAIL = 2020010;//泰伯利亚建筑侦察失败 2020010
		// TBLY_SPY_BUILD_BE = 2020011; //泰伯利亚建筑被侦察 2020011
		ITBLYBuilding build = (ITBLYBuilding) point;
		List<ITBLYWorldMarch> quarteredMarchs = getParent().getParent().getPointMarches(getMarchEntity().getTerminalId(), WorldMarchStatus.MARCH_STATUS_MARCH_QUARTERED);
		...
		if (quarteredMarchs.get(0).getParent().getCamp() == getParent().getCamp()) {
			...
			// .setMailId(MailId.TBLY_SPY_BUILD_FAIL)
			// .build());
			return;
		}
		...
		PBDetectTBLYBuilding.Builder builder = PBDetectTBLYBuilding.newBuilder().setLevel(radarLevel).setPointX(point.getX()).setPointY(point.getY());
		...
		// 侦查邮件
		MailParames.Builder playerParamesBuilder = MailParames.newBuilder().setPlayerId(getParent().getId()).setMailId(MailId.TBLY_SPY_BUILD_SUCCESS)
				.addSubTitles(point.getX(), point.getY())
				.setDuntype(DungeonMailType.TBLY)
				.addContents(DetectMail.newBuilder().setTblyBuild(builder)).addTips(point.getX()).addTips(point.getY());
		...
		for (IWorldMarch tempMarch : quarteredMarchs) {
			FightMailService.getInstance()
					.sendMail(MailParames.newBuilder().setPlayerId(tempMarch.getPlayerId()).setMailId(MailId.TBLY_SPY_BUILD_BE).addSubTitles(point.getX(), point.getY())
	...
	private void doFuelBankSpy(ITBLYWorldPoint point) {
		// TBLY_SPY_RES_SUCCESS = 2020005;//泰伯利亚侦察资源点 2020005
		// TBLY_SPY_RES_BE = 2020006;//泰伯利亚被侦察资源点 2020006
		// TBLY_SPY_RES_FAIL = 2020007;//泰伯利亚资源点侦察失败（撤离） 2020007
		// TBLY_SPY_RES_FAIL_SAME_GUILD = 2020008;//泰伯利亚资源点侦察失败（联盟变更） 2020008
		TBLYFuelBank resPoint = (TBLYFuelBank) point;
		if (resPoint.getMarch() == null) {
			return;
		}

		TBLYCollectFuelMarch enemy = resPoint.getMarch();
		...
		// 侦查邮件
		MailParames.Builder playerParamesBuilder = MailParames.newBuilder().setPlayerId(getParent().getId()).setMailId(MailId.TBLY_SPY_RES_SUCCESS)
				.addSubTitles(point.getX(), point.getY())
				.addMidTitles(enemy.getParent().getGuildTag(), enemy.getParent().getName())
				.setDuntype(DungeonMailType.TBLY)
				.addTips(point.getX(),point.getY(),enemy.getParent().getName())
				.addContents(DetectMail.newBuilder().setTblyBuild(builder));
		MailParames mparames = playerParamesBuilder.build();
		FightMailService.getInstance().sendMail(mparames);
		...
		for (IWorldMarch tempMarch : quarteredMarchs) {
			FightMailService.getInstance()
					.sendMail(MailParames.newBuilder().setPlayerId(tempMarch.getPlayerId()).setMailId(MailId.TBLY_SPY_RES_BE)
							.addSubTitles(point.getX(), point.getY())
							.addMidTitles(player.getGuildTag(), player.getName())
							.setDuntype(DungeonMailType.TBLY)
	...
	private void doPlayerSpy(ITBLYWorldPoint point) {
		ITBLYPlayer tarPlayer = (ITBLYPlayer) point;
	 ...
	 * @return
	 */
	public boolean sendDetectMail(Player player, Player targetPlayer, ITBLYWorldPoint point) {
		...
		MailParames.Builder playerParamesBuilder = MailParames.newBuilder().setPlayerId(player.getId()).addSubTitles(subTitle).addContents(builder)
				...
				.addTips(targetPlayer.getName()).setDuntype(DungeonMailType.TBLY)
		...
		FightMailService.getInstance().sendMail(playerParamesBuilder.setDuntype(DungeonMailType.TBLY).build());

		FightMailService.getInstance().sendMail(targetParamesBuilder.setDuntype(DungeonMailType.TBLY).build());
		return true;
	}

	public DetectMail.Builder createDetectMail(final int radarLevel, Player player, Player targetPlayer, ITBLYWorldPoint point, boolean isMirror) {
		...
		if (pointType == WorldPointType.RESOURCE_VALUE || pointType == WorldPointType.QUARTERED_VALUE) {
			// 资源点 驻扎点
			List<ITBLYWorldMarch> marchs = getParent().getParent().getPointMarches(point.getPointId(), WorldMarchStatus.MARCH_STATUS_MARCH_QUARTERED);
			if (!marchs.isEmpty()) {
				leaderMarch = marchs.get(0);
			}
		} else if (pointType == WorldPointType.PLAYER_VALUE) {
			// 玩家城堡---基地
			List<ITBLYWorldMarch> massMarches = getParent().getParent().getPlayerMarches(targetPlayer.getId(), WorldMarchStatus.MARCH_STATUS_WAITING);
			if (!massMarches.isEmpty()) {
				leaderMarch = massMarches.get(0);
			}
			List<ITBLYWorldMarch> marchs = getParent().getParent().getPointMarches(point.getPointId(), WorldMarchStatus.MARCH_STATUS_MARCH_ASSIST);
		...
		if (pointType == WorldPointType.PLAYER_VALUE && !targetPlayer.getData().getArmyEntities().isEmpty()) {
			...
			if (radarLevel >= GsConst.RadarLevel.LV21 && leaderMarch != null) {
				List<ITBLYWorldMarch> massMarches = getParent().getParent().getPointMarches(point.getPointId(), WorldMarchStatus.MARCH_STATUS_MARCH,
		...
		if (pointType == WorldPointType.PLAYER_VALUE) {
			...
			// 藏兵洞
			List<ITBLYWorldMarch> hiddenMarchs = getParent().getParent().getPointMarches(point.getPointId(), WorldMarchType.HIDDEN_MARCH);
			if (radarLevel >= GsConst.RadarLevel.LV10) {
	...
	public Set<String> attackReportRecipients() {
		// 防守方援军
		List<ITBLYWorldMarch> helpMarchList = getParent().getParent().getPointMarches(getMarchEntity().getTerminalId(), WorldMarchStatus.MARCH_STATUS_MARCH_ASSIST,
