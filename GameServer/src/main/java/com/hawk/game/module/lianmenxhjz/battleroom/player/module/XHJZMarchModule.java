...
import com.hawk.game.module.lianmenxhjz.battleroom.invoker.XHJZItemConsumeInvoker;
...
import com.hawk.game.protocol.World.XHJZBuildMarches;
import com.hawk.game.protocol.World.XHJZBuildMarchesResp;
import com.hawk.game.protocol.XHJZ.PBXHJZQuartedMarchReq;
import com.hawk.game.protocol.XHJZ.XHJZWorldMarchGuaiWanReq;
...
public class XHJZMarchModule extends PlayerModule {
	private IXHJZPlayer player;
	public XHJZMarchModule(IXHJZPlayer player) {
		super(player);
		this.player = player;
	}
	...
	public boolean marchEmoticonUse(HawkProtocol protocol, int emoticonId, String marchId) {
		IXHJZWorldMarch march = player.getParent().getMarch(marchId);
		if (march == null) {
		...
		if (march.getMarchStatus() != WorldMarchStatus.MARCH_STATUS_MARCH_VALUE 
			...
			IXHJZWorldPoint worldPoint = player.getParent().getWorldPoint(march.getTerminalId()).get();
			if (worldPoint instanceof IXHJZBuilding && ((IXHJZBuilding) worldPoint).getLeaderMarch() != null) {
				WorldMarch firstMarch = ((IXHJZBuilding) worldPoint).getLeaderMarch().getMarchEntity();
	...
	private boolean onInviteMass(HawkProtocol protocol) {
		...
		IXHJZWorldMarch march = player.getParent().getMarch(req.getMarchId());
		if (march == null) {
		...
		Optional<IXHJZWorldPoint> worldPoint = player.getParent().getWorldPoint(march.getMarchEntity().getTerminalId());
		if (worldPoint.isPresent()) {
			XHJZGuildFormationObj formationObj = player.getParent().getGuildFormation(player.getGuildId());
	 ...
	 * 发送公告
	 */
	public void sendNotice(IXHJZWorldPoint point, IXHJZWorldMarch march, int formationIndex, String formationName) {
		IXHJZPlayer player = march.getParent();
		 if (point instanceof IXHJZBuilding) {
			ChatParames parames = ChatParames.newBuilder().setPlayer(player).setChatType(ChatType.CHAT_FUBEN_TEAM).setKey(NoticeCfgId.XHJZ_MASS_START_BUILD)
					.addParms(march.getXHJZTerminalX())
					.addParms(march.getXHJZTerminalY())
...
//		// 世界点
//		IXHJZWorldPoint worldPoint = player.getParent().getWorldPoint(req.getX(), req.getY()).orElse(null);
//		if (worldPoint == null) {
...
//
//		// 战区
//		if (worldPoint instanceof IXHJZBuilding) {
//			IXHJZBuilding building = (IXHJZBuilding) worldPoint;
...
//	}
	...
	protected boolean onWorldMassDissolve(HawkProtocol protocol) {
		...
		// check
		IXHJZWorldMarch worldMarch = player.getParent().getMarch(req.getMarchId());
		if (worldMarch == null) {
			player.sendError(protocol.getType(), Status.Error.WORLD_MARCH_NOT_EXISIT);
			return false;
		}
		...
		for (IXHJZWorldMarch jm : worldMarch.getMassJoinMarchs(true)) {
			jm.onMarchCallback();
			// // 发送邮件---集结失败：发起者解散
	...
	protected boolean onWorldMassRepatriate(HawkProtocol protocol) {
		...
		IXHJZWorldMarch leaderMarch = player.getParent().getMarch(march.getTargetId());
		if (leaderMarch == null) {
			player.sendError(protocol.getType(), Status.Error.WORLD_MARCH_NOT_LEADER);
			return false;
		}
	...
	/**
	 * XHJZ_SINGLE_MARCH = 487; // 副本内特有建筑单人行军
	 */
	@ProtocolHandler(code = HP.code2.XHJZ_SINGLE_MARCH_VALUE)
	private boolean onXHJZ_SINGLE_MARCHStart(HawkProtocol protocol) {
		WorldMarchReq req = protocol.parseProtocol(WorldMarchReq.getDefaultInstance());
		Position startPoint = req.getXhjzPointsList().get(0);
		IXHJZWorldPoint point = player.getParent().getWorldPoint(startPoint.getX(), startPoint.getY()).orElse(null);
		// 路点为空
		if (point == null) {
			player.sendError(protocol.getType(), Status.Error.WORLD_POINT_NOT_EXIST);
			return false;
...
//			player.sendError(protocol.getType(), Status.Error.XHJZ_BUILD_PROCTED);
//			return false;
//		}
...
//		}
		...
		WorldMarchType marchType = WorldMarchType.XHJZ_BUILDING_SINGLE;

		// 带兵出征通用检查
		List<ArmyInfo> armyList = new ArrayList<ArmyInfo>(); // 出征的士兵列表 
		int gasoline = (int) (player.getParent().getCfg().getFuelMarchNeed() + player.getParent().getCampBase(player.getCamp()).getOccupyMarchFuel()); 
		if (!checkMarchReq(player, req, protocol.getType(), armyList, point, true, gasoline)) {
			return false;
		}
		...
		player.getParent().startMarch(player, player, point, marchType, "", 0, new EffectParams(req, armyList),gasoline);
		
		// 回复协议
		WorldMarchResp.Builder builder = WorldMarchResp.newBuilder();
		builder.setSuccess(true);
		protocol.response(HawkProtocol.valueOf(HP.code2.XHJZ_SINGLE_MARCH_S, builder));
		return true;
	...
	private boolean onMarchArmyDetailInfo(HawkProtocol protocol) {
		...
		Set<IXHJZWorldMarch> allMarch = new HashSet<>();
		allMarch.add(march);
		...
		// 是否有自己的行军
		for (IXHJZWorldMarch thisMarch : allMarch) {
		...
		for (IXHJZWorldMarch thisMarch : allMarch) {
			...
			IXHJZPlayer mp = thisMarch.getParent();
...
//		}
		...
		if (march.speedUp(timeReducePercent, speedUpTimes)) {
			// 消耗掉
			player.dealMsg(MsgId.WORLD_MARCH, new XHJZItemConsumeInvoker(player, consumeItems, Action.WORLD_MARCH_SPEEDUP));
	...
	private boolean onMarchCallBack(HawkProtocol protocol) {
		...
		// 检测行军是否存在
		IXHJZWorldMarch march = player.getParent().getPlayerMarch(player.getId(), req.getMarchId());
		if (march == null) {
		...
		// 不是自己的行军不能召回
		if (!player.getId().equals(march.getPlayerId()) || march instanceof IXHJZMassJoinMarch) {
			return false;
		}
	...
	private boolean onPointMarchCallBack(HawkProtocol protocol) {
		...
		if (worldPoint instanceof IXHJZBuilding) {
			// 获取是否有驻军信息
			IXHJZWorldMarch march = player.getParent().getPointMarches(GameUtil.combineXAndY(req.getX(), req.getY()), WorldMarchStatus.MARCH_STATUS_MARCH_QUARTERED).stream()
					.filter(ma -> Objects.equals(ma.getPlayerId(), player.getId()))
					.filter(ma -> Objects.equals(ma.getMarchId(), req.getMarchId()))
					.findFirst().orElse(null);
			if (march == null || march instanceof IXHJZMassJoinMarch) {
//				player.sendError(protocol.getType(), Status.Error.POINT_NO_MARCH_CALL_BACK);// 提示目标点没有援助行军
				return false;
			}
			for (IXHJZWorldMarch jm : march.getMassJoinMarchs(true)) {
				jm.onMarchCallback();
			}
			// 召回
			march.onMarchCallback();
	 ...
	 * @return
	 */
	public boolean isHasFreeMarch(IXHJZPlayer player) {
		// 机器人不受行军数量限制
		if (player.isRobot()) {
			return true;
		}
		List<IXHJZWorldMarch> playerMarches = player.getParent().getPlayerMarches(player.getId());
		// 行军个数
		int marchCount = 0;
		for (IXHJZWorldMarch march : playerMarches) {
	 ...
	 * @return
	 */
	public boolean checkMarchReq(IXHJZPlayer player, WorldMarchReq req, int hpCode, List<ArmyInfo> armyList, IXHJZWorldPoint worldPoint, boolean checkFreeMarch, int gasoline) {
		if(player.getGasoline() < gasoline){
			return false;
		}
		...
		// 能不能链接
		IXHJZBuilding start = player.getParent().getWorldPointService().getBaseByCamp(player.getCamp());
		for(Position posi : req.getXhjzPointsList()){
			IXHJZBuilding tar = (IXHJZBuilding) player.getParent().getWorldPoint(posi.getX(), posi.getY()).orElse(null);
			if(tar.getBuildType() == XHJZBuildType.LIU && tar.underGuildControl(player.getGuildId())){
				continue;
			}
			
			if (!tar.getCfg().getLinkList().contains(start.getCfgId())) { // 不链接
//				return false;
			}
			start = tar;
		}
		if (HawkTime.getMillisecond() < start.getProtectedEndTime()) { // 终点有保护
			return false;
		}
...
//		}
		...
		String targetId = "";
		WorldMarchType marchType = WorldMarchType.XHJZ_BUILDING_MASS;
		...
		IXHJZWorldMarch march = player.getParent().startMarch(player, player, point, marchType, targetId, waitTime, new EffectParams(req, armyList), gasoline);
	...
	private boolean onWorldMassJoinStart(HawkProtocol protocol) {
		...
		IXHJZWorldMarch worldMarch = player.getParent().getMarch(massMarchId);
		// 取队长行军
		if (worldMarch == null) {
			player.sendError(protocol.getType(), Status.Error.MASS_ERR_MARCH_NOT_EXIST);
			return false;
		...
		int marchType = req.getType().getNumber();
		// 不能加入自己的集结
		IXHJZPlayer leader = worldMarch.getParent();
		...
		IXHJZMassMarch massMarch = (IXHJZMassMarch) worldMarch;
		// 集结行军类型和参与集结行军类型不一致
		...
		int gasoline = (int) massMarch.getGasoline();// (int) (player.getParent().getCfg().getFuelMarchNeed() + player.getParent().getCampBase(player.getCamp()).getOccupyMarchFuel()); 
		// 带兵出征通用检查
		List<ArmyInfo> armyList = new ArrayList<ArmyInfo>(); // 出征的士兵列表
		if (!checkMarchReq(player, req, protocol.getType(), armyList, leader, true,gasoline)) {
			return false;
		}


		// 已经有加入这支部队的集结
		Set<IXHJZWorldMarch> massJoinMarchs = massMarch.getMassJoinMarchs(false);
		...
		// 出发
		player.getParent().startMarch(player, player, leader, req.getType(), massMarchId, 0, new EffectParams(req, armyList),gasoline);
	...
	public boolean checkArmyAndMarch(int protoType, IXHJZPlayer player, List<ArmyInfo> armyList, List<Integer> heroIdList, int superSoldierId) {
	...
	public boolean checkMarchDressReq(IXHJZPlayer player, List<Integer> marchDressList) {
		try {
	...
	private Activity.ChatMassCardInfo.Builder getChatMassCardInfo(String marchId, IXHJZPlayer viewer) {
		try {
	...
	@ProtocolHandler(code = HP.code2.XHJZ_BUILD_QUARTERED_MARCHES_C_VALUE)
	private void onQuarteredMarchDetailInfo(HawkProtocol protocol) {
		PBXHJZQuartedMarchReq req = protocol.parseProtocol(PBXHJZQuartedMarchReq.getDefaultInstance());
		IXHJZWorldPoint point = player.getParent().getWorldPoint(req.getX(), req.getY()).orElse(null);
		if (point == null) {
			return;
		}
		IXHJZBuilding build = (IXHJZBuilding) point;
		List<IXHJZWorldMarch> stayMarches = player.getParent().getPointMarches(build.getPointId(), WorldMarchStatus.MARCH_STATUS_MARCH_QUARTERED);
		Collections.sort(stayMarches, Comparator.comparingLong(IXHJZWorldMarch::getEndTime).reversed().thenComparing(Comparator.comparingLong(IXHJZWorldMarch::getCreatetime)));
		HashMultimap<String, IXHJZWorldMarch> groupMap = HashMultimap.create();
		for (IXHJZWorldMarch march : stayMarches) {
			if (march instanceof IXHJZMassJoinMarch) {
				groupMap.put(march.getMarchEntity().getTargetId(), march);
			} else {
				groupMap.put(march.getMarchId(), march);
			}
		}

		XHJZBuildMarchesResp.Builder resp = XHJZBuildMarchesResp.newBuilder();
		for (Collection<IXHJZWorldMarch> mcol : groupMap.asMap().values()) {
			XHJZBuildMarches.Builder marchGroup = XHJZBuildMarches.newBuilder();
			for (IXHJZWorldMarch march : mcol) {
				WorldMarchRelation relation = march.getRelation(player);
				marchGroup.addMarchs(march.toBuilder(relation));
			}

			resp.addMarchGroup(marchGroup);
		}

		player.sendProtocol(HawkProtocol.valueOf(HP.code2.XHJZ_BUILD_QUARTERED_MARCHES_S_VALUE, resp));
	}
	
	/**改变行军目标*/
	@ProtocolHandler(code = HP.code2.XHJZ_MARCH_GUAIWAN_REQ_VALUE)
	private boolean onMarchZhuanwan(HawkProtocol protocol) {
		XHJZWorldMarchGuaiWanReq req = protocol.parseProtocol(XHJZWorldMarchGuaiWanReq.getDefaultInstance());
		String marchId = req.getMarchId();
		IXHJZWorldMarch march = player.getParent().getMarch(marchId);
...
//			return false;
//		}
		IXHJZWorldPoint fPoint = player.getParent().getWorldPoint(march.getTerminalId()).get();
		...
		for (Position p : req.getXhjzPointsList()) {
			roadAllPoints.add(p.toBuilder());
		}
		march.calFullpath(player.getParent().getCurTimeMil(), roadAllPoints);
		return true;
	}
	

	/**集结出发*/
	@ProtocolHandler(code = HP.code2.XHJZ_MARCH_MASS_GO_REQ_VALUE)
	private boolean onMassMarchGo(HawkProtocol protocol) {
		...
		Set<IXHJZWorldMarch> reachJoinMarchs = march.getMassJoinMarchs(true);
