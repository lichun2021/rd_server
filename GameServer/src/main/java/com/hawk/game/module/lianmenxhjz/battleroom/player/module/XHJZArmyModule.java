...
import com.hawk.game.protocol.XHJZ.XHJZDistributeGasoline;
import com.hawk.game.protocol.XHJZ.XHJZDistributeGasolineRecord;
import com.hawk.game.protocol.XHJZ.XHJZDistributeGasolineRecordResp;
import com.hawk.game.protocol.XHJZ.XHJZDistributeGasolineReq;
...
public class XHJZArmyModule extends PlayerModule {

	private IXHJZPlayer player;

	public XHJZArmyModule(IXHJZPlayer player) {
		super(player);
		this.player = player;
	}
	...
	private QueueEntity addQueue(IXHJZPlayer player, int queueType, int queueStatus, String itemId,
	...
	private List<ItemInfo> cureConsume(IXHJZPlayer player, List<ArmySoldierPB> cureList, int recoverTime, boolean immediate, boolean useGold, int hpCode) {
	 ...
	 * @return
	 */
	private Map<Integer, Integer> cureSoldier(IXHJZPlayer player, List<ArmySoldierPB> cureList, boolean immediate) {
	 ...
	 * 分配燃油
	 */
	@ProtocolHandler(code = HP.code2.XHJZ_DISTRIBUTE_GASOLINE_REQ_VALUE)
	private void onDistributeGasoline(HawkProtocol protocol) {
		XHJZBattleRoom battleroom = player.getParent();
		long now = battleroom.getCurTimeMil();
		XHJZDistributeGasolineReq req = protocol.parseProtocol(XHJZDistributeGasolineReq.getDefaultInstance());
		XHJZGuildBaseInfo baseInfo = battleroom.getCampBase(player.getCamp());
		for(XHJZDistributeGasoline dg :  req.getDistributeList()){
			IXHJZPlayer tar = battleroom.getPlayer(dg.getPlayerId());
			if(tar.getCamp()!= baseInfo.getCamp()){
				return;
			}
			int add = dg.getXhjzGasoline();
			add = (int) Math.ceil(Math.min(add, tar.getMaxGasoline() - tar.getGasoline()));
			if( baseInfo.getGasoline() - add <0){
				return ;
			}
			tar.setGasoline(tar.getGasoline()+ add);
			tar.setXhjzDistributeGasoline(tar.getXhjzDistributeGasoline() + add);
			baseInfo.setGasoline(baseInfo.getGasoline() - add);
			
			XHJZDistributeGasolineRecord.Builder record = XHJZDistributeGasolineRecord.newBuilder();
			record.setPlayerName(tar.getName()).setTime(now).setXhjzGasoline(add).setLeaderName(player.getName());
			battleroom.getCampBase(player.getCamp()).getFuelDisRecords().add(record.build());

			LogUtil.logXhjzXhjzFuelDistribute(player.getParent().getId(), player.getId(), player.getGuildId(),(int)baseInfo.getGasoline(), tar.getId(), add,(int) tar.getGasoline());
		}
		
//		PBXHJZGameInfoSync.Builder lastSyncpbBul = battleroom.getLastSyncpb().toBuilder();
//		for(PBXHJZGuildInfo.Builder guildInfo : lastSyncpbBul.getGuildInfoBuilderList()){
//			if( guildInfo.getCamp() == player.getCamp().intValue()){
//			guildInfo.setXhjzGasoline(0);
//			}
//		}
		
		battleroom.setLastSyncpb(null);
		battleroom.sync(player);
		player.responseSuccess(protocol.getType());
	}
	 ...
	 * 分配燃油
	 */
	@ProtocolHandler(code = HP.code2.XHJZ_DISTRIBUTE_RECORD_C_VALUE)
	private void onDistributeGasolineRecord(HawkProtocol protocol) {
		
		XHJZDistributeGasolineRecordResp.Builder resp = XHJZDistributeGasolineRecordResp.newBuilder();
		resp.addAllRecords(player.getParent().getCampBase(player.getCamp()).getFuelDisRecords());
		player.sendProtocol(HawkProtocol.valueOf(HP.code2.XHJZ_DISTRIBUTE_RECORD_S, resp));
	}
