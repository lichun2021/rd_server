...
import com.hawk.game.module.lianmengfgyl.battleroom.FGYLConst;
...
import com.hawk.game.module.lianmengfgyl.battleroom.FGYLWorldScene;
...
import com.hawk.game.module.lianmengfgyl.battleroom.player.module.guildformation.FGYLGuildFormationModule;
...
public abstract class IFGYLWorldMarch implements IWorldMarch {

	private final IFGYLPlayer parent;
	private FGYLMarchEntity marchEntity;
	private int hashThread;
	private int fgylSkill;
	private int useHonor;

	public IFGYLWorldMarch(IFGYLPlayer parent) {
		this.parent = parent;
	}

	public final IFGYLPlayer getParent() {
		return parent;
	}

	@Override
	public FGYLMarchEntity getMarchEntity() {
		return marchEntity;
	}

	public void setMarchEntity(FGYLMarchEntity marchEntity) {
	...
	public WorldMarchStatus getMassJoinMarchStatus(String playerId) {
		try {
			...
			Set<IFGYLWorldMarch> massJoinMarchs = getMassJoinMarchs(false);
	...
	public GuildWarShowPB getGuildWarShoPb(IFGYLPlayer viewer) {
		// 行军状态
		...
		if (viewer.isInSameGuild(getParent())) {
			showPb.setIsatk(true);
		}
		try {
			FGYLGuildFormationObj formationObj = getParent().getParent().getGuildFormation(viewer.getGuildId());
			...
			if (formation != null) {
				...
				showPb.setNeedJoin(formation.fight(viewer.getId()));
			}
		} catch (Exception e) {
			HawkException.catchException(e);
		}
	...
	public double armyBattlePoint() {
		...
		for (ArmyInfo army : getArmys()) {
			BattleSoldierCfg cfg = HawkConfigManager.getInstance().getConfigByKey(BattleSoldierCfg.class,
					army.getArmyId());
			if (cfg == null) {
				continue;
			}
...
//		}
	...
	public WorldMarchPB.Builder toBuilder(WorldMarchRelation relation) {
		...
		DressItem marchDress = WorldPointService.getInstance().getShowDress(entity.getPlayerId(),
				DressType.MARCH_DRESS_VALUE);
		if (marchDress != null) {
	...
	public GuildWarSingleInfo.Builder getGuildWarSingleInfo(GuildFormationCell formation) {
		...
		builder.setFgylSkill(getFgylSkill());
	...
	public WorldMarchPB.Builder toBuilder() {
		...
		DressItem marchDress = WorldPointService.getInstance().getShowDress(marchEntity.getPlayerId(),
				DressType.MARCH_DRESS_VALUE);
		if (marchDress != null) {
			long now = HawkTime.getMillisecond();
			if (marchDress.getStartTime() + marchDress.getContinueTime() > now) {
	...
	public boolean speedUp(int timeReducePercent, int speedUpTimes) {
		...
		if (this.isMassMarch()) {
			Set<IFGYLWorldMarch> joinMarchs = getMassJoinMarchs(false);
			for (IFGYLWorldMarch joinMarch : joinMarchs) {
				joinMarch.getMarchEntity().setEndTime(march.getEndTime());
				// 发给自己
				HawkProtocol protocol = HawkProtocol.valueOf(HP.code.WORLD_MARCH_UPDATE_PUSH_VALUE,
						joinMarch.toBuilder(WorldMarchRelation.SELF));
				joinMarch.getParent().sendProtocol(protocol);
			}
		}

		if (this instanceof IFGYLReportPushMarch) {
			((IFGYLReportPushMarch) this).reachTimeChange();
		}
		return true;
	}

	@Override
	public Set<IFGYLWorldMarch> getQuarterMarch() {
		if (getMarchStatus() != WorldMarchStatus.MARCH_STATUS_MARCH_QUARTERED_VALUE) {
			return Collections.emptySet();
		}
		List<IFGYLWorldMarch> defMarchList = getParent().getParent().getPointMarches(getTerminalId(), WorldMarchStatus.MARCH_STATUS_MARCH_QUARTERED);
		return new HashSet<>(defMarchList);
	}

	@Override
	public Set<IFGYLWorldMarch> getMassJoinMarchs(boolean b) {
		return Collections.emptySet();
	}

	/** 通知行军事件 */
	public void notifyMarchEvent(MarchEvent eventType) {
		if (getParent().getParent().isGameOver()) {
			return;
		}
		FGYLNotifyMarchEventFunc func = new FGYLNotifyMarchEventFunc();
		func.setMarch(this);
		func.setEventType(eventType);
		getParent().getParent().notifyMarchEventAsync(func);
	}

	public void pushMarchEvent(MarchEvent eventType) {
		FGYLWorldScene worldScene = getParent().getParent().getWorldPointService().getWorldScene();
		for (IFGYLPlayer player : getParent().getParent().getPlayerList(FGYLState.GAMEING)) {
	...
	public boolean isInview(final int x, final int y) {
		...
		if (dis > GameConstCfg.getInstance().getSyncSearchRadius()) {
			return false;
		}
		return true;
	}
	 ...
	 * @return
	 */
	public WorldMarchRelation getRelation(IFGYLPlayer tarplayer) {
		...
		// 目标点
		Optional<IFGYLWorldPoint> targetPointOp = getParent().getParent().getWorldPoint(marchEntity.getTerminalId());
		...
		IFGYLWorldPoint targetPoint = targetPointOp.get();
	...
	public void sendBattleResultInfo(boolean isWin, List<ArmyInfo> atkArmyList, List<ArmyInfo> defArmyList, boolean isMonsterDead) {
		...
		List<IFGYLPlayer> players = getParent().getParent().getPlayerList(FGYLState.GAMEING);
		for (IFGYLPlayer pla : players) {
	...
	public synchronized void remove() {
		try {
			if (this instanceof IFGYLReportPushMarch) {
				((IFGYLReportPushMarch) this).removeAttackReport();
			}
	...
	public void updateDefMarchAfterWar(List<? extends IWorldMarch> helpMarchList, Map<String, List<ArmyInfo>> defArmyMap) {
		// 更新援助防御玩家行军的部队
	...
	public boolean onMarchReturn(int origionId, int terminalId, List<ArmyInfo> selfArmys) {
		if (marchEntity.getMarchStatus() == WorldMarchStatus.MARCH_STATUS_RETURN_BACK_VALUE) {
			return true;
		}
		// 修改状态
		...
		for (ArmyInfo army : selfArmys) {
			...
			// int deadCount = leftArmy.getDeadCount();
			// int toWound = (int) (deadCount * getParent().getParent().getCfg().getDeadToWoundPer());
			//
			// leftArmy.setWoundedCount(leftArmy.getWoundedCount() + toWound);
			// leftArmy.setDeadCount(deadCount - toWound);
		...
		// marchTime = Math.min(marchEntity.getCallBackTime() -
		// marchEntity.getStartTime(), marchTime);
		// }
		...
		// 刷新出征
		this.updateMarch();
		DungeonRedisLog.log(getPlayerId(), "onMarchReturn {}", this);
		return true;
	}
	...
	public long getMarchNeedTime() {
		...
		List<MarchPart> parts = new ArrayList<MarchPart>(1);
		parts.add(new MarchPart(origion, terminal, false, true));
		double time = 0;
	...
	private double effectToMarchTime(double time, boolean slowDown, double slowDownTime) {
		...
		if (slowDown) {
			...
			slowDownTime /= 1 + player.getEffect().getEffVal(EffType.MARCH_SPD_SNOW_LAND, marchEntity.getEffectParams())
					* GsConst.EFF_PER;
			return time + slowDownTime;
		}

		int marchType = marchEntity.getMarchType();

		int speedUpEffectVal = 0;

		// 作用号203： 行军速度增加 -> 行军时间 = 基础时间/(1 + 作用值/10000)
		speedUpEffectVal += player.getEffect().getEffVal(EffType.MARCH_SPD, marchEntity.getEffectParams());

		// 作用号204： 攻击野怪行军速度增加 -> 行军时间 = 基础时间/(1 + 作用值/10000)
		if (WorldUtil.isAtkMonsterMarch(this)) {
			speedUpEffectVal += player.getEffect().getEffVal(EffType.MARCH_SPD_MONSTER, marchEntity.getEffectParams());
		}

		// 作用号206： 侦察时行军速度增加 -> 行军时间 = 基础时间/(1 + 作用值/10000)
		if (WorldUtil.isSpyMarch(marchType)) {
			speedUpEffectVal += player.getEffect().getEffVal(EffType.MARCH_SPD_SPY, marchEntity.getEffectParams());
		}

		// 作用号211： 新版野怪行军速度增加 -> 行军时间 = 基础时间/(1 + 作用值/10000)
		if (marchType == WorldMarchType.NEW_MONSTER_VALUE) {
			speedUpEffectVal += player.getEffect().getEffVal(EffType.MARCH_SPD_NEW_MONSTER,
					marchEntity.getEffectParams());
		}

		// 作用号212： 野怪行军速度增加 -> 行军时间 = 基础时间/(1 + 作用值/10000)
		if (WorldUtil.isAtkMonsterMarch(this) || marchType == WorldMarchType.NEW_MONSTER_VALUE) {
			speedUpEffectVal += player.getEffect().getEffVal(EffType.MARCH_SPD_ALL_MONSTER,
					marchEntity.getEffectParams());
		}

		// 作用号610： 联盟交易时行军速度增加 -> 行军时间 = 基础时间/(1 + 作用值/10000)
		if (marchType == WorldMarchType.ASSISTANCE_RES_VALUE) {
			speedUpEffectVal += player.getEffect().getEffVal(Const.EffType.GUILD_TRADE_MARCH_SPD,
					marchEntity.getEffectParams());
		}

		// 作用号614： 联盟援助时，援军部队行军速度增加 -> 行军时间 = 基础时间/(1 + 作用值/10000)
		if (this.isAssistanceMarch()) {
			speedUpEffectVal += player.getEffect().getEffVal(Const.EffType.GUILD_HELP_MARCH_SPD,
					marchEntity.getEffectParams());
		}
		...
		if (this.isMassJoinMarch()) {
			Optional<IFGYLWorldPoint> targetPoint = getParent().getParent().getWorldPoint(marchEntity.getTerminalId());
			IFGYLWorldPoint iswWorldPoint = targetPoint.orElse(null);
			if (iswWorldPoint instanceof IFGYLPlayer) {
				IFGYLPlayer leader = (IFGYLPlayer) iswWorldPoint;
				slowDownEffectVal += leader.getEffect().getEffVal(Const.EffType.MARCH_SPD_MASS,
						marchEntity.getEffectParams());
			}
		}

		// // 作用号445：玩家基地被侦察或攻击时，敌人行军时间提升XX倍 -> 实际行军时间 = 其他作用号加速后行军时间 *（1 +
		// // 作用值/10000）
		// IJBFGYLorldPoint terminalPoint;
		// if (this.isReturnBackMarch()) {
		// terminalPoint =
		// getParent().getParent().getWorldPoint(marchEntity.getOrigionId()).orElse(null);
		// } else {
		// terminalPoint =
		// getParent().getParent().getWorldPoint(marchEntity.getTerminalId()).orElse(null);
		// }
		//
		...
		// && terminalPoint != null
		// && terminalPoint instanceof IJBSPlayer) {
		// IJBSPlayer defPlayer = (IJBSPlayer) terminalPoint;
		// if (defPlayer != null &&
		// defPlayer.getEffect().getEffVal(EffType.CITY_ENEMY_MARCH_SPD) > 0) {
		// slowDownEffectVal +=
		// defPlayer.getEffect().getEffVal(EffType.CITY_ENEMY_MARCH_SPD);
		// }
		// }
	...
	public double getMarchBaseSpeed() {
		...
		if (isMassMarch()) {
			Set<IFGYLWorldMarch> joinMarchs = getMassJoinMarchs(true);
			for (IFGYLWorldMarch tempMarch : joinMarchs) {
	...
	public void updateMarch() {
		HawkProtocol protocol = HawkProtocol.valueOf(HP.code.WORLD_MARCH_UPDATE_PUSH_VALUE,
				this.toBuilder(WorldMarchRelation.SELF));
	...
	public GuildWarTeamInfo.Builder getGuildWarInitiativeInfo() {
		...
		// 联盟编队
		FGYLGuildFormationObj formationObj = getParent().getParent().getGuildFormation(leader.getGuildId());
		GuildFormationCell formation = formationObj.getGuildFormation(getMarchId());
		// 队长位置
		...
		// 加入集结的行军
		Set<IFGYLWorldMarch> joinMarchs = this.getMassJoinMarchs(false);
		for (IFGYLWorldMarch joinMarch : joinMarchs) {
		...
		if (formation != null) {
			for (String fightId : formation.getFightIds()) {
				...
				Optional<IFGYLWorldMarch> findAny = joinMarchs.stream().filter(m -> m.getPlayerId().equals(fightId)).findAny();
				...
				FGYLGuildFormationModule module = joinMemeber.getModule(FGYLConst.ModuleType.FGYLGuildFormation);
	...
	public GuildWarTeamInfo.Builder getGuildWarPassivityInfo() {
		...
		IFGYLPlayer leader = FGYLRoomManager.getInstance().makesurePlayer(leaderId);
		// 队长位置
		...
		List<IFGYLWorldMarch> assistandMarchs = getParent().getParent().getPointMarches(leader.getPointId(),
				WorldMarchStatus.MARCH_STATUS_MARCH_ASSIST, WorldMarchType.ASSISTANCE);
		for (IFGYLWorldMarch assistandMarch : assistandMarchs) {
	...
	/*** 返回家中 */
	public abstract void onMarchBack();
	...
	public <T extends IFGYLWorldPoint> T getTerminalWorldPoint() {
		return (T) getParent().getParent().getWorldPoint(getTerminalId()).orElse(null);
	}
	...
	public void setFgylSkill(int fgylSkill) {
	...
	public void setUseHonor(int useHonor) {
		this.useHonor = useHonor;
	}
