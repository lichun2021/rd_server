 ...
 * @author hf
 */
@ObeliskMission(missionType = ObeliskMissionType.WorldNationStatus)
public class WorldNationStatusMission implements IObeliskMission {
	...
	public void onTick(ObeliskMissionItem missionItem) {
		...
		if (missionItem.getState() == Obelisk.PBObeliskMissionState.OPEN) {
			...
			if (Objects.nonNull(nationStatus) && nationStatus.getNumber() >= missionItem.getObeliskCfg().getPara1()) {
				missionItem.setNum(missionItem.getObeliskCfg().getCount());
			}
	...
	public Builder buildPbToPlayer(Player player, ObeliskMissionItem missionItem) {
		...
		if (missionItem.getState() == Obelisk.PBObeliskMissionState.NOTOPEN) {
			...
			if (obeliskEntity.getState().getNumber() < Obelisk.PBObeliskPlayerState.FAILED_VALUE) {
				...
				HawkLog.logPrintln("WorldNationStatusMission buildPbToPlayer end change state playerId:{},obeliskNum:{},state:{}", player.getId(), obeliskNum,
