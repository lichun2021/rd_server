 ...
 *
 */
@HawkConfigManager.XmlResource(file = "xml/plant_soldier_crack.xml")
public class PlantSoldierCrackCfg extends HawkConfigBase {
	...
	private static Map<SoldierType, PlantSoldierCrackCfg> zeroLevelCfgMap = new HashMap<>();
	private SoldierType type;

	public PlantSoldierCrackCfg() {
		id = 0;
		level = 0;
		power = 0;
		chips = "";
	...
	protected boolean assemble() {
		...
		if (level == 0 && frontStage == 0) {
			zeroLevelCfgMap.put(SoldierType.valueOf(soldierType), this);
		}
	...
	protected boolean checkValid() {
		...
		if (frontStage != 0) {
			PlantSoldierCrackCfg cfg = HawkConfigManager.getInstance().getConfigByKey(PlantSoldierCrackCfg.class, frontStage);
			if (cfg == null) {
		...
		if (postStage != 0) {
			PlantSoldierCrackCfg cfg = HawkConfigManager.getInstance().getConfigByKey(PlantSoldierCrackCfg.class, postStage);
			if (cfg == null) {
	...
	public static Map<SoldierType, PlantSoldierCrackCfg> getZeroLevelCfgMap() {
