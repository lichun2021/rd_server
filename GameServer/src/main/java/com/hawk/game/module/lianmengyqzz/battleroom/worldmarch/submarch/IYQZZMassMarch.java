...
/** 集结行军 */
public abstract class IYQZZMassMarch extends IYQZZWorldMarch {

	public IYQZZMassMarch(IYQZZPlayer parent) {
		super(parent);
	}
	...
	public void waitingStatusMarchProcess() {
		...
		Set<IYQZZWorldMarch> reachJoinMarchs = getMassJoinMarchs(true);
	...
	public int getMaxMassJoinSoldierNum(Player leader, Player perReachMarchPlayer) {
		int result = super.getMaxMassJoinSoldierNum(leader, perReachMarchPlayer);
		result += leader.getEffect().getEffVal(EffType.YQZZ_634);
		return result;
	}

	@Override
	public Set<IYQZZWorldMarch> getMassJoinMarchs(boolean needReach) {
		// 等待和到达的行军
		Set<IYQZZWorldMarch> retMarchs = new HashSet<>();
		// 不是队长行军
		if (isMassMarch()) {
			// 已到达队长家的行军
			List<IYQZZWorldMarch> passiveMarchs = getParent().getParent().getWorldMarchList();
			for (IYQZZWorldMarch march : passiveMarchs) {
				// 类型不匹配
	...
	public void teamMarchReached(IYQZZMassJoinMarch massJoinMarch) {
		if (this instanceof IYQZZReportPushMarch) {
			((IYQZZReportPushMarch) this).pushAttackReport();
		}
		massJoinMarch.setHashThread(getHashThread());
	}

	public void teamMarchCallBack(IYQZZMassJoinMarch massJoinMarch) {
		if(this instanceof IYQZZReportPushMarch){
			((IYQZZReportPushMarch)this).pushAttackReport();
		}
	}
