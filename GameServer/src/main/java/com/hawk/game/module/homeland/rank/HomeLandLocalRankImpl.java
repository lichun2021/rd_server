 ...
 * @author zhy
 */
public class HomeLandLocalRankImpl implements HomeLandRankImpl {
    ...
    public HomeLandLocalRankImpl(HomeLandRankCfg cfg, String serverId) {
        this.cfg = cfg;
    ...
    protected String getRedisKey() {
        return HOME_LAND_RANK + HomeLandRankType.getValue(this.cfg.getType()).name() + ":" +
                HomeLandRankServerType.getValue(this.cfg.getRange()).name() + ":" + serverId;
    }
    ...
    public void doRankSort() {
        ...
        for (Tuple rank : rankSet) {
            if (!GlobalData.getInstance().isLocalPlayer(rank.getElement())) {
