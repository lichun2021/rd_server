...
import com.hawk.game.module.lianmenxhjz.battleroom.worldpoint.XHJZBase;
import com.hawk.game.module.lianmenxhjz.battleroom.worldpoint.XHJZBuildType;
import com.hawk.game.module.lianmenxhjz.battleroom.worldpoint.XHJZCommandCenter;
import com.hawk.game.module.lianmenxhjz.battleroom.worldpoint.XHJZHeadQuarter;
import com.hawk.game.module.lianmenxhjz.battleroom.worldpoint.XHJZTower;
import com.hawk.game.module.lianmenxhjz.battleroom.worldpoint.state.XHJZBuildingStateZhanLing;
import com.hawk.game.util.GameUtil;
import com.hawk.game.util.GsConst;
import com.hawk.game.world.object.AreaObject;
import com.hawk.game.world.object.Point;

public class XHJZWorldPointService {
	private final XHJZBattleRoom parent;
	private boolean inited;

	private XHJZWorldScene worldScene;
	/**
	 * 资源区域块 <块id,块数据>
	 */
	private Map<Integer, XHJZAreaObject> areas = new ConcurrentHashMap<>();
	...
	private Multimap<XHJZBuildType, ? super IXHJZBuilding> builTypeMap = HashMultimap.create();
	private List<IXHJZBuilding> buildingList = new CopyOnWriteArrayList<>();
	private Map<Integer, IXHJZBuilding> buildIdBuildMap = new ConcurrentHashMap<>();
	/** 服务器对应国家飞船*/
	private Map<XHJZ_CAMP, XHJZBase> baseBuildMap = new ConcurrentHashMap<>();
	/** 战场中的对象 玩家城点, 怪, npc建筑等等 */
	private Map<Integer, IXHJZWorldPoint> viewPoints = new ConcurrentHashMap<>();


	public XHJZWorldPointService(XHJZBattleRoom parent) {
		this.parent = parent;
	}
	...
	public void init() {
		...
		XHJZBattleCfg bcfg = parent.getCfg();
		worldMaxX = bcfg.getMapX() + 1;
		worldMaxY = bcfg.getMapY() + 1;
		initWorldScene();
		loadWorldAreas();

		ConfigIterator<XHJZBuildCfg> buildRefreshIt = HawkConfigManager.getInstance().getConfigIterator(XHJZBuildCfg.class);
		for (XHJZBuildCfg rfcfg : buildRefreshIt) {
			refreshBuild(rfcfg);
		}
	...
	private void refreshBuild(XHJZBuildCfg bcfg) {
		try {
			int index = 0;
			IXHJZBuilding icd = null;
			switch (bcfg.getBuildTypeId()) {
			case 8:
				icd = new XHJZBase(parent);
				XHJZ_CAMP bornCamp = XHJZ_CAMP.valueOf(bcfg.getCamp());
				((XHJZBase) icd).setBornCamp(bornCamp);
				baseBuildMap.put(bornCamp, (XHJZBase) icd);
			
				break;
			case 7:
				icd = new XHJZHeadQuarter(parent);
				break;
			case 5:
			case 9:
				icd = new XHJZTower(parent);
				break;
			default:
				icd = new XHJZCommandCenter(parent);
				break;
			}
			...
			icd.setProtectedEndTime(getParent().getCreateTime() + icd.getBuildTypeCfg().getPeaceTime() * 1000);
//			icd.setFoggyFortressId(icd.getBuildTypeCfg().getFoggyFortressId());
				icd.setStateObj(new XHJZBuildingStateZhanLing(icd));
	...
	/** 添加世界点 */
	public void addViewPoint(IXHJZWorldPoint... vp) {
		for (IXHJZWorldPoint point : vp) {
			if (point == null) {
				continue;
			}
			viewPoints.put(point.getPointId(), point);
			int type = point instanceof IXHJZPlayer ? GsConst.WorldObjType.CITY : point.getPointType().getNumber();
	...
	public boolean removeViewPoint(IXHJZWorldPoint vp) {
	...
	private void initWorldScene() {
		...
		// 初始化世界场景
		worldScene = new XHJZWorldScene(worldMaxX, worldMaxY, searchRadius);
	}

	private void loadWorldAreas() {
		// 创建区域对象列表
		List<XHJZAreaObject> areaList = createWorldAreas();
		for (XHJZAreaObject areaObj : areaList) {
			// 创建区域内所有点对象
	...
	private List<XHJZAreaObject> createWorldAreas() {
		List<XHJZAreaObject> areaList = new LinkedList<>();
		...
		for (int row = 0; row < areaRows; row++) {
			for (int col = 0; col < areaCols; col++) {
				...
				XHJZAreaObject areaObj = new XHJZAreaObject(areaId, beginX, beginY, endX, endY);
	...
	public XHJZWorldScene getWorldScene() {
		return worldScene;
	}

	public Map<Integer, XHJZAreaObject> getAreas() {
		return areas;
	}
	...
	public boolean tryOccupied(int x, int y, int distance) {
		XHJZAreaObject areaObj = getArea(x, y);
		...
		// 中心点是否被占用
		Optional<IXHJZWorldPoint> currPoint = parent.getWorldPoint(centerPoint.getX(), centerPoint.getY());
	 ...
	 * @return
	 */
	public List<IXHJZWorldPoint> getAroundWorldPointsWithType(int centerX, int centerY, int radiusX, int radiusY, int pointType) {
		List<Point> points = getRhoAroundPointsAll(centerX, centerY, radiusX, radiusY);
		List<IXHJZWorldPoint> aroundWorldPoints = new ArrayList<>(points.size());
		for (Point point : points) {
			IXHJZWorldPoint worldPoint = getWorldPoint(point.getX(), point.getY());
	 ...
	 * @return
	 */
	public IXHJZWorldPoint getWorldPoint(int x, int y) {
	 ...
	 * @return
	 */
	public IXHJZWorldPoint getWorldPoint(int pointId) {
	 ...
	 * @return
	 */
	public XHJZAreaObject getArea(int areaId) {
		return getAreas().get(areaId);
	}

	public Collection<XHJZAreaObject> getAreaVales() {
	 ...
	 * @return
	 */
	public XHJZAreaObject getArea(int x, int y) {
	 ...
	 * @param worldPoint
	 */
	public void addToAreaFreePoint(IXHJZWorldPoint worldPoint) {
		// 自身点从AreaObject移除
	 ...
	 * @param worldPoint
	 */
	public void rmFromAreaFreePoint(IXHJZWorldPoint worldPoint) {
		// 自身点从AreaObject移除
		XHJZAreaObject areaObject = getArea(worldPoint.getAreaId());
		...
		for (Point point : aroundPoints) {
			XHJZAreaObject aroundAreaObject = getArea(point.getAreaId());
			aroundAreaObject.removeFreePoint(point.getX(), point.getY());
		}
	}

//	public int[] randomBornPoint(IXHJZPlayer player) {
//		XHJZBase base = getBaseByCamp(player.getCamp());
//		return randomSubareaPoint(base.getSubarea(), player.getGridCnt());
//	}

//	public int[] randomSubareaPoint(int subareaNum, int redis) {
//		try {
//			XHJZRandPointSeed pointSeed = XHJZMapBlock.getInstance().getRandPointSeedBySubareaNum(subareaNum);
...
//			HawkException.catchException(e);
//		}
//		IXHJZBuilding build = HawkRand.randomObject(getBuildingByType(XHJZBuildType.BA));
...
//	}
	...
	public List<IXHJZBuilding> getBuildingList() {
		return buildingList;
	}

	@SuppressWarnings("unchecked")
	public <T extends IXHJZBuilding> List<T> getBuildingByType(XHJZBuildType type) {
	...
	/**通过建筑id取得建筑*/
	@SuppressWarnings("unchecked")
	public <T extends IXHJZBuilding> T getBuildingByCfgId(int buildId) {
		return (T) buildIdBuildMap.get(buildId);
	}

	/**取得阵营国家飞船*/
	public XHJZBase getBaseByCamp(XHJZ_CAMP camp) {
		return baseBuildMap.get(camp);
	}

	public IXHJZBuilding randomBuildByType(XHJZBuildType type) {
		List<IXHJZBuilding> list = getBuildingByType(type);
		if (list.isEmpty()) {
			return null;
		}
	...
	public Map<Integer, IXHJZWorldPoint> getViewPoints() {
		return viewPoints;
	}
