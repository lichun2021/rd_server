...
/** 集结行军 */
public abstract class IXQHXMassMarch extends IXQHXWorldMarch {

	public IXQHXMassMarch(IXQHXPlayer parent) {
		super(parent);
	}
	...
	public void waitingStatusMarchProcess() {
		...
		Set<IXQHXWorldMarch> reachJoinMarchs = getMassJoinMarchs(true);
	...
	@Override
	public Set<IXQHXWorldMarch> getMassJoinMarchs(boolean needReach) {
		// 等待和到达的行军
		Set<IXQHXWorldMarch> retMarchs = new HashSet<>();
		// 不是队长行军
		if (isMassMarch()) {
			// 已到达队长家的行军
			List<IXQHXWorldMarch> passiveMarchs = getParent().getParent().getWorldMarchList();
			for (IXQHXWorldMarch march : passiveMarchs) {
				// 类型不匹配
	...
	public void teamMarchReached(IXQHXMassJoinMarch JBSMassJoinMarch) {
		if(this instanceof IXQHXReportPushMarch){
			((IXQHXReportPushMarch)this).pushAttackReport();
		}
	}

	public void teamMarchCallBack(IXQHXMassJoinMarch JBSMassJoinMarch) {
		if(this instanceof IXQHXReportPushMarch){
			((IXQHXReportPushMarch)this).pushAttackReport();
		}
	}
