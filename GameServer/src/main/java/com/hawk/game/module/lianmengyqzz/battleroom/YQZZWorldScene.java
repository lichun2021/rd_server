...
import com.hawk.game.module.lianmengyqzz.battleroom.player.YQZZPlayerEye;
 ...
 * @date 2020年9月14日
 */
public class YQZZWorldScene extends HawkAOIScene {
	 ...
	 * @param maxRadius
	 */
	public YQZZWorldScene(int width, int height, int searchRadius) {
	...
	protected void notifyObjEnter(HawkAOIObj aoiObj, Set<HawkAOIObj> targetObjs) {
		...
		IYQZZWorldPoint worldPoint = (IYQZZWorldPoint) aoiObj.getUserData();
		if (worldPoint == null) {
			return;
		}
		...
		for (HawkAOIObj targetObj : targetObjs) {
			if (targetObj.getType() == WorldObjType.PLAYER) {
				YQZZPlayerEye playerEye = (YQZZPlayerEye) targetObj.getUserData();
				if (playerEye == null) {
					continue;
				}
				builder.clearPoints();
	...
	protected void syncSceneObjs(HawkAOIObj aoiObj, Set<HawkAOIObj> oldInviewObjs, Set<HawkAOIObj> nowInviewObjs, float moveSpeed) {
		if (aoiObj.getType() == GsConst.WorldObjType.PLAYER && GsApp.getInstance().isRunning()) {
			YQZZPlayerEye playerEye = (YQZZPlayerEye) aoiObj.getUserData();
			if (playerEye == null) {
				return;
			}
			...
			for (HawkAOIObj inviewObj : inviewObjs) {
				try {
					...
					IYQZZWorldPoint worldPoint = (IYQZZWorldPoint) inviewObj.getUserData();
					if (worldPoint == null) {
						continue;
					}
	 ...
	 * @return
	 */
	public List<WorldPointPB.Builder> getPlayerViewObjs(YQZZPlayerEye viewer, int x, int y, float moveSpeed) {
	 ...
	 * @return
	 */
	private int fillWorldPoints(YQZZPlayerEye viewer, Set<HawkAOIObj> inviewObjs, float moveSpeed, List<WorldPointPB.Builder> builderList) {
		float randFactor = 1.0f;
		if (!HawkOSOperator.isZero(GameConstCfg.getInstance().getMoveSyncFactor())) {
			randFactor = (float) Math.pow(GameConstCfg.getInstance().getMoveSyncFactor() - moveSpeed, 2);
		}
