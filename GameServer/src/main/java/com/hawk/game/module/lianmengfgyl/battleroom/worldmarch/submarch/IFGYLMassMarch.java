...
/** 集结行军 */
public abstract class IFGYLMassMarch extends IFGYLWorldMarch {

	public IFGYLMassMarch(IFGYLPlayer parent) {
		super(parent);
	}
	...
	public void waitingStatusMarchProcess() {
		...
		Set<IFGYLWorldMarch> reachJoinMarchs = getMassJoinMarchs(true);
	...
	@Override
	public Set<IFGYLWorldMarch> getMassJoinMarchs(boolean needReach) {
		// 等待和到达的行军
		Set<IFGYLWorldMarch> retMarchs = new HashSet<>();
		// 不是队长行军
		if (isMassMarch()) {
			// 已到达队长家的行军
			List<IFGYLWorldMarch> passiveMarchs = getParent().getParent().getWorldMarchList();
			for (IFGYLWorldMarch march : passiveMarchs) {
				// 类型不匹配
	...
	public void teamMarchReached(IFGYLMassJoinMarch JBSMassJoinMarch) {
		if(this instanceof IFGYLReportPushMarch){
			((IFGYLReportPushMarch)this).pushAttackReport();
		}
	}

	public void teamMarchCallBack(IFGYLMassJoinMarch JBSMassJoinMarch) {
		if(this instanceof IFGYLReportPushMarch){
			((IFGYLReportPushMarch)this).pushAttackReport();
		}
	}
