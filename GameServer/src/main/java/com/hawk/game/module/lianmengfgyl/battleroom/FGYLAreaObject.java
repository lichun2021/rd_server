...
public class FGYLAreaObject extends AreaObject {

	public FGYLAreaObject(int areaId, int beginX, int beginY, int endX, int endY) {
	...
	public boolean initAreaPoints(FGYLBattleRoom battleroom) {
		for (int i = beginX; i <= endX; i++) {
			for (int j = beginY; j <= endY; j++) {
				if (FGYLMapBlock.getInstance().isStopPoint(GameUtil.combineXAndY(i, j))) {
					continue;
				}
