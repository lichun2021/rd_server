 ...
 * @author lwt
 * @date 2019年1月2日 */
public class XQHXMassSingleMarch extends IXQHXMassMarch implements IXQHXReportPushMarch {

	public XQHXMassSingleMarch(IXQHXPlayer parent) {
		super(parent);
	}
	...
	public void onMarchReach(Player parent) {
		// 行军
		XQHXMarchEntity atkLeaderMarch = getMarchEntity();
		Set<IXQHXWorldMarch> massJoinMarchs = getMassJoinMarchs(true);
		// 目标点
		...
		// 填充参与集结信息
		for (IXQHXWorldMarch massJoinMarch : massJoinMarchs) {
		...
		// 防守方援军
		List<IXQHXWorldMarch> helpMarchList = defplayer.assisReachMarches();
		for (IXQHXWorldMarch iWorldMarch : helpMarchList) {
		...
		battleIncome.setXQHXMail(getParent().getParent().getExtParm());
		// 战斗数据输出
	...
	public GuildWarTeamInfo.Builder getGuildWarPassivityInfo() {
		...
		List<IXQHXWorldMarch> assistandMarchs = getParent().getParent().getPointMarches(
				leader.getPointId(),
	...
	public Set<String> attackReportRecipients() {
		// 防守方援军
		List<IXQHXWorldMarch> helpMarchList = getParent().getParent().getPointMarches(getMarchEntity().getTerminalId(),
				WorldMarchStatus.MARCH_STATUS_MARCH_ASSIST,
				WorldMarchType.ASSISTANCE);
