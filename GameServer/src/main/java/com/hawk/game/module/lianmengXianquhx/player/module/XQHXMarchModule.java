...
import com.hawk.game.module.lianmengXianquhx.invoker.XQHXItemConsumeInvoker;
...
public class XQHXMarchModule extends PlayerModule {
	private IXQHXPlayer player;
	public XQHXMarchModule(IXQHXPlayer player) {
		super(player);
		this.player = player;
	}
	...
	private boolean onWorldPylonMarchStart(HawkProtocol protocol) {
		...
		// 目标点
		IXQHXWorldPoint ipoint = player.getParent().getWorldPoint(req.getPosX(), req.getPosY()).orElse(null);

		// 坐标点检查
		if (ipoint == null || ipoint.getPointType() != WorldPointType.XQHX_PYLON) {
			sendError(protocol.getType(), Status.Error.WORLD_POINT_NOT_EXIST);
			return false;
		}
		List<IXQHXWorldMarch> playerMarches = player.getParent().getPlayerMarches(player.getId(), WorldMarchType.PYLON_MARCH);
...
//		}
		...
		XQHXPylon point = (XQHXPylon) ipoint;
		// 所需体力
...
//		}
		...
		if (totalCnt < CrossConstCfg.getInstance().getCrossPylonArmyCountLimit()) {
			sendError(protocol.getType(), Status.XQHXError.XQHX_CROSS_PYLON_MARCH_LIMIT);
			return false;
		}
		...
		// 生成行军
		IXQHXWorldMarch worldMatch = player.getParent().startMarch(player, player, point, WorldMarchType.PYLON_MARCH, point.getResourceId()+"", 0, new EffectParams(req, armyList));
		
//		((XQHXPylonMarch)worldMatch).setVitBack(cfg.getStrongpointCost());
//		
//		// 扣除体力
//		consumeItems.consumeAndPush(player, Action.WORLD_PYLON_MARCH_VIT_COST);
	...
	public boolean marchEmoticonUse(HawkProtocol protocol, int emoticonId, String marchId) {
		IXQHXWorldMarch march = player.getParent().getMarch(marchId);
		if (march == null) {
		...
		if (march.getMarchStatus() != WorldMarchStatus.MARCH_STATUS_MARCH_VALUE 
			...
			IXQHXWorldPoint worldPoint = player.getParent().getWorldPoint(march.getTerminalId()).get();
			if (worldPoint instanceof IXQHXBuilding && ((IXQHXBuilding) worldPoint).getLeaderMarch() != null) {
				WorldMarch firstMarch = ((IXQHXBuilding) worldPoint).getLeaderMarch().getMarchEntity();
	...
	private boolean onInviteMass(HawkProtocol protocol) {
		...
		IXQHXWorldMarch march = player.getParent().getMarch(req.getMarchId());
		if (march == null) {
		...
		Optional<IXQHXWorldPoint> worldPoint = player.getParent().getWorldPoint(march.getMarchEntity().getTerminalId());
		if (worldPoint.isPresent()) {
			XQHXGuildFormationObj formationObj = player.getParent().getGuildFormation(player.getGuildId());
	 ...
	 * 发送公告
	 */
	public void sendNotice(IXQHXWorldPoint point, IXQHXWorldMarch march, int formationIndex, String formationName) {
		IXQHXPlayer player = march.getParent();
		if (point instanceof IXQHXPlayer) {
			ChatParames parames = ChatParames.newBuilder().setPlayer(player).setChatType(ChatType.CHAT_FUBEN_TEAM).setKey(NoticeCfgId.XQHX_MASS_START_PLAYER)
					.addParms(((IXQHXPlayer) point).getName())
		...
		} else if (point instanceof IXQHXBuilding) {
			ChatParames parames = ChatParames.newBuilder().setPlayer(player).setChatType(ChatType.CHAT_FUBEN_TEAM).setKey(NoticeCfgId.XQHX_MASS_START_BUILD)
					.addParms(point.getX())
					.addParms(point.getY())
					.addParms(march.getMarchId()).addParms(player.getX()).addParms(player.getY()).addParms(formationIndex)
					.addParms(formationName).build();
	...
	private void onYuriMonsterQueryHelp(HawkProtocol protocol) {
		...
		// 行军已不存在
		IXQHXWorldMarch worldMarch = player.getParent().getMarch(req.getMarhcId());
		if (worldMarch == null) {
			return;
		}

		Optional<IXQHXWorldPoint> point = player.getParent().getWorldPoint(worldMarch.getTerminalId());
		int pointType = point.isPresent() ? point.get().getPointType().getNumber() : 0;
		ChatParames parames = ChatParames.newBuilder().setPlayer(player).setChatType(ChatType.CHAT_FUBEN_TEAM).setKey(NoticeCfgId.XQHX_MARCH_QUERY_HELP)
	...
	private boolean onRepatriateManormarch(HawkProtocol protocol) {
		...
		// 世界点
		IXQHXWorldPoint worldPoint = player.getParent().getWorldPoint(req.getX(), req.getY()).orElse(null);
		if (worldPoint == null) {
		...
		// 战区
		if (worldPoint instanceof IXQHXBuilding) {
			IXQHXBuilding building = (IXQHXBuilding) worldPoint;
	...
	protected boolean onWorldMassDissolve(HawkProtocol protocol) {
		...
		// check
		IXQHXWorldMarch worldMarch = player.getParent().getMarch(req.getMarchId());
		if (worldMarch == null) {
			player.sendError(protocol.getType(), Status.Error.WORLD_MARCH_NOT_EXISIT);
			return false;
		}
		...
		for (IXQHXWorldMarch jm : worldMarch.getMassJoinMarchs(true)) {
			jm.onMarchCallback();
			// // 发送邮件---集结失败：发起者解散
	...
	protected boolean onWorldMassRepatriate(HawkProtocol protocol) {
		...
		IXQHXWorldMarch leaderMarch = player.getParent().getMarch(march.getTargetId());
		if (leaderMarch == null) {
			player.sendError(protocol.getType(), Status.Error.WORLD_MARCH_NOT_LEADER);
			return false;
		}
	...
	/**
	 * XQHX_SINGLE_MARCH = 487; // 副本内特有建筑单人行军
	 */
	@ProtocolHandler(code = HP.code2.XQHX_SINGLE_MARCH_VALUE)
	private boolean onXQHX_SINGLE_MARCHStart(HawkProtocol protocol) {
		WorldMarchReq req = protocol.parseProtocol(WorldMarchReq.getDefaultInstance());
		IXQHXWorldPoint point = player.getParent().getWorldPoint(req.getPosX(), req.getPosY()).orElse(null);
		// 路点为空
		if (point == null) {
			player.sendError(protocol.getType(), Status.Error.WORLD_POINT_NOT_EXIST);
			return false;
		...
		if (point.getProtectedEndTime() > player.getParent().getCurTimeMil()) {
			player.sendError(protocol.getType(), Status.XQHXError.XQHX_BUILD_PROCTED);
			return false;
		}
		...
		case XQHX_BUILDING:// 指挥部
			marchType = WorldMarchType.XQHX_BUILDING_SINGLE;
			break;
		default:
		...
		WorldMarchResp.Builder builder = WorldMarchResp.newBuilder();
		builder.setSuccess(true);
		protocol.response(HawkProtocol.valueOf(HP.code2.XQHX_SINGLE_MARCH_S, builder));
		return true;
...
//		WorldMarchReq req = protocol.parseProtocol(WorldMarchReq.getDefaultInstance());
//		IXQHXWorldPoint reqPoint = player.getParent().getWorldPoint(req.getPosX(), req.getPosY()).orElse(null);
//
//		// check点数据
//		if (reqPoint == null || !(reqPoint instanceof XQHXFuelBank)) {
//			player.sendError(protocol.getType(), Status.Error.WORLD_POINT_NOT_RESOURCE);
//			return false;
//		}
//		XQHXFuelBank point = (XQHXFuelBank) reqPoint;
//
//		// 带兵出征通用检查
//		List<ArmyInfo> armyList = new ArrayList<ArmyInfo>(); // 出征的士兵列表
//		if (!checkMarchReq(player, req, protocol.getType(), armyList, point, true)) {
...
//	}
	...
	private boolean onMarchArmyDetailInfo(HawkProtocol protocol) {
		...
		Set<IXQHXWorldMarch> allMarch = new HashSet<>();
		allMarch.add(march);
		...
		// 是否有自己的行军
		for (IXQHXWorldMarch thisMarch : allMarch) {
		...
		for (IXQHXWorldMarch thisMarch : allMarch) {
			...
			IXQHXPlayer mp = thisMarch.getParent();
	...
	private boolean onWorldAttackPlayerStart(HawkProtocol protocol) {
		...
		if(point.getPointType() == WorldPointType.PLAYER){
			player.sendError(protocol.getType(), Status.XQHXError.XQHX_MARCH_BAN);
			return false;
		}
		if (HawkTime.getMillisecond() < point.getProtectedEndTime()) {
			return false;
		...
		// 目标玩家和自己同盟
		IXQHXPlayer target = (IXQHXPlayer) point;
...
//		}
		...
		if (march.speedUp(timeReducePercent, speedUpTimes)) {
			// 消耗掉
			player.dealMsg(MsgId.WORLD_MARCH, new XQHXItemConsumeInvoker(player, consumeItems, Action.WORLD_MARCH_SPEEDUP));
	...
	private boolean onMarchCallBack(HawkProtocol protocol) {
		...
		// 检测行军是否存在
		IXQHXWorldMarch march = player.getParent().getPlayerMarch(player.getId(), req.getMarchId());
		if (march == null) {
	...
	private boolean onPointMarchCallBack(HawkProtocol protocol) {
		...
		IXQHXWorldMarch march = player.getParent().getPointMarches(GameUtil.combineXAndY(req.getX(), req.getY())).stream()
	...
	private boolean onWorldSpyStart(HawkProtocol protocol) {
		...
		if (useGold && player instanceof XQHXPlayer) {
		...
		String targetId = "";
		if (point instanceof XQHXPlayer) {
			IXQHXPlayer targetPlayer = (IXQHXPlayer) point;
			// 普通玩家基地，放玩家的ID
	...
	private boolean onWorldAssistanceStart(HawkProtocol protocol) {
		...
		// 获取目标玩家
		final IXQHXPlayer tarPlayer = (IXQHXPlayer) point;
	 ...
	 * @return
	 */
	public boolean isHasFreeMarch(IXQHXPlayer player) {
		// 机器人不受行军数量限制
		if (player.isRobot()) {
			return true;
		}
		List<IXQHXWorldMarch> playerMarches = player.getParent().getPlayerMarches(player.getId());
		// 行军个数
		int marchCount = 0;
		for (IXQHXWorldMarch march : playerMarches) {
	 ...
	 * @return
	 */
	public boolean checkMarchReq(IXQHXPlayer player, WorldMarchReq req, int hpCode, List<ArmyInfo> armyList, IXQHXWorldPoint worldPoint, boolean checkFreeMarch) {
		if(player.isAnchor()){
			return false;
		}
		...
		if (player.getParent().getStartTime() > HawkTime.getMillisecond()) {
			player.sendError(hpCode, Status.XQHXError.XQHX_PRETIME_CANNOT_MARCH);
			return false;
		}
	...
	private boolean onWorldMassStart(HawkProtocol protocol) {
		...
		switch (point.getPointType()) {
		case XQHX_BUILDING:// 指挥部
			marchType = WorldMarchType.XQHX_BUILDING_MASS;
			break;
		...
		IXQHXWorldMarch march = player.getParent().startMarch(player, player, point, marchType, targetId, waitTime, new EffectParams(req, armyList));
	...
	private boolean onWorldMassJoinStart(HawkProtocol protocol) {
		...
		IXQHXWorldMarch worldMarch = player.getParent().getMarch(massMarchId);
		// 取队长行军
		if (worldMarch == null) {
			player.sendError(protocol.getType(), Status.Error.MASS_ERR_MARCH_NOT_EXIST);
			return false;
		...
		int marchType = req.getType().getNumber();
		// 不能加入自己的集结
		IXQHXPlayer leader = worldMarch.getParent();
		...
		IXQHXMassMarch massMarch = (IXQHXMassMarch) worldMarch;
		// 集结行军类型和参与集结行军类型不一致
		...
		// 已经有加入这支部队的集结
		Set<IXQHXWorldMarch> massJoinMarchs = massMarch.getMassJoinMarchs(false);
	...
	private boolean onWorldAttackMonsterStart(HawkProtocol protocol) {
		...
		// 目标点
		IXQHXWorldPoint targetPoint = player.getParent().getWorldPoint(posX, posY).orElse(null);
		...
		// 开启行军,目标放怪物ID
		XQHXMonster monster = (XQHXMonster) targetPoint;
	...
	public boolean checkArmyAndMarch(int protoType, IXQHXPlayer player, List<ArmyInfo> armyList, List<Integer> heroIdList, int superSoldierId) {
	...
	public boolean checkMarchDressReq(IXQHXPlayer player, List<Integer> marchDressList) {
		try {
	...
	private Activity.ChatMassCardInfo.Builder getChatMassCardInfo(String marchId, IXQHXPlayer viewer) {
		try {
