 ...
 * @author lwt
 * @date 2019年1月2日 */
public class TBLYMassSingleMarch extends ITBLYMassMarch implements ITBLYReportPushMarch {

	public TBLYMassSingleMarch(ITBLYPlayer parent) {
		super(parent);
	}
	...
	public void onMarchReach(Player parent) {
		// 行军
		TBLYMarchEntity atkLeaderMarch = getMarchEntity();
		Set<ITBLYWorldMarch> massJoinMarchs = getMassJoinMarchs(true);
		// 目标点
		...
		// 填充参与集结信息
		for (ITBLYWorldMarch massJoinMarch : massJoinMarchs) {
		...
		// 防守方援军
		List<ITBLYWorldMarch> helpMarchList = defplayer.assisReachMarches();
		for (ITBLYWorldMarch iWorldMarch : helpMarchList) {
	...
	public GuildWarTeamInfo.Builder getGuildWarPassivityInfo() {
		...
		List<ITBLYWorldMarch> assistandMarchs = getParent().getParent().getPointMarches(
				leader.getPointId(),
	...
	public boolean isEvident() {
		return ITBLYReportPushMarch.super.isEvident() || this.getMarchEntity().getMarchStatus() == WorldMarchStatus.MARCH_STATUS_WAITING_VALUE;
	}

	@Override
	public Set<String> attackReportRecipients() {
		// 防守方援军
		List<ITBLYWorldMarch> helpMarchList = getParent().getParent().getPointMarches(getMarchEntity().getTerminalId(),
				WorldMarchStatus.MARCH_STATUS_MARCH_ASSIST,
				WorldMarchType.ASSISTANCE);
