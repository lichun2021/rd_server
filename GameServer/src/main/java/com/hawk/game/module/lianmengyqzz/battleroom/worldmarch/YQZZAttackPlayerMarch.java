...
public class YQZZAttackPlayerMarch extends IYQZZWorldMarch implements IYQZZReportPushMarch {

	public YQZZAttackPlayerMarch(IYQZZPlayer parent) {
		super(parent);
	}
	...
	public void onMarchReach(Player parent) {
		...
		// 路点为空
		if (point == null || !(point instanceof IYQZZPlayer) || point.getProtectedEndTime() > getParent().getParent().getCurTimeMil()) {
		...
		atkMarchs.add(this);
		// 防守方援军
		List<IYQZZWorldMarch> helpMarchList = helpMarches();
		// 防守方行军
		...
		for (Player defer : defPlayers) {
			((IYQZZPlayer) defer).setLastAttacker(getParent());
		}
	}
	...
	private List<IYQZZWorldMarch> helpMarches() {
