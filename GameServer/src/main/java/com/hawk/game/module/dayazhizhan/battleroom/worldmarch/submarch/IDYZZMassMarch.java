...
/** 集结行军 */
public abstract class IDYZZMassMarch extends IDYZZWorldMarch {

	public IDYZZMassMarch(IDYZZPlayer parent) {
		super(parent);
	}
	...
	public void waitingStatusMarchProcess() {
		...
		Set<IDYZZWorldMarch> reachJoinMarchs = getMassJoinMarchs(true);
	...
	@Override
	public Set<IDYZZWorldMarch> getMassJoinMarchs(boolean needReach) {
		// 等待和到达的行军
		Set<IDYZZWorldMarch> retMarchs = new HashSet<>();
		// 不是队长行军
		if (isMassMarch()) {
			// 已到达队长家的行军
			List<IDYZZWorldMarch> passiveMarchs = getParent().getParent().getWorldMarchList();
			for (IDYZZWorldMarch march : passiveMarchs) {
				// 类型不匹配
	...
	public void teamMarchReached(IDYZZMassJoinMarch JBSMassJoinMarch) {
		if(this instanceof IDYZZReportPushMarch){
			((IDYZZReportPushMarch)this).pushAttackReport();
		}
	}

	public void teamMarchCallBack(IDYZZMassJoinMarch JBSMassJoinMarch) {
		if(this instanceof IDYZZReportPushMarch){
			((IDYZZReportPushMarch)this).pushAttackReport();
		}
	}
