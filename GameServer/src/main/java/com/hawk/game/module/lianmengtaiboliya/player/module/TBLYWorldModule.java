...
import com.hawk.game.module.lianmengtaiboliya.invoker.TBLYMoveCityMsgInvoker;
...
import com.hawk.game.protocol.TBLY.PBTBLYAnchorGetGuildWarCellInfoReq;
import com.hawk.game.protocol.TBLY.PBTBLYAnchorGetGuildWarInfoReq;
import com.hawk.game.protocol.TBLY.PBTBLYAnchorListMailReq;
import com.hawk.game.protocol.TBLY.PBTBLYChronoStartReq;
import com.hawk.game.protocol.TBLY.PBTBLYNuclearStartReq;
...
public class TBLYWorldModule extends PlayerModule {
	private ITBLYPlayer player;
	private int moveCnt;
	public TBLYWorldModule(ITBLYPlayer player) {
		super(player);
		this.player = player;
	}
	 ...
	 * 核弹发射
	 */
	@ProtocolHandler(code = HP.code.TBLY_NUCLEAR_SEND_C_VALUE)
	private void onNuclearStart(HawkProtocol protocol) {
		
		PBTBLYNuclearStartReq req = protocol.parseProtocol(PBTBLYNuclearStartReq.getDefaultInstance());

		if (!Objects.equals(player.getParent().getNuclearReadGuild(), player.getGuildId())) {
			return;
		}
		...
		if(worldPoint.getPointType() == WorldPointType.PLAYER){
			player.sendError(protocol.getType(), Status.Error.TBLY_DAODAN_BAN);
			return;
		}
		TBLYNuclearMissileSilo w = player.getParent().getTBLYBuildingByClass(TBLYNuclearMissileSilo.class).get(0);
		if (w.underGuildControl(player.getGuildId())) {
			w.onNuclearShoot(worldPoint, player);
		}
	 ...
	 * 现在是双倍积分
	 */
	@ProtocolHandler(code = HP.code.TBLY_CHRONO_SEND_REQ_VALUE)
	private void onChronoStart(HawkProtocol protocol) {
		
		PBTBLYChronoStartReq req = protocol.parseProtocol(PBTBLYChronoStartReq.getDefaultInstance());

		if (!Objects.equals(player.getParent().getChronoReadGuild(), player.getGuildId())) {
			return;
		}

		ITBLYWorldPoint worldPoint = player.getParent().getWorldPoint(req.getX(), req.getY()).orElse(null);
		if (worldPoint == null || !(worldPoint instanceof ITBLYBuilding)) {
			return;
		}
		ITBLYBuilding tarBuild = (ITBLYBuilding) worldPoint;
		if (tarBuild.getState() != TBLYBuildState.ZHAN_LING || tarBuild instanceof TBLYHeadQuarters) { // 已占不用了
			return;
		}
		if (!Objects.equals(tarBuild.getGuildId(), player.getGuildId())) {
			return;
		}

		TBLYChronoSphere w = player.getParent().getTBLYBuildingByClass(TBLYChronoSphere.class).get(0);
		if (w.underGuildControl(player.getGuildId())) {
			w.onChronoShoot(tarBuild, player);
		}
		player.responseSuccess(protocol.getType());
	}
	 ...
	 * 主播查看建筑战斗邮件
	 */
	@ProtocolHandler(code = HP.code.TBLY_ANCHOR_LIST_MAIL_C_VALUE)
	private boolean onListBuildMail(HawkProtocol protocol) {
		
		if (!player.isAnchor()) {
			return false;
		}
		PBTBLYAnchorListMailReq req = protocol.parseProtocol(PBTBLYAnchorListMailReq.getDefaultInstance());
		int posX = req.getX();
		int posY = req.getY();
		int type = req.getType();
		ITBLYBuilding build = (ITBLYBuilding) player.getParent().getWorldPoint(posX, posY).orElse(null);
	...
	private boolean onShareCoordinate(HawkProtocol protocol) {
		...
		ChatParames parames = ChatParames.newBuilder().setChatType(Const.ChatType.CHAT_FUBEN_TEAM).setKey(Const.NoticeCfgId.TBLY_SHARECOORDINATE).setPlayer(player)
	...
	@ProtocolHandler(code = HP.code.TBLY_SECOND_MAP_C_VALUE)
	...
	private boolean onGetGuildAssistenceInfo(HawkProtocol protocol) {
		...
		ITBLYPlayer tarPlayer = player.getParent().getPlayer(req.getPlayerId());
		if (tarPlayer == null) {
			return false;
		}
		...
		List<WorldMarch> helpMarchList = player.getParent().getPointMarches(tarPlayer.getPointId(),
				...
				WorldMarchType.ASSISTANCE).stream().map(ITBLYWorldMarch::getMarchEntity).collect(Collectors.toList());
	...
	private void getPresidentTowerQuarterInfo(HawkProtocol protocol) {
		...
		ITBLYBuilding weapon = (ITBLYBuilding) player.getParent().getWorldPoint(req.getPosX(), req.getPosY()).orElse(null);
		if (weapon == null) {
			return;
		}
	...
	@ProtocolHandler(code = HP.code.TBLY_GAME_SYNC_C_VALUE)
	...
	private boolean onPlayerMoveCity(HawkProtocol protocol) {
		...
		// 迁移前城点
		// ITBLYWorldPoint beforePoint = player;
		...
		if (WorldUtil.isRandomMoveCity(moveCityType)) {
			// 随机迁城
			int[] xy = player.getParent().randomFreePoint(player.getParent().randomPoint(), player.getPointType());
			// 迁城失败
			if (xy == null) {
		...
		if (player.getCostCityMoveCount() > 0) {
			// 投递回玩家线程：消耗道具
			player.dealMsg(MsgId.MOVE_CITY, new TBLYMoveCityMsgInvoker(player, consumeItems, moveCityType));
		}
		...
		if (player.getParent().getCurTimeMil() > player.getNextCityMoveTime()) {
			...
			for (TBLYChronoSphere w : player.getParent().getTBLYBuildingByClass(TBLYChronoSphere.class)) {
				if (w.underGuildControl(player.getGuildId())) {
					moveCityCd = (int) (moveCityCd * 0.01 * TBLYChronoSphere.getCfg().getCoolDownReducePercentage());
				}
			}
			// 记录迁城时间
	...
	private boolean moveCityCheck(ITBLYPlayer player, int hp, WorldMoveCityReq req, ConsumeItems consumeItems) {
		if (player.isAnchor()) {
			return false;
		}
	 ...
	 * @return
	 */
	private boolean checkMoveCityConsume(ITBLYPlayer player, int hp, int type, ConsumeItems consumeItems, boolean forceMove) {
		if (player.getParent().getCurTimeMil() > player.getNextCityMoveTime()) {
	 ...
	 * @return
	 */
	private boolean randomMoveCityCheck(ITBLYPlayer player, boolean forceMove) {
		// 自己有出征队伍
		...
		// 是否被攻击
		List<ITBLYWorldMarch> marchList = player.getParent().getPointMarches(player.getPointId(), WorldMarchStatus.MARCH_STATUS_MARCH, WorldMarchType.ATTACK_PLAYER);
	 ...
	 * @return
	 */
	private boolean selectMoveCityCheck(ITBLYPlayer player, int hp, WorldMoveCityReq req) {
		if (player.getParent().checkPlayerCanOccupy(player, req.getX(), req.getY())) {
			return true;
		}
		return false;
	}
	 ...
	 * @return
	 */
	private boolean guildMoveCityCheck(ITBLYPlayer player) {
		return false;
	}
	 ...
	 * @param player
	 */
	public void onPlayerMove(ITBLYPlayer player, int x, int y) {
		// 当前的
	...
	// /** 手动进入房间 */
	// @ProtocolHandler(code = HP.code.TBLY_JOIN_ROOM_REQ_VALUE)
	// private void onPlayerrJoinRoom(HawkProtocol protocol) {
	// 
	// TBLYJoinRoomMsg msg = TBLYJoinRoomMsg.valueOf(player.player.getParent(),
	// player);
	...
	/** 主动退出 */
	@ProtocolHandler(code = HP.code.TBLY_QUIT_ROOM_REQ_VALUE)
	private void onPlayerQuitRoom(HawkProtocol protocol) {
		
		player.getParent().quitWorld(player, QuitReason.LEAVE);
	}
	...
	private boolean onGetOtherPlayerEffect(HawkProtocol protocol) {
		...
		Player otherPlayer = TBLYRoomManager.getInstance().makesurePlayer(playerId);
		if (otherPlayer == null) {
	...
	private boolean onPlayerDetailOtherRequest(HawkProtocol protocol) {
		...
		String playerId = req.getPlayerId();
		// 快照数据
		Player snapshot = TBLYRoomManager.getInstance().makesurePlayer(playerId);
		if (snapshot == null) {
			player.sendError(protocol.getType(), Status.SysError.ACCOUNT_NOT_EXIST);
			return false;
		}
	...
	private void onReqWorldPointDetail(HawkProtocol protocol) {
		...
		ITBLYWorldPoint worldPoint = player.getParent().getWorldPoint(req.getPointX(), req.getPointY()).orElse(null);
		if (worldPoint == null) {
			// 删除点
	...
	@ProtocolHandler(code = HP.code.TBLY_ANCHOR_GET_GUILD_WAR_INFO_C_VALUE)
	protected boolean onAnchorGetGuildWarInfo(HawkProtocol protocol) {

		PBTBLYAnchorGetGuildWarInfoReq req = protocol.parseProtocol(PBTBLYAnchorGetGuildWarInfoReq.getDefaultInstance());
		
		if (!player.isAnchor()) {
			return false;
		}
		String viwerGuildId = player.getParent().getCampAGuild();
		if (req.getCamp() == 2) {
			viwerGuildId = player.getParent().getCampBGuild();
		}
		player.setGuildId(viwerGuildId);
		HPGetGuildWarInfoResp.Builder resp = buildGuildWarInfo(player);
		player.sendProtocol(HawkProtocol.valueOf(HP.code.TBLY_ANCHOR_GET_GUILD_WAR_INFO_S_VALUE, resp));
		return true;
	}

	@ProtocolHandler(code = HP.code.TBLY_ANCHOR_GET_GUILD_WAR_CELL_INFO_C_VALUE)
	protected boolean onAnchorGetGuildWarCellInfo(HawkProtocol protocol) {
		
		if (!player.isAnchor()) {
			return false;
		}
		PBTBLYAnchorGetGuildWarCellInfoReq req = protocol.parseProtocol(PBTBLYAnchorGetGuildWarCellInfoReq.getDefaultInstance());
		final String marchId = req.getMarchId();
		String viwerGuildId = player.getParent().getCampAGuild();
		if (req.getCamp() == 2) {
			viwerGuildId = player.getParent().getCampBGuild();
		...
		// 回复协议
		HPGetGuildWarInfoResp.Builder resp = buildGuildWarCellInfo(marchId, player);
		player.sendProtocol(HawkProtocol.valueOf(HP.code.TBLY_ANCHOR_GET_GUILD_WAR_CELL_INFO_S_VALUE, resp));
		return true;
	}

	private HPGetGuildWarInfoResp.Builder buildGuildWarInfo(ITBLYPlayer player) {
		// 回复协议
		...
		for (ITBLYWorldMarch guildMarch : guildMarchs) {
			try {
	...
	private HPGetGuildWarInfoResp.Builder buildGuildWarCellInfo(final String marchId, ITBLYPlayer player) {
		HPGetGuildWarInfoResp.Builder resp = HPGetGuildWarInfoResp.newBuilder();
		ITBLYWorldMarch guildMarch = player.getParent().getMarch(marchId);
		if (guildMarch != null) {
	...
	private boolean onGetGuildAssistantInfo(HawkProtocol protocol) {
		...
		List<ITBLYWorldMarch> marchList = player.assisReachMarches();
		...
		if (marchList != null) {
			...
			for (ITBLYWorldMarch march : marchList) {
				...
				ITBLYPlayer assistPlayer = march.getParent();
