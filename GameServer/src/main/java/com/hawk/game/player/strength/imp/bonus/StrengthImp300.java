 ...
 *
 */
@StrengthType(strengthType = 300)
public class StrengthImp300 implements StrengthBonusImp {
	...
	public void calc(Player player, SoldierType soldierType, PlayerStrengthCell cell) {
		...
		int atkValue = Math.min(typeCfg.getAtkAttrMax(), calc1.first);
		int hpValue = Math.min(typeCfg.getHpAttrMax(), calc1.second);
	...
	private HawkTuple2<Integer, Integer> calcOnce(PlayerData playerData, String param, SoldierType soldierType) {
		...
		for (int i = 0; i < heroCount; i++) {
			...
			// 工业值
			PBHeroAttr pbHeroAttr = hero.attrs().get(102);
			if (pbHeroAttr == null) {
				continue;
			}
