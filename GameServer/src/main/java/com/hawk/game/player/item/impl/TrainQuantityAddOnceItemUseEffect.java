...
public class TrainQuantityAddOnceItemUseEffect extends AbstractItemUseEffect {

	@Override
	public int itemType() {
		return Const.ToolType.TRAIN_QUANTITY_ADD_ONCE_VALUE;
	}
	...
	public boolean useItemCheck(Player player, ItemCfg itemCfg, int itemId, int itemCount, int protoType, String targetId) {
		...
		if (entity != null && entity.getValue() > 0) {
			...
			if (onceAddNum != itemCfg.getNum()) {
				player.sendError(protoType, Status.Error.TRAIN_QUANTITY_ADD_FAILD, 0);
