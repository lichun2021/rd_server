 ...
 *
 */
@StrengthType(strengthType = 40)
public class StrengthImp40 implements StrengthBonusImp {
	...
	public void calc(Player player, SoldierType soldierType, PlayerStrengthCell cell) {
		...
		for (int i = 0; i < heroCount; i++) {
			...
			HeroCfg heroCfg = hero.getConfig();
			atkValue += Math.min(talentExp * 10000 / heroCfg.getMaxTalent(), 10000) * heroCfg.getMarchTalentAtkAttr(soldierType.getNumber()) * GsConst.EFF_PER;
			hpValue += Math.min(talentExp * 10000 / heroCfg.getMaxTalent(), 10000) * heroCfg.getMarchTalentHpAttr(soldierType.getNumber()) * GsConst.EFF_PER;
		}
