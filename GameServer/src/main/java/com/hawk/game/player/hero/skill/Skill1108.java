...
@HeroSkill(skillID = { 110801, 110802, 110803, 110804, 110805 })
public class Skill1108 extends ISSSHeroSkill {
	/**
	【12521】
- 【万分比】【12521】战技持续期间，自身出征数量最多的主战坦克受到攻击时，伤害减少 +XX.XX%【12521】
  ...
  - 此为英雄战技专属作用号，配置格式如下
	...
	private int effect12485;// 填效果1作用号【12071】
	private double effect12485p1;
	...
	public List<PBHeroEffect> effectVal() {
		...
		try {
			...
			effect12485 = NumberUtils.toInt(ps[0]);
			effect12485p1 = NumberUtils.toDouble(ps[1]);
	...
	public int getProficiencyEffect(EffType effType, EffectParams effParams) {
		...
		if (effType.getNumber() == effect12485) {
			return (int) Math.ceil(effect12485p1 * getParent().getParent().attrVale(101));
		}
	...
	public int effectTime() {
		return effectTime * 1000 + getSoulEffVal(EffType.HERO_12534) * 1000;
	}
