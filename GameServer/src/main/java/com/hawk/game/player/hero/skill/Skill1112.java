...
@HeroSkill(skillID = { 111201, 111202, 111203, 111204, 111205 })
public class Skill1112 extends ISSSHeroSkill {
	/**
	【12621】
- 【万分比】【12621】战技持续期间，集结战斗中突击步兵受到攻击时，伤害减少 +XX.XX%
  ...
  - 此为英雄战技专属作用号，配置格式如下
	...
	private int effect12621;// 填效果1作用号【12071】
	private double effect12621p1;
	...
	public List<PBHeroEffect> effectVal() {
		...
		try {
			...
			effect12621 = NumberUtils.toInt(ps[0]);
			effect12621p1 = NumberUtils.toDouble(ps[1]);
	...
	public int getProficiencyEffect(EffType effType, EffectParams effParams) {
		...
		if (effType.getNumber() == effect12621) {
			return (int) Math.ceil(effect12621p1 * getParent().getParent().attrVale(101));
		}
	...
	public int effectTime() {
		return effectTime * 1000 + getSoulEffVal(EffType.HERO_12634) * 1000;
	}
