...
/** 集结行军 */
public abstract class ICYBORGMassMarch extends ICYBORGWorldMarch {

	public ICYBORGMassMarch(ICYBORGPlayer parent) {
		super(parent);
	}
	...
	public void waitingStatusMarchProcess() {
		...
		Set<ICYBORGWorldMarch> reachJoinMarchs = getMassJoinMarchs(true);
	...
	@Override
	public Set<ICYBORGWorldMarch> getMassJoinMarchs(boolean needReach) {
		// 等待和到达的行军
		Set<ICYBORGWorldMarch> retMarchs = new HashSet<>();
		// 不是队长行军
		if (isMassMarch()) {
			// 已到达队长家的行军
			List<ICYBORGWorldMarch> passiveMarchs = getParent().getParent().getWorldMarchList();
			for (ICYBORGWorldMarch march : passiveMarchs) {
				// 类型不匹配
	...
	public void teamMarchReached(ICYBORGMassJoinMarch JBSMassJoinMarch) {
		if(this instanceof ICYBORGReportPushMarch){
			((ICYBORGReportPushMarch)this).pushAttackReport();
		}
	}

	public void teamMarchCallBack(ICYBORGMassJoinMarch JBSMassJoinMarch) {
		if(this instanceof ICYBORGReportPushMarch){
			((ICYBORGReportPushMarch)this).pushAttackReport();
		}
	}
