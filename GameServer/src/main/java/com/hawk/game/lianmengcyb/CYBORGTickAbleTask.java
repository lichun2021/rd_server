...
public class CYBORGTickAbleTask extends HawkTask {
	private String battleId;
	private int threadNum;
	private Deque<ICYBORGWorldPoint> viewPoints = new LinkedList<>();

	private Deque<ICYBORGWorldMarch> marchs = new LinkedList<>();
	...
	public Object run() {
		...
		try {
			while (!viewPoints.isEmpty()) {
				ICYBORGWorldPoint point = viewPoints.pop();
			...
			while (!marchs.isEmpty()) {
				ICYBORGWorldMarch march = marchs.pop();
				marchTick(march);
			}
	...
	private void marchTick(ICYBORGWorldMarch march) {
		try {
	...
	private void buildTick(ICYBORGWorldPoint point) {
		try {
			...
			CYBORGBuildState stateOld = build.getState();
	...
	private void pointTick(ICYBORGWorldPoint point) {
		try {
			if (point.getParent().isGameOver()) {
				return;
			}
	...
	public boolean addPoint(ICYBORGWorldPoint task) {
		if (readyToRace.get()) {
			return false;
		}
	...
	public boolean addMarch(ICYBORGWorldMarch task) {
		if (readyToRace.get()) {
			return false;
		}
