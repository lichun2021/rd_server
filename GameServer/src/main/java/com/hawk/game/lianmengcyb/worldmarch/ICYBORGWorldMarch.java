...
public abstract class ICYBORGWorldMarch implements IWorldMarch {
	private int hashThread;
	private final ICYBORGPlayer parent;
	private CYBORGMarchEntity marchEntity;

	public ICYBORGWorldMarch(ICYBORGPlayer parent) {
		this.parent = parent;
	}

	public final ICYBORGPlayer getParent() {
		return parent;
	}

	@Override
	public CYBORGMarchEntity getMarchEntity() {
		return marchEntity;
	}

	public void setMarchEntity(CYBORGMarchEntity marchEntity) {
	...
	public GuildWarShowPB getGuildWarShoPb(String viwerGuildId) {
		...
		if (Objects.equals(viwerGuildId, getParent().getGuildId())) {
	...
	public WorldMarchPB.Builder toBuilder(WorldMarchRelation relation) {
		...
		CYBORGMarchEntity entity = getMarchEntity();
...
//				builder.addHeroLvl(hero.getLevel());
	...
	public GuildWarSingleInfo.Builder getGuildWarSingleInfo() {
		...
		ICYBORGPlayer player = getParent();
	...
	public boolean speedUp(int timeReducePercent, int speedUpTimes) {
		...
		if (this.isMassMarch()) {
			Set<ICYBORGWorldMarch> joinMarchs = getMassJoinMarchs(false);
			for (ICYBORGWorldMarch joinMarch : joinMarchs) {
		...
		if (this instanceof ICYBORGReportPushMarch) {
			((ICYBORGReportPushMarch) this).reachTimeChange();
		}
		return true;
	}
	
	@Override
	public Set<ICYBORGWorldMarch> getQuarterMarch() {
		if (getMarchStatus() != WorldMarchStatus.MARCH_STATUS_MARCH_QUARTERED_VALUE) {
			return Collections.emptySet();
		}
		List<ICYBORGWorldMarch> defMarchList = getParent().getParent().getPointMarches(getTerminalId(), WorldMarchStatus.MARCH_STATUS_MARCH_QUARTERED);
		return new HashSet<>(defMarchList);
	}

	@Override
	public Set<ICYBORGWorldMarch> getMassJoinMarchs(boolean b) {
		return Collections.emptySet();
	}
	...
	public void notifyMarchEvent(MarchEvent eventType) {
		...
		for (ICYBORGPlayer player : getParent().getParent().getPlayerList(CYBORGState.GAMEING)) {
		...
		// 发送主播
		if (getParent().getParent().hasAnchor()) {
			MarchEventSync.Builder builder = createSyncPB(eventType, getRelation(getParent().getParent().getAnchor()));
			getParent().getParent().getAnchor().sendProtocol(HawkProtocol.valueOf(HP.code.WORLD_MARCH_EVENT_SYNC_VALUE, builder));
		}

	}

	public void notifyMarchEvent(MarchEvent eventType, ICYBORGPlayer player) {
		// 自己的行军不走这种同步模式
		WorldMarchRelation relation = getRelation(player);
	 ...
	 * @return
	 */
	public WorldMarchRelation getRelation(ICYBORGPlayer tarplayer) {
		...
		// 目标点
		Optional<ICYBORGWorldPoint> targetPointOp = getParent().getParent().getWorldPoint(marchEntity.getTerminalId());
		...
		ICYBORGWorldPoint targetPoint = targetPointOp.get();
	...
	public void sendBattleResultInfo(boolean isWin, List<ArmyInfo> atkArmyList, List<ArmyInfo> defArmyList, boolean isMonsterDead) {
		...
		List<ICYBORGPlayer> players = getParent().getParent().getPlayerList(CYBORGState.GAMEING);
		for (ICYBORGPlayer pla : players) {
			pla.sendProtocol(HawkProtocol.valueOf(HP.code.BATTLE_INFO_S_VALUE, builder));
		}
		if (getParent().getParent().hasAnchor()) {
			getParent().getParent().getAnchor().sendProtocol(HawkProtocol.valueOf(HP.code.BATTLE_INFO_S_VALUE, builder));
		}
	...
	public void remove() {
		if (this instanceof ICYBORGReportPushMarch) {
			((ICYBORGReportPushMarch) this).removeAttackReport();
		}
	...
	public void updateDefMarchAfterWar(List<ICYBORGWorldMarch> helpMarchList, Map<String, List<ArmyInfo>> defArmyMap) {
		// 更新援助防御玩家行军的部队
		for (ICYBORGWorldMarch tmpMarch : helpMarchList) {
			...
			if (tmpMarch.getMarchType() == WorldMarchType.ASSISTANCE) {
				...
				ICYBORGPlayer assistPlayer = tmpMarch.getParent();
	...
	private double effectToMarchTime(double time, boolean slowDown, double slowDownTime) {
		...
		if (this.isMassJoinMarch()) {
			Optional<ICYBORGWorldPoint> targetPoint = getParent().getParent().getWorldPoint(marchEntity.getTerminalId());
			if (targetPoint.isPresent()) {
				ICYBORGPlayer leader = (ICYBORGPlayer) targetPoint.get();
	...
	public double getMarchBaseSpeed() {
		...
		if (isMassMarch()) {
			Set<ICYBORGWorldMarch> joinMarchs = getMassJoinMarchs(true);
			for (ICYBORGWorldMarch tempMarch : joinMarchs) {
	...
	public GuildWarTeamInfo.Builder getGuildWarInitiativeInfo() {
		...
		// 加入集结的行军
		Set<ICYBORGWorldMarch> joinMarchs = this.getMassJoinMarchs(false);
		for (ICYBORGWorldMarch joinMarch : joinMarchs) {
	...
	public GuildWarTeamInfo.Builder getGuildWarPassivityInfo() {
		...
		List<ICYBORGWorldMarch> assistandMarchs = getParent().getParent().getPointMarches(leader.getPointId(), WorldMarchStatus.MARCH_STATUS_MARCH_ASSIST,
