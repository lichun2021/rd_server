...
public class CYBORGArmyModule extends CYBORGBattleRoomModule {

	public CYBORGArmyModule(CYBORGBattleRoom appObj) {
		super(appObj);
	}
	
	/** 使用号令 */
	@ProtocolHandler(code = HP.code2.CYBORG_ORDER_USE_C_VALUE)
	private boolean onUseOrder(CYBORGProtocol protocol) {
		PBCYBORGOrderUseReq req = protocol.parseProtocol(PBCYBORGOrderUseReq.getDefaultInstance());
		...
		//消耗
		CYBORGOrderCollection orderCollection = battleRoom.getCampBase(player.getCamp()).orderCollection;
		if(orderCollection == null){
			return true;
		}
		int orderId = req.getOrderId();
		int rlt = orderCollection.canStartOrder(req);
		if(rlt > 0){
			player.sendError(protocol.getType(), rlt ,0);
			return true;
		}
		CYBORGOrder order = orderCollection.getOrder(orderId);
	...
	private boolean onCreateSoldier(CYBORGProtocol protocol) {
		ICYBORGPlayer player = protocol.getPlayer();
	...
	private boolean onQueueSpeedUp(CYBORGProtocol protocol) {
		ICYBORGPlayer player = protocol.getPlayer();
	...
	private boolean onCureSoldier(CYBORGProtocol protocol) {
		ICYBORGPlayer player = protocol.getPlayer();
		if (battleRoom.maShangOver()) {
			player.responseSuccess(protocol.getType());
			return true;
	...
	private QueueEntity addQueue(ICYBORGPlayer player, int queueType, int queueStatus, String itemId,
	...
	private List<ItemInfo> cureConsume(ICYBORGPlayer player, List<ArmySoldierPB> cureList, int recoverTime, boolean immediate, boolean useGold, int hpCode) {
	 ...
	 * @return
	 */
	private Map<Integer, Integer> cureSoldier(ICYBORGPlayer player, List<ArmySoldierPB> cureList, boolean immediate) {
	...
	private boolean onCurePlantSoldier(CYBORGProtocol protocol) {
		ICYBORGPlayer player = protocol.getPlayer();
		if (battleRoom.maShangOver()) {
			player.responseSuccess(protocol.getType());
			return true;
		...
		if (!immediate) {
			addQueue(player, QueueType.CURE_PLANT_QUEUE_VALUE, QueueStatus.QUEUE_STATUS_COMMON_VALUE,
					req.getBuildingUUID(), Const.BuildingType.PLANT_HOSPITAL_VALUE, HawkTime.getMillisecond(), recoverTime * 1000, resCost, GsConst.QueueReusage.PLANT_SOLDIER_CURE);
		}
		player.getPush().syncArmyInfo(ArmyChangeCause.SOLDIER_CURE, armyIds.keySet().toArray(new Integer[armyIds.size()]));
