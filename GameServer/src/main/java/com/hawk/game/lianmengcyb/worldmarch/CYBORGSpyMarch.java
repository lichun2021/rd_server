...
import com.hawk.game.protocol.Mail.PBDetectCYBORGBuilding;
...
public class CYBORGSpyMarch extends ICYBORGWorldMarch implements ICYBORGReportPushMarch {

	public CYBORGSpyMarch(ICYBORGPlayer parent) {
		super(parent);
	}
	...
	private void doBuildSpy(ICYBORGWorldPoint point) {
		// CYBORG_SPY_BUILD_SUCCESS = 2020009;//泰伯利亚建筑侦察成功 2020009
		// CYBORG_SPY_BUILD_FAIL = 2020010;//泰伯利亚建筑侦察失败 2020010
		// CYBORG_SPY_BUILD_BE = 2020011; //泰伯利亚建筑被侦察 2020011
		ICYBORGBuilding build = (ICYBORGBuilding) point;
		List<ICYBORGWorldMarch> quarteredMarchs = getParent().getParent().getPointMarches(getMarchEntity().getTerminalId(), WorldMarchStatus.MARCH_STATUS_MARCH_QUARTERED);
		...
		if (quarteredMarchs.get(0).getParent().getCamp() == getParent().getCamp()) {
			...
			// .setMailId(MailId.CYBORG_SPY_BUILD_FAIL)
			// .build());
			return;
		}
		...
		PBDetectCYBORGBuilding.Builder builder = PBDetectCYBORGBuilding.newBuilder().setLevel(radarLevel).setPointX(point.getX()).setPointY(point.getY());
		...
		// 侦查邮件
		MailParames.Builder playerParamesBuilder = MailParames.newBuilder().setPlayerId(getParent().getId()).setMailId(MailId.CYBORG_SPY_BUILD_SUCCESS)
				.addSubTitles(point.getX(), point.getY())
				.setDuntype(DungeonMailType.CYBORG)
				.addContents(DetectMail.newBuilder().setCyborgBuild(builder)).addTips(point.getX()).addTips(point.getY());
		...
		for (IWorldMarch tempMarch : quarteredMarchs) {
			FightMailService.getInstance()
					.sendMail(MailParames.newBuilder().setPlayerId(tempMarch.getPlayerId()).setMailId(MailId.CYBORG_SPY_BUILD_BE).addSubTitles(point.getX(), point.getY())
	...
	private void doPlayerSpy(ICYBORGWorldPoint point) {
	 ...
	 * @return
	 */
	public boolean sendDetectMail(Player player, Player targetPlayer, ICYBORGWorldPoint point) {
		...
		MailParames.Builder playerParamesBuilder = MailParames.newBuilder().setPlayerId(player.getId()).addSubTitles(subTitle).addContents(builder)
				...
				.addTips(targetPlayer.getName()).setDuntype(DungeonMailType.CYBORG)
		...
		FightMailService.getInstance().sendMail(playerParamesBuilder.setDuntype(DungeonMailType.CYBORG).build());
	
		FightMailService.getInstance().sendMail(targetParamesBuilder.setDuntype(DungeonMailType.CYBORG).build());
		return true;
	}

	public DetectMail.Builder createDetectMail(final int radarLevel, Player player, Player targetPlayer, ICYBORGWorldPoint point, boolean isMirror) {
		...
		if (pointType == WorldPointType.RESOURCE_VALUE || pointType == WorldPointType.QUARTERED_VALUE) {
			// 资源点 驻扎点
			List<ICYBORGWorldMarch> marchs = getParent().getParent().getPointMarches(point.getPointId(), WorldMarchStatus.MARCH_STATUS_MARCH_QUARTERED);
			if (!marchs.isEmpty()) {
				leaderMarch = marchs.get(0);
			}
		} else if (pointType == WorldPointType.PLAYER_VALUE) {
			// 玩家城堡---基地
			List<ICYBORGWorldMarch> massMarches = getParent().getParent().getPlayerMarches(targetPlayer.getId(), WorldMarchStatus.MARCH_STATUS_WAITING);
			if (!massMarches.isEmpty()) {
				leaderMarch = massMarches.get(0);
			}
			List<ICYBORGWorldMarch> marchs = getParent().getParent().getPointMarches(point.getPointId(), WorldMarchStatus.MARCH_STATUS_MARCH_ASSIST);
		...
		if (pointType == WorldPointType.PLAYER_VALUE && !targetPlayer.getData().getArmyEntities().isEmpty()) {
			...
			if (radarLevel >= GsConst.RadarLevel.LV21 && leaderMarch != null) {
				List<ICYBORGWorldMarch> massMarches = getParent().getParent().getPointMarches(point.getPointId(), WorldMarchStatus.MARCH_STATUS_MARCH,
		...
		if (pointType == WorldPointType.PLAYER_VALUE) {
			...
			// 藏兵洞
			List<ICYBORGWorldMarch> hiddenMarchs = getParent().getParent().getPointMarches(point.getPointId(), WorldMarchType.HIDDEN_MARCH);
			if (radarLevel >= GsConst.RadarLevel.LV10) {
	...
	public Set<String> attackReportRecipients() {
		// 防守方援军
		List<ICYBORGWorldMarch> helpMarchList = getParent().getParent().getPointMarches(getMarchEntity().getTerminalId(), WorldMarchStatus.MARCH_STATUS_MARCH_ASSIST,
