...
public class CYBORGAttackPlayerMarch extends ICYBORGWorldMarch implements ICYBORGReportPushMarch {

	public CYBORGAttackPlayerMarch(ICYBORGPlayer parent) {
		super(parent);
	}
	...
	public void onMarchReach(Player parent) {
		...
		atkMarchs.add(this);
		// 防守方援军
		List<ICYBORGWorldMarch> helpMarchList = helpMarches();
		// 防守方行军
		...
		for (Player defer : defPlayers) {
			((ICYBORGPlayer) defer).setLastAttacker(getParent());
		}
	}
	...
	private List<ICYBORGWorldMarch> helpMarches() {
