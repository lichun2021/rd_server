...
import com.hawk.game.lianmengcyb.player.action.CYBORGTestMarchAction;
...
public class CYBORGPlayer extends ICYBORGPlayer {
	...
	private CYBORGTestMarchAction action;

	public CYBORGPlayer(Player player) {
		super(player.getXid());
	...
	public void init() {
		...
		this.setPlayerData(CYBORGPlayerData.valueOf(this));
		CYBORGPlayerPush playerPush = new CYBORGPlayerPush(this);
		this.setPlayerPush(playerPush);
		getParent();
		this.nextLoghonor = getParent().getCurTimeMil() + CYBORGConst.MINUTE_MICROS;
		...
		if (isRobot()) {
			action = new CYBORGTestMarchAction();
	...
	public boolean onTick() {
		...
		if (now - lastTickTime > 1000) {
			...
			setCYBORGState(CYBORGState.GAMEING);
			...
			if (getOnFireEndTime() > now) {
				...
				// .setChatType(ChatType.CHAT_CYBORG_SYS)
				// .setKey(NoticeCfgId.CYBORG_CITY_DEF_30)
				// .addParms(getName())
				// .build();
...
//				action.onTick();
			...
			if (getLastbuildBuffLevelExtra() != getParent().getCampBase(getCamp()).buildBuffLevelExtra) {
				int blecfgId = Math.max(getLastbuildBuffLevelExtra(), getParent().getCampBase(getCamp()).buildBuffLevelExtra);
				CYBORGBuildBuffLevelExtraCfg blecfg = HawkConfigManager.getInstance().getConfigByKey(CYBORGBuildBuffLevelExtraCfg.class, blecfgId);
				if (Objects.nonNull(blecfg) && !blecfg.getControleBuffMap().isEmpty()) {
					EffType[] arr = blecfg.getControleBuffMap().keySet().toArray(new EffType[0]);
					getPlayerPush().syncPlayerEffect(arr);
				}
				setLastbuildBuffLevelExtra(getParent().getCampBase(getCamp()).buildBuffLevelExtra);
			}
		}

		if (now > nextLoghonor) {
			nextLoghonor = now + CYBORGConst.MINUTE_MICROS;
//			LogUtil.logCYBORGPlayerHonor(this, getHonor());
		}
	...
	public void checkSuperVipSkinToMassMarch() {
		try {
			...
			List<ICYBORGWorldMarch> marchs = getParent().getPlayerMarches(getId());
	...
	public void quitGame() {
		if (isAnchor()) {
			getParent().setAnchor(null);
			getParent().synAnchorPageInfo(this, false);
		}
		PlayerInfo.Builder newBuilder = PlayerInfo.newBuilder();
		newBuilder.setLmjyState(0);
		newBuilder.setIsCYBORGAnchor(false);
		sendProtocol(HawkProtocol.valueOf(HP.code.PLAYER_INFO_SYNC_S, HPPlayerInfoSync.newBuilder().setPlayerInfo(newBuilder)));

		source.setCYBORGRoomId("");
		source.setCYBORGState(null);
		source.getPush().syncPlayerInfo();

		CYBORGRoomManager.getInstance().invalidate(this);
	}

	@Override
	public String getCYBORGRoomId() {
		return source.getCYBORGRoomId();
	}

	@Override
	public void setCYBORGRoomId(String CYBORGRoomId) {
		source.setCYBORGRoomId(CYBORGRoomId);
	}

	@Override
	public CYBORGState getCYBORGState() {
		return source.getCYBORGState();
	}

	@Override
	public void setCYBORGState(CYBORGState CYBORGState) {
		source.setCYBORGState(CYBORGState);
	}
	...
	public boolean sendProtocol(HawkProtocol protocol) {
		if (Objects.equals(getCYBORGRoomId(), getParent().getXid().getUUID())) {
...
//		HawkTaskManager.getInstance().postProtocol(getParent().getXid(), CYBORGProtocol.valueOf(protocol, this));
		return true;
	}
	...
	public int getMaxMassJoinMarchNum() {
		return source.getMaxMassJoinMarchNum();
	}
