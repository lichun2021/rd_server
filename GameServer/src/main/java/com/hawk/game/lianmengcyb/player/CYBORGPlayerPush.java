...
public class CYBORGPlayerPush extends ICYBORGPlayerPush {
	...
	public CYBORGPlayerPush(Player player) {
		super(player);
	}
	...
	public void syncGuildWarCount() {
		ICYBORGPlayer player = getPlayer();
		HPGuildWarCountPush.Builder builder = HPGuildWarCountPush.newBuilder();
		int count = player.getParent().getCampBase(player.getCamp()).campGuildWarCount;
		if (player.isAnchor()) {
			count = 0;
		}
	...
	public void syncPlayerInfo() {
		...
		newBuilder.setCyborgCamp(getPlayer().getCamp().intValue());
		newBuilder.setIsCYBORGAnchor(getPlayer().isAnchor());
	...
	public void pushJoinGame() {
		// CYBORGBattleRoom battleRoom = getPlayer().getParent();
		CYBORGPlayerData playerData = (CYBORGPlayerData) getData();
		// 隐藏真实数据
		getPlayer().sendProtocol(HawkProtocol.valueOf(HP.code.CYBORG_CLEAN_CLAIENT_DATA_VALUE));
		...
		getPlayer().refreshPowerElectric(PowerChangeReason.CYBORG_INIT);
		// // 世办点开罩子
	...
	public void pushGameOver() {
		...
		source.sendProtocol(HawkProtocol.valueOf(HP.code.CYBORG_CLEAN_CLAIENT_DATA_VALUE));
		// 清除军演数据
		...
		source.refreshPowerElectric(PowerChangeReason.CYBORG_INIT);
		// // 世办点开罩子
		// WorldPoint worldPoint = WorldPlayerService.getInstance().getPlayerWorldPoint(player.getId());
		// long shiledEnd = battleRoom.getOverTime() + PORTECTED_A;
		// if (Objects.nonNull(worldPoint) && worldPoint.getShowProtectedEndTime() == shiledEnd) {
		...
		// }
		// }
		CYBORGQuitRoomMsg msg = CYBORGQuitRoomMsg.valueOf(getPlayer().getQuitReason());
	...
	public CYBORGPlayer getPlayer() {
		return (CYBORGPlayer) player;
	}

	@Override
	public ICYBORGPlayerData getData() {
		return (ICYBORGPlayerData) player.getData();
	}
