...
@EffectChecker(effType = EffType.XQHX_10065)
public class Checker10065 implements IChecker {
	@Override
	public CheckerKVResult value(CheckerParames parames) {
		int effPer = 0;
		int effNum = 0;
		if (parames.unity.getPlayer().getXQHXState() == XQHXState.GAMEING && BattleConst.WarEff.MASS.check(parames.troopEffType) && isSoldier(parames.type)) {
			effPer = (int) (parames.unity.getEffVal(effType())* GsConst.EFF_PER * ConstProperty.getInstance().effect10065SoldierAdjustMap.getOrDefault(parames.type, 10000));
		}
