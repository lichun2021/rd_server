 ...
 * 
- 【万分比】【12011】远程支援：集结战斗开始前，若自身出征携带不低于集结部队总数 5% 且 超过自身部队总数 50% 的攻城车，全体远程部队攻击 +XX.XX%；（多个杰拉尼同时存在时，上述效果可叠加，至多 2 层）
- 【万分比】【12012】远程支援：集结战斗开始前，若自身出征携带不低于集结部队总数 5% 且 超过自身部队总数 50% 的攻城车，全体远程部队防御 +XX.XX%；（多个杰拉尼同时存在时，上述效果可叠加，至多 2 层）
- 【万分比】【12013】远程支援：集结战斗开始前，若自身出征携带不低于集结部队总数 5% 且 超过自身部队总数 50% 的攻城车，全体远程部队生命 +XX.XX%；（多个杰拉尼同时存在时，上述效果可叠加，至多 2 层）
  ...
  - 集结战斗开始前，若自身出征携带不低于集结部队总数 5% 的攻城车
    ...
    - 数量2 = 集结部队的出征数量总和
      - 真实出征部队数量，在战斗开始前判定，即所罗门、吉迪恩 这种战斗中改变部队数量的机制对此无影响
    - 数量1/数量2 >= 指定数值时生效
      - 指定数值读取const表，字段effect12011AllNumLimit、effect12012AllNumLimit、effect12013AllNumLimit
        - 配置格式：万分比
  - 集结战斗开始前，若自身携带超过自身部队总数 50% 的攻城车
    ...
    - 数量2 = 某玩家出征数量
      - 真实出征部队数量，在战斗开始前判定，即所罗门、吉迪恩 这种战斗中改变部队数量的机制对此无影响
    - 数量1/数量2 > 指定数量时生效
      - 指定数值读取const表，字段effect12011SelfNumLimit、effect12012SelfNumLimit、effect12013SelfNumLimit
        - 配置格式：万分比
  - 全体远程部队攻击/防御/生命 +XX.XX%；（多个杰拉尼同时存在时，上述效果可叠加，至多 2 层）
    ...
    - 效果记录在己方部队身上，数值直接叠加；限制该作用号生效层数上限
      ...
      - 层数上限读取const表，字段effect12011Maxinum、effect12012Maxinum、effect12013Maxinum
        - 配置格式：绝对值
 */
@BattleTupleType(tuple = Type.DEF)
@EffectChecker(effType = EffType.HERO_12012)
public class Checker12012 implements IChecker {
	@Override
	public CheckerKVResult value(CheckerParames parames) {
		...
		int effPer = valMap.values().stream()
				...
				.limit(ConstProperty.getInstance().getEffect12012Maxinum())
	...
	private int effvalue(BattleUnity unity, CheckerParames parames) {
		try {
			...
			if (march8cnt / parames.unitStatic.getTotalCountMarch() >= ConstProperty.getInstance().getEffect12012AllNumLimit() * GsConst.EFF_PER) {
				// 若自身携带超过自身部队总数 50% 的采矿车
				if (march8cnt * 1D / parames.unitStatic.getPlayerArmyCountMapMarch().get(playerId) > ConstProperty.getInstance().getEffect12012SelfNumLimit() * GsConst.EFF_PER) {
