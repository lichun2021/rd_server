...
@EffectChecker(effType = EffType.XQHX_10068)
public class Checker10068 implements IChecker {
	@Override
	public CheckerKVResult value(CheckerParames parames) {
		...
		if (parames.unity.getPlayer().getXQHXState() == XQHXState.GAMEING && parames.unitStatic.isNotMass() && parames.tarStatic.isNotMass() && isSoldier(parames.type)) {
			effPer = (int) (parames.unity.getEffVal(effType())* GsConst.EFF_PER * ConstProperty.getInstance().effect10068SoldierAdjustMap.getOrDefault(parames.type, 10000));
		}
