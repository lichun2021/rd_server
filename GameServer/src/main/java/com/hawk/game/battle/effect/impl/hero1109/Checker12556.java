...
/**
 * 【12556】
- 【万分比】【12556】任命在战备参谋部时，集结战斗开始前，集结全队出征采矿车数量每满 5（effect12556NumLimit） 万，自身采矿车受到攻击时，伤害减少 +XX.XX%（12556）（该效果可叠加，至多 20(effect12556Maxinum)层）
  - 战报相关
    - 于战报中展示
    - 不合并至精简战报中
  ...
  - 集结全队出征采矿车数量每满 5 万
    ...
    - 真实出征部队数量，在战斗开始前判定，即参谋军威技能、所罗门、吉迪恩 这种战斗中改变部队数量的机制对此无影响
      - 指定数值读取const表，字段effect12556NumLimit
        - 配置格式：绝对值
  - 自身采矿车受到攻击时，伤害减少 +XX.XX%
    - 兵种包含有：采矿车（兵种类型 = 8）
    - 该伤害减少效果与其他伤害减少效果累乘计算；即实际伤害 = 基础伤害*（1 + 敌方各类伤害加成）*（1 - 己方某伤害减少）*（1 - 本作用值伤害减少）
  - （该效果可叠加，至多 20 层）
    - 效果记录在己方部队身上，数值直接叠加；限制该作用号生效层数上限
      - 层数上限读取const表，字段effect12556Maxinum
        - 配置格式：绝对值
 */
@BattleTupleType(tuple = { Type.REDUCE_HURT_PCT })
@EffectChecker(effType = EffType.EFF_12556)
public class Checker12556 implements IChecker {
	...
	private int selectPlayer(CheckerParames parames) {
		int tankCnt = (int) parames.unitStatic.getArmyCountMapMarch().get(SoldierType.CANNON_SOLDIER_8);

		int cen = Math.min(tankCnt / ConstProperty.getInstance().effect12556NumLimit, ConstProperty.getInstance().effect12556Maxinum);
