...
@EffectChecker(effType = EffType.HERO_12002)
public class Checker12002 implements IChecker {
	@Override
	public CheckerKVResult value(CheckerParames parames) {
		...
		int effPer = valMap.values().stream()
				...
				.limit(ConstProperty.getInstance().getEffect12002Maxinum())
	...
	private int effvalue(BattleUnity unity, CheckerParames parames) {
		try {
			...
			if (march8cnt / parames.unitStatic.getTotalCountMarch() >= ConstProperty.getInstance().getEffect12002AllNumLimit() * GsConst.EFF_PER) {
				// 若自身携带超过自身部队总数 50% 的采矿车
				if (march8cnt * 1D / parames.unitStatic.getPlayerArmyCountMapMarch().get(playerId) > ConstProperty.getInstance().getEffect12002SelfNumLimit() * GsConst.EFF_PER) {
