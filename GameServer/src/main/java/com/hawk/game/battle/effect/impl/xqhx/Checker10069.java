...
@EffectChecker(effType = EffType.XQHX_10069)
public class Checker10069 implements IChecker {
	@Override
	public CheckerKVResult value(CheckerParames parames) {
		...
		if (parames.unity.getPlayer().getXQHXState() == XQHXState.GAMEING && parames.unitStatic.isNotMass() && parames.tarStatic.isNotMass() && isSoldier(parames.type)) {
			effPer = (int) (parames.unity.getEffVal(effType())* GsConst.EFF_PER * ConstProperty.getInstance().effect10069SoldierAdjustMap.getOrDefault(parames.type, 10000));
		}
