 ...
 * 
 - 【万分比】【12004】近战支援：集结战斗开始前，若自身出征携带不低于集结部队总数 5% 且 超过自身部队总数 50% 的采矿车，额外为近战友军中出征数量最多的单位增加超能攻击 +XX.XX%*自身属性加成（自身属性加成为自身攻击加成、自身防御加成和自身生命加成的均值）；（多个杰拉尼同时存在时，上述效果可叠加，至多 2 层）
  - 战报相关
    - 于战报中展示
    - 合并至精简战报中
  ...
  - 集结战斗开始前，若自身出征携带不低于集结部队总数 5% 的采矿车
    ...
    - 数量2 = 集结部队的出征数量总和
      - 真实出征部队数量，在战斗开始前判定，即所罗门、吉迪恩 这种战斗中改变部队数量的机制对此无影响
    - 数量1/数量2 >= 指定数值时生效
      - 指定数值读取const表，字段effect12004AllNumLimit
        - 配置格式：万分比
  - 集结战斗开始前，若自身携带超过自身部队总数 50% 的采矿车
    ...
    - 数量2 = 某玩家出征数量
      - 真实出征部队数量，在战斗开始前判定，即所罗门、吉迪恩 这种战斗中改变部队数量的机制对此无影响
    - 数量1/数量2 > 指定数量时生效
      - 指定数值读取const表，字段effect12004SelfNumLimit
        - 配置格式：万分比
  - 额外为近战友军中出征数量最多的单位增加超能攻击 +XX.XX%*自身属性加成（自身属性加成为自身攻击加成、自身防御加成和自身生命加成的均值）
    - 近战友军
  ...
  - （多个杰拉尼同时存在时，上述效果可叠加，至多 2 层）
    ...
    - 效果记录在己方部队身上，数值直接叠加；限制该作用号生效层数上限
      ...
      - 层数上限读取const表，字段effect12004Maxinum
        - 配置格式：绝对值
 */
@BattleTupleType(tuple = Type.ATKFIRE)
@EffectChecker(effType = EffType.HERO_12004)
public class Checker12004 implements IChecker {
	@Override
	public CheckerKVResult value(CheckerParames parames) {
		...
		for (Entry<String, Integer> ent : effPlayerVal.entrySet()) {
			if (theMaxFriend(ent.getKey(), parames) == parames.unity) {
				...
				parames.addDebugLog("-Jelani {} 为 {} 附加 12004 val:{}", ent.getKey(), solider.getUUID(), ent.getValue());
			}
		}
	...
	private int effvalue(BattleUnity unity, CheckerParames parames) {
		try {
			...
			if (march8cnt / parames.unitStatic.getTotalCountMarch() >= ConstProperty.getInstance().getEffect12004AllNumLimit() * GsConst.EFF_PER) {
				// 若自身携带超过自身部队总数 50% 的采矿车
				if (march8cnt * 1D / parames.unitStatic.getPlayerArmyCountMapMarch().get(playerId) > ConstProperty.getInstance().getEffect12004SelfNumLimit() * GsConst.EFF_PER) {
					BattleSoldier maxSoldier8 = parames.getPlayerMaxFreeArmy(playerId, SoldierType.CANNON_SOLDIER_8).getSolider();
					...
					int eff12021 = unity.getEffVal(EffType.HERO_12021);
					effPer = (int) (avg * GsConst.EFF_PER * unity.getEffVal(effType())) + eff12021;

					parames.addDebugLog("-Jelani {} 12004 仅对友军部队（集结己方部队，不包含自身）中的近战部队生效  {} atk: {} def: {} hp: {} avg: {}, eff12021:{} 最终 12004: {}",
							unity.getPlayerName(), maxSoldier8.getUUID(), atkper, defper, hpper, avg, eff12021, effPer);
				}
			}
			return effPer;
		} catch (Exception e) {
