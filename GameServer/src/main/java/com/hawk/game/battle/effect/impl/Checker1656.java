...
@EffectChecker(effType = EffType.HERO_1656)
public class Checker1656 implements IChecker {
	/**
	 * •芯片描述      
	 ...
	 */
	final int kaiEnId = 1078;

	@Override
	public CheckerKVResult value(CheckerParames parames) {
		if (parames.type != SoldierType.TANK_SOLDIER_2 || !parames.heroId.contains(kaiEnId) || parames.unity.getEffVal(effType()) <= 0) {
			return CheckerKVResult.DefaultVal;
		}
		String playerId = parames.unity.getPlayer().getId();
		BattleUnity temp = parames.getPlayerMaxFreeArmy(playerId, SoldierType.TANK_SOLDIER_2);
		if (temp != parames.unity) {
			return CheckerKVResult.DefaultVal;
		}
		// 负面效果的概率 必带sss凯恩, 没有就骂驾驶证
		Optional<PlayerHero> kaienOp = parames.unity.getPlayer().getHeroByCfgId(kaiEnId);
		if (!kaienOp.isPresent()) {
			return CheckerKVResult.DefaultVal;
		}
		PlayerHero kaiEn = kaienOp.get();
		// # M10英雄专属芯片作用号【1656】负面效果基础概率 effect1656Per = 1000,1500,2200,3000,4000
		int effect1656per = NumberUtils.toInt(ConstProperty.getInstance().getEffect1656Per().split(",")[kaiEn.getStar() - 1]);
		((BattleSoldier_2) parames.solider).setEffect1656per(effect1656per);
