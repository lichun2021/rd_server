...
@EffectChecker(effType = EffType.XQHX_10063)
public class Checker10063 implements IChecker {
	@Override
	public CheckerKVResult value(CheckerParames parames) {
		...
		if (parames.unity.getPlayer().getXQHXState() == XQHXState.GAMEING && BattleConst.WarEff.MASS.check(parames.troopEffType) && isSoldier(parames.type)) {
			effPer = (int) (parames.unity.getEffVal(effType())* GsConst.EFF_PER * ConstProperty.getInstance().effect10063SoldierAdjustMap.getOrDefault(parames.type, 10000));
		}
