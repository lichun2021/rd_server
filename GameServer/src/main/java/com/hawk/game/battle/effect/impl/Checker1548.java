...
@EffectChecker(effType = EffType.HERO_1548)
public class Checker1548 implements IChecker {
	@Override
	public CheckerKVResult value(CheckerParames parames) {
		...
		if (isSoldier(parames.type) && effVal > 0 && BattleConst.WarEff.SELF_FIGHT.check(parames.troopEffType)) {
			int effect1547Maxinum = ConstProperty.getInstance().getEffect1548Maxinum();
