...
/**
 * 12262】
- 【万分比】【12262】远程支援：集结战斗开始前，若自身出征携带不低于集结部队总数 5% 且 超过自身部队总数 50% 的攻城车，全体远程部队伤害额外 +XX.XX%；（多个杰拉尼同时存在时，至多有 2 个攻城车单位生效）
- 战报相关
  - 于战报中展示
  - 合并至精简战报中
...
- 集结战斗开始前，若自身出征携带不低于集结部队总数 5% 的攻城车
  ...
  - 数量2 = 集结部队的出征数量总和
    - 真实出征部队数量，在战斗开始前判定，即参谋军威技能、所罗门、吉迪恩 这种战斗中改变部队数量的机制对此无影响
  - 数量1/数量2 >= 指定数值时生效
    - 指定数值读取const表，字段effect12262AllNumLimit
      - 配置格式：万分比
- 集结战斗开始前，若自身携带超过自身部队总数 50% 的攻城车
  ...
  - 数量2 = 某玩家出征数量
    - 真实出征部队数量，在战斗开始前判定，即参谋军威技能、所罗门、吉迪恩 这种战斗中改变部队数量的机制对此无影响
  - 数量1/数量2 > 指定数量时生效
    - 指定数值读取const表，字段effect12262SelfNumLimit
      - 配置格式：万分比
- 全体远程部队伤害额外 +XX.XX%；（多个杰拉尼同时存在时，至多有 2 个攻城车单位生效）
  ...
  - 效果记录在己方部队身上，数值直接叠加；限制该作用号生效层数上限
    ...
    - 层数上限读取const表，字段effect12262Maxinum
      - 配置格式：绝对值
 */
@BattleTupleType(tuple = Type.HURT)
@EffectChecker(effType = EffType.EFF_12262)
public class Checker12262 implements IChecker {
	@Override
	public CheckerKVResult value(CheckerParames parames) {
		...
		int effPer = valMap.values().stream()
				...
				.limit(ConstProperty.getInstance().getEffect12262Maxinum())
	...
	private int effvalue(BattleUnity unity, CheckerParames parames) {
		try {
			...
			if (march8cnt / parames.unitStatic.getTotalCountMarch() >= ConstProperty.getInstance().getEffect12262AllNumLimit() * GsConst.EFF_PER) {
				// 若自身携带超过自身部队总数 50% 的采矿车
				if (march8cnt * 1D / parames.unitStatic.getPlayerArmyCountMapMarch().get(playerId) > ConstProperty.getInstance().getEffect12262SelfNumLimit() * GsConst.EFF_PER) {
