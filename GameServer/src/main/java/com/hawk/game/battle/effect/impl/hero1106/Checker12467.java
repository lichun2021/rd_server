...
@EffectChecker(effType = EffType.HERO_12467)
public class Checker12467 implements IChecker {
	...
	private int selectPlayer(CheckerParames parames) {
		...
		int val = valMap.entrySet().stream()
				...
				.limit(ConstProperty.getInstance().getEffect12466Maxinum())
				.mapToInt(item -> item.getValue().intValue()).sum();
		return val;
	}

	private int effvalue(BattleUnity unity, CheckerParames parames) {
		if (unity.getSolider().getEffVal(EffType.EFF_12062) <= 0) {
			return 0;
		}
