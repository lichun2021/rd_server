...
/**
 * - 【万分比】【12192】任命在战备参谋部时，集结战斗开始前，若集结全队出征直升机数量不低于集结部队总数 10%，己方全体空军部队发起轰炸时，伤害增加【固定数值 + XX.XX%】（多个克丽丝同时存在时，上述效果可叠加，至多 7 层）
  - 战报相关
    - 于战报中展示
    - 合并至精简战报中
  - 仅对集结战斗生效（包含集结进攻和集结防守）
  - 在战斗开始前判定，满足条件后本次战斗全程生效
  - 集结战斗开始前，若集结全队出征直升机数量不低于集结部队总数 10%
    - 数量1 = 集结全队出征携带的直升机（兵种类型 = 3）数量
      - 真实出征部队数量，在战斗开始前判定，即参谋军威技能、所罗门、吉迪恩 这种战斗中改变部队数量的机制对此无影响
    - 数量2 = 集结部队的出征数量总和
      - 真实出征部队数量，在战斗开始前判定，即参谋军威技能、所罗门、吉迪恩 这种战斗中改变部队数量的机制对此无影响
    - 数量1/数量2 >= 指定数值时生效
      - 指定数值读取const表，字段effect12192AllNumLimit
        - 配置格式：万分比
  - 己方全体空军部队发起轰炸时，伤害增加【固定数值 + XX.XX%】
    - 该作用号数值效果为【光环】效果，生效到己方全体空军部队上
      - 空军部队类型包含有：直升机（兵种类型 = 4）和轰炸机（兵种类型 = 3）
    - 轰炸伤害
      - 对直升机兵种，是指其兵种技能【黑鹰轰炸】【id = 403】发动时
      - 对轰炸机兵种，是指其兵种技能【俯冲轰炸】【id = 303】发动时
    - 单个玩家提供数值 = 该作用号固定数值 + 英雄参谋值*技能系数/10000
      - 该作用号固定数值读取const表，字段effect12192BaseVaule
        - 配置格式：万分比
    - 该作用号为常规外围属性加成效果，与其他作用号累加计算
      - 即 实际伤害 = 基础伤害*（1 + 各类加成 +【本作用值】）
    - （多个克丽丝同时存在时，上述效果可叠加，至多 7 层）
      ...
      - 效果记录在己方部队身上，数值直接叠加；限制该作用号生效层数上限
        ...
        - 层数上限读取const表，字段effect12192Maxinum
          - 配置格式：绝对值
 */
@BattleTupleType(tuple = { Type.SOLDIER_SKILL })
@EffectChecker(effType = EffType.HERO_12192)
public class Checker12192 implements IChecker {
	@Override
	public CheckerKVResult value(CheckerParames parames) {
		if (!isPlan(parames.type)||!BattleConst.WarEff.MASS.check(parames.troopEffType)) {
	...
	private int selectPlayer(CheckerParames parames) {
		...
		double tankCnt = parames.unitStatic.getArmyCountMapMarch().get(SoldierType.PLANE_SOLDIER_3);
		if (tankCnt / total < ConstProperty.getInstance().getEffect12192AllNumLimit() * GsConst.EFF_PER) {
			return 0;
		}
		...
		int val = valMap.entrySet().stream()
				...
				.limit(ConstProperty.getInstance().getEffect12192Maxinum())
