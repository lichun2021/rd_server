...
/**
 * - 【万分比】【12441】任命在战备参谋部时，集结战斗开始前，若集结全队出征直升机数量不低于集结部队总数 10%，己方全体远程部队攻击、防御、生命增加 【固定数值 + XX.XX%】（多个莫妮卡同时存在时，上述效果可叠加，至多 7 层）
  - 战报相关
    - 于战报中展示
    - 合并至精简战报中
  ...
  - 集结战斗开始前，若集结全队出征直升机数量不低于集结部队总数 10%
    ...
    - 数量2 = 集结部队的出征数量总和
      - 真实出征部队数量，在战斗开始前判定，即参谋军威技能、所罗门、吉迪恩 这种战斗中改变部队数量的机制对此无影响
    - 数量1/数量2 >= 指定数值时生效
      - 指定数值读取const表，字段effect12441AllNumLimit
        - 配置格式：万分比
  - 己方全体远程部队攻击、防御、生命增加 【固定数值+XX.XX%】
    ...
    - 单个玩家提供数值 = 该作用号固定数值 + 英雄参谋值*技能系数/10000
      - 该作用号固定数值读取const表，字段effect12441BaseVaule
        - 配置格式：万分比
    - 该作用号为常规外围属性加成效果，与其他作用号累加计算
      - 即 实际属性 = 基础属性*（1 + 各类加成 +【本作用值】）
  - （多个莫妮卡同时存在时，上述效果可叠加，至多 7 层）
    ...
    - 效果记录在己方部队身上，数值直接叠加；限制该作用号生效层数上限
      ...
      - 层数上限读取const表，字段effect12441Maxinum
        - 配置格式：绝对值
 */
@BattleTupleType(tuple ={Type.ATK,Type.DEF,Type.HP})
@EffectChecker(effType = EffType.HERO_12441)
public class Checker12441 implements IChecker {
	...
	private int selectPlayer(CheckerParames parames) {
		...
		if (tankCnt / total < ConstProperty.getInstance().getEffect12441AllNumLimit() * GsConst.EFF_PER) {
			return 0;
		}
		...
		int val = valMap.entrySet().stream()
				...
				.limit(ConstProperty.getInstance().getEffect12441Maxinum())
