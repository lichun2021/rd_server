...
/**
 * 【12553】
- 【万分比】【12553】任命在战备参谋部时，集结战斗开始前，若集结全队出征攻城车数量不低于集结部队总数 10%(effect12553AllNumLimit)，自身出征数量最多的攻城车单位在命中远程单位时，
降低目标攻击、防御、生命+XX.XX%（12553）+固定值（effect12553BaseVaule）*战备参谋部中任命艾拉的友方成员数量（至多 2 (effect12553Maxinum) 个攻城车单位生效，该效果可叠加，至多XX%(effect12553MaxValue)，己方艾拉计数时至多取 7  (effect12553CountMaxinum)个）
  - 战报相关
    - 于战报中隐藏
    - 不合并至精简战报中
  ...
  - 集结战斗开始前，若集结全队出征攻城车数量不低于集结部队总数 10%
    ...
    - 数量2 = 集结部队的出征数量总和
      - 真实出征部队数量，在战斗开始前判定，即参谋军威技能、所罗门、吉迪恩 这种战斗中改变部队数量的机制对此无影响
    - 数量1/数量2 >= 指定数值时生效
      - 指定数值读取const表，字段effect12553AllNumLimit
        - 配置格式：万分比
  - 自身出征数量最多的攻城车单位在命中远程单位时，降低目标攻击、防御、生命+XX.XX%+固定值*战备参谋部中任命艾拉的友方成员数量
    ...
    - 固定值读取const表，字段effect12553BaseVaule
      - 配置格式：万分比
  - （至多 2 个攻城车单位生效，该效果可叠加，至多XX%)
    - 集结中存在多个此作用号时，可以叠加
    - 集结己方生效此作用号的单位有数量限制，超出数量限制则取作用号数值高的单位
      - 数值上限读取const表，字段effect12553Maxinum
        - 配置格式：绝对值
    - 效果记录在敌方部队身上，数值直接叠加，即
      - 本作用号属性加成降低 = 攻城车A造成累计属性降低 + 攻城车B造成累计属性降低 
      - 限制该作用号叠加数值上限
        - 数值上限读取const表，字段effect12553MaxValue
          - 配置格式：万分比
  - （己方艾拉计数时至多取 7  个）
    - 取集结己方委任英雄中携带艾拉（ID1109）的玩家数量
    - 该计数有最高值限制，读取const表，字段effect12553CountMaxinum
      - 配置格式：绝对值
 */
@BattleTupleType(tuple = { Type.SOLDIER_SKILL })
@EffectChecker(effType = EffType.EFF_12553)
public class Checker12553 implements IChecker {

	@Override
	public CheckerKVResult value(CheckerParames parames) {
		if (parames.unity != parames.getPlayerMaxFreeArmy(parames.unity.getPlayerId(), SoldierType.CANNON_SOLDIER_7) || !BattleConst.WarEff.MASS.check(parames.troopEffType)) {
	...
	private Map<String, Integer> selectPlayer(CheckerParames parames) {
		...
		if (tankCnt / total < ConstProperty.getInstance().effect12553AllNumLimit * GsConst.EFF_PER) {
			return new HashMap<>();
		}

		// 战备参谋部中任命艾拉的友方成员数量
		int effect12553CountMaxinum = parames.unityList.stream().filter(unity -> unity.getEffVal(effType()) > 0).map(unity -> unity.getPlayerId()).collect(Collectors.toSet())
				.size();
		...
		for (BattleUnity unity : parames.unityList) {
			...
			if (unity.getEffVal(effType()) > 0) {
				int effvalue = ConstProperty.getInstance().effect12553BaseVaule * effect12553CountMaxinum + unity.getEffVal(effType());
		...
		Map<String, Integer> val = valMap.entrySet().stream()
				...
				.limit(ConstProperty.getInstance().effect12553Maxinum)
