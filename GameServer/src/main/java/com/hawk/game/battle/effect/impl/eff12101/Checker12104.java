...
import com.hawk.game.battle.IPlanSoldier;
...
@EffectChecker(effType = EffType.EFF_12104)
public class Checker12104 implements IChecker {
	@Override
	public CheckerKVResult value(CheckerParames parames) {
		int effPer = 0;
		int effNum = 0;
		if (parames.type == SoldierType.FOOT_SOLDIER_5 || parames.type == SoldierType.FOOT_SOLDIER_6) {
			int tarPlancnt = (int) parames.tarStatic.getUnityList().stream().filter(u -> u.getSolider() instanceof IPlanSoldier).count();
			tarPlancnt = Math.min(tarPlancnt, ConstProperty.getInstance().getEffect12104MaxNum());
			effPer = parames.unity.getEffVal(effType()) * tarPlancnt;
		}
