...
/**
 * - 【万分比】【12193】任命在战备参谋部时，直升机每受到 1 次攻击后，伤害减少 +XX.XX%（该效果可叠加，至多 20 层）
  - 战报相关
    - 于战报中展示
    - 不合并至精简战报中
  - 该作用号对玩家携带的所有直升机（兵种类型 = 4）部队均生效
  - 该作用号可叠加，受叠加次数上限限制
    - 层数上限读取const表，字段effect12193Maxinum
      - 配置格式：绝对值
  - 该伤害减少效果与其他伤害减少效果累乘计算；即实际伤害 = 基础伤害*（1 + 敌方各类伤害加成）*（1 - 己方某伤害减少）*（1 - 本作用值伤害减少）
 */
@BattleTupleType(tuple = { Type.SOLDIER_SKILL })
@EffectChecker(effType = EffType.HERO_12193)
public class Checker12193 implements IChecker {
	@Override
