...
/**
 * 【12281】
- 【万分比】【12281】近战鼓舞：集结战前，若自身出征防御坦克数量超过自身出征部队总数 50%且不低于集结部队总数 5%，使自身出征数量最多的防御坦克受到攻击时，生命加成 +XX.XX%（多个埃琳娜存在时，至多有 2 个防御坦克生效）
  - 战报相关
    - 于战报中展示
    - 合并至精简战报中
  ...
  - 若自身出征防御坦克数量超过自身出征部队总数 50%
    ...
    - 数量2 = 某玩家出征数量
      - 真实出征部队数量，在战斗开始前判定，即参谋军威技能、所罗门、吉迪恩 这种战斗中改变部队数量的机制对此无影响
    - 数量1/数量2 > 指定数量时生效
      - 指定数值读取const表，字段effect12281SelfNumLimit
        - 配置格式：万分比
  - 若自身出征携带防御坦克数量不低于集结部队总数 5%
    ...
    - 数量2 = 集结部队的出征数量总和
      - 真实出征部队数量，在战斗开始前判定，即参谋军威技能、所罗门、吉迪恩 这种战斗中改变部队数量的机制对此无影响
    - 数量1/数量2 >= 指定数值时生效
      - 指定数值读取const表，字段effect12281AllNumLimit
        - 配置格式：万分比
  - 使自身出征数量最多的防御坦克受到攻击时，生命加成 +XX.XX%（多个埃琳娜存在时，至多有 2 个防御坦克生效）
    - 满足条件后，该作用号效果仅对玩家出征时数量最多的防御坦克（兵种类型 = 1）生效（若存在多个防御坦克数量一样且最高，取等级高的）（战中不改变目标）
    - 此加成效果为常规外围属性加成；即
      - 实际生命 = 基础生命*（1 + 各类生命加成 + 【本作用值】）
    - 单玩家拥有此作用号时，只能提供 1 层效果；集结中存在多个此作用号时，可以同时生效
    - 效果记录在己方部队身上；限制该作用号生效的玩家数量
      - 注：这里限制的是生效玩家数量，若集结中拥有此作用号的玩家数量超出此上限，取作用号数值高的生效
      - 层数上限读取const表，字段effect12281Maxinum
        - 配置格式：绝对值
 */
@BattleTupleType(tuple = Type.HP)
@EffectChecker(effType = EffType.EFF_12281)
public class Checker12281 implements IChecker {
	@Override
	...
	private Map<String, Integer> selectPlayer(CheckerParames parames) {
		...
		valMap = valMap.entrySet().stream()
				...
				.limit(ConstProperty.getInstance().getEffect12281Maxinum())
	...
	private int effvalue(BattleUnity unity, CheckerParames parames) {
		try {
			...
			if (march8cnt / parames.unitStatic.getTotalCountMarch() >= ConstProperty.getInstance().getEffect12281AllNumLimit() * GsConst.EFF_PER) {
				// 若自身携带超过自身部队总数 50% 的采矿车
				if (march8cnt * 1D / parames.unitStatic.getPlayerArmyCountMapMarch().get(playerId) > ConstProperty.getInstance().getEffect12281SelfNumLimit() * GsConst.EFF_PER) {
