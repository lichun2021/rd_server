...
@EffectChecker(effType = EffType.HERO_1612)
public class Checker1612 implements IChecker {
	@Override
	public CheckerKVResult value(CheckerParames parames) {
		...
		if (count > 0 && isSoldier(parames.type) && BattleConst.WarEff.MASS.check(parames.troopEffType)) {
			...
			effPer = Math.min(15, count / ConstProperty.getInstance().getEffect1612NumParam()) * effPer;
		}
