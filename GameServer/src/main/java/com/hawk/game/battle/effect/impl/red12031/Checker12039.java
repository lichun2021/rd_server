...
/**
- 【万分比】【12039】集结战斗时，自身出征数量最大的采矿车（兵种类型 = 8）部队，每受到1次攻击，则降低敌方伤害加成 +XX.XX%（至多叠加 10 层）
- 战报相关
  - 于战报中展示
  - 不合并至精简战报中
...
- 集结中存在多个此作用号时，可以同时生效
- 效果记录在敌方部队身上，数值直接叠加；限制该作用号叠加层数上限
  - 先到先叠，叠满 10 层后则不再变化
  - 层数上限读取const表，字段effect12039Maxinum
    - 配置格式：绝对值
 */

@BattleTupleType(tuple = Type.SOLDIER_SKILL)
@EffectChecker(effType = EffType.EFF_12039)
public class Checker12039 implements IChecker {
	@Override
