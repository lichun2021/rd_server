...
@EffectChecker(effType = EffType.HERO_1650)
public class Checker1650 implements IChecker {
	@Override
	public CheckerKVResult value(CheckerParames parames) {
		...
		if (BattleConst.WarEff.MASS.check(parames.troopEffType)) {
			...
			double taotal1 = parames.getPlayerArmyCount(playerId, SoldierType.TANK_SOLDIER_1);
			if (taotal1 > 0) {
				effPer = parames.unity.getEffVal(effType());
				double taotalTai1 = Optional.ofNullable(parames.unitStatic.getPlantUnityStatistics().getPlayerSoldierCount().get(playerId, SoldierType.TANK_SOLDIER_1)).orElse(0);
				if (taotalTai1 / taotal1 > ConstProperty.getInstance().getEffect1650Per() * GsConst.EFF_PER) {
					effPer = effPer * 2;
				}
			}
		}
