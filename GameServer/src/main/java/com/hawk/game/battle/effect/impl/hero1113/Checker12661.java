...
/**
 *【12661】
- 【万分比】【12661】任命在战备参谋部时，集结战斗开始前，若集结全队出征空军数量不低于集结部队总数 20%(effect12661AllNumLimit)，己方全体部队攻击、防御、生命增加【固定数值(effect12661BaseVaule)  + XX.XX%（12661）】（多个乔希同时存在时，上述效果可叠加，至多 7  (effect12661Maxinum)层）
  - 战报相关
    - 于战报中展示
    - 合并至精简战报中
  ...
  - （多个乔希同时存在时，上述效果可叠加，至多 7 层）
    ...
    - 效果记录在己方部队身上，数值直接叠加；限制该作用号生效层数上限
      ...
      - 层数上限读取const表，字段effect12661Maxinum
        - 配置格式：绝对值
  - ->针对兵种留个内置系数
    - 实际针对自身各兵种类型，单独配置系数；自身兵种修正系数 读取const表，字段effect12661SoldierAdjust
      - 配置格式：兵种类型id1_修正系数1，......兵种类型id8_修正系数8
        - 修正系数具体配置为万分比
    - 配置格式：万分比
 */
@BattleTupleType(tuple = { Type.ATK, Type.DEF, Type.HP })
@EffectChecker(effType = EffType.EFF_12661)
public class Checker12661 implements IChecker {
	@Override
	public CheckerKVResult value(CheckerParames parames) {
		...
		effPer = (int) (effPer * GsConst.EFF_PER * ConstProperty.getInstance().effect12661SoldierAdjustMap.getOrDefault(parames.type, 10000));
	...
	private int selectPlayer(CheckerParames parames) {
		...
		double tankCnt = parames.unitStatic.getArmyCountMapMarch().get(SoldierType.PLANE_SOLDIER_3) + parames.unitStatic.getArmyCountMapMarch().get(SoldierType.PLANE_SOLDIER_4);
		if (tankCnt / total < ConstProperty.getInstance().effect12661AllNumLimit * GsConst.EFF_PER) {
			return 0;
		}
		...
		for (BattleUnity unity : parames.unityList) {
			...
			if (unity.getEffVal(effType()) > 0) {
				int effvalue = ConstProperty.getInstance().effect12661BaseVaule + unity.getEffVal(effType());
		...
		int val = valMap.entrySet().stream()
				...
				.limit(ConstProperty.getInstance().effect12661Maxinum)
