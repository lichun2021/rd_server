...
@EffectChecker(effType = EffType.EFF_12571)
public class Checker12571 implements IChecker {
	@Override
	...
	private Map<String, Integer> selectPlayer(CheckerParames parames) {
		...
		valMap = valMap.entrySet().stream()
				...
				.limit(ConstProperty.getInstance().effect12571Maxinum)
	...
	private int effvalue(BattleUnity unity, CheckerParames parames) {
		try {
			...
			if (march8cnt / parames.unitStatic.getTotalCountMarch() >= ConstProperty.getInstance().effect12571AllNumLimit * GsConst.EFF_PER) {
				// 若自身携带超过自身部队总数 50% 的采矿车
				if (march8cnt * 1D / parames.unitStatic.getPlayerArmyCountMapMarch().get(playerId) > ConstProperty.getInstance().effect12571SelfNumLimit * GsConst.EFF_PER) {
