...
@EffectChecker(effType = EffType.EFF_12575)
public class Checker12575 implements IChecker {
	@Override
	public CheckerKVResult value(CheckerParames parames) {
		if (parames.solider.getEffVal(EffType.EFF_12571) <= 0) {
			return CheckerKVResult.DefaultVal;
		}
		int atkper = parames.solider.tupleValue(BattleTupleType.Type.ATKFIRE, SoldierType.XXXXXXXXXXXMAN).first;
		int avg = (atkper ) / 3;
		
		int effPer = (int) (parames.unity.getEffVal(effType()) * GsConst.EFF_PER * avg + ConstProperty.getInstance().effect12575BaseVaule);
