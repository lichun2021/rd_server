...
/**
 * 【12337~12338】
- 【万分比】【12337~12338】集结战斗开始前，若自身出征攻城车数量超过自身出征部队总数 50%且不低于集结部队总数 5%，自身出征数量最多的攻城车于本场战斗中获取如下效果（多个威尔森同时存在时，至多有 2 个攻城车单位生效）;在战斗处于偶数回合时，开启聚能环护模式:  于本回合为至多 2 个友军远程单位提供战场援护，使其防御、生命增加 【XX.XX% + 援护值*XX.XX%】（该效果无法叠加）
  - 战报相关
    - 于战报中隐藏
    - 不合并至精简战报中
  - 其中【12337】为主作用号，主导上述所有作用号逻辑；【12338】为辅作用号，仅关联计算属性加成效果
  ...
  - 集结战斗开始前，若自身出征携带不低于集结部队总数 5% 的攻城车
    ...
    - 数量2 = 集结部队的出征数量总和
      - 真实出征部队数量，在战斗开始前判定，即参谋军威技能、所罗门、吉迪恩 这种战斗中改变部队数量的机制对此无影响
    - 数量1/数量2 >= 指定数值时生效
      - 指定数值读取const表，字段effect12337AllNumLimit
        - 配置格式：万分比
  - 集结战斗开始前，若自身携带超过自身出征部队总数 50% 的攻城车
    ...
    - 数量2 = 某玩家出征数量
      - 真实出征部队数量，在战斗开始前判定，即参谋军威技能、所罗门、吉迪恩 这种战斗中改变部队数量的机制对此无影响
    - 数量1/数量2 > 指定数量时生效
      - 指定数值读取const表，字段effect12337SelfNumLimit
        - 配置格式：万分比
  - 自身出征数量最多的攻城车
  ...
  - 于本回合为至多 2 个友军远程单位提供战场援护，使其防御、生命增加 【XX.XX% + 援护值*XX.XX%】（该效果无法叠加）
    ...
    - 至多选中的友军单位数量读取const表，字段effect12337AffectNum
      - 配置格式：绝对值
  - （多个威尔森同时存在时，至多有 2 个攻城车单位生效）
    ...
    - 限制该作用号生效人数上限
      ...
      - 层数上限读取const表，字段effect12337Maxinum
        - 配置格式：绝对值
 */
@BattleTupleType(tuple = Type.SOLDIER_SKILL)
@EffectChecker(effType = EffType.HERO_12337)
public class Checker12337 implements IChecker {
	@Override
	...
	private Map<String, Integer> selectPlayer(CheckerParames parames) {
		...
		valMap = valMap.entrySet().stream()
				...
				.limit(ConstProperty.getInstance().getEffect12337Maxinum())
	...
	private int effvalue(BattleUnity unity, CheckerParames parames) {
		try {
			...
			if (march8cnt / parames.unitStatic.getTotalCountMarch() >= ConstProperty.getInstance().getEffect12337AllNumLimit() * GsConst.EFF_PER) {
				// 若自身携带超过自身部队总数 50% 的采矿车
				if (march8cnt * 1D / parames.unitStatic.getPlayerArmyCountMapMarch().get(playerId) > ConstProperty.getInstance().getEffect12337SelfNumLimit() * GsConst.EFF_PER) {
