...
@EffectChecker(effType = EffType.HERO_1681)
public class Checker1681 implements IChecker {
	@Override
	public CheckerKVResult value(CheckerParames parames) {
		...
		BattleUnity max8 = parames.unitStatic.getPlayerSoldierTable().get(playerId, SoldierType.CANNON_SOLDIER_8).get(0);
		...
		// 泰能采矿车
		int march8Plantcnt = parames.unitStatic.getPlantUnityStatistics().getPlayerSoldierCountMarch().get(playerId, SoldierType.CANNON_SOLDIER_8);
		// 若自身出征携带不低于集结部队总数 5% 的采矿车
		if (march8cnt / parames.unitStatic.getTotalCountMarch() >= ConstProperty.getInstance().getEffect1681AllNumLimit() * GsConst.EFF_PER) {
			effPer = parames.unity.getEffVal(effType());
		}
		// 若自身出征携带超过自身出征数量 50% 的泰能采矿车
		if (march8Plantcnt * 1D / parames.unitStatic.getPlayerArmyCountMapMarch().get(playerId) > ConstProperty.getInstance().getEffect1681SelfNumLimit() * GsConst.EFF_PER) {
			effPer = (int) (effPer * GsConst.EFF_PER * ConstProperty.getInstance().getEffect1681RateValue());
		}
