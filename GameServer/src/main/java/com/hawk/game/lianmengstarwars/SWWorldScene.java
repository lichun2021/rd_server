...
import com.hawk.game.lianmengstarwars.player.SWPlayerEye;
 ...
 * @date 2020年9月14日
 */
public class SWWorldScene extends HawkAOIScene {
	 ...
	 * @param maxRadius
	 */
	public SWWorldScene(int width, int height, int searchRadius) {
	...
	protected void notifyObjEnter(HawkAOIObj aoiObj, Set<HawkAOIObj> targetObjs) {
		...
		ISWWorldPoint worldPoint = (ISWWorldPoint) aoiObj.getUserData();
		if (worldPoint == null) {
			return;
		}
		...
		for (HawkAOIObj targetObj : targetObjs) {
			if (targetObj.getType() == WorldObjType.PLAYER) {
				SWPlayerEye playerEye = (SWPlayerEye) targetObj.getUserData();
				if (playerEye == null) {
					continue;
				}
				builder.clearPoints();
	...
	protected void syncSceneObjs(HawkAOIObj aoiObj, Set<HawkAOIObj> oldInviewObjs, Set<HawkAOIObj> nowInviewObjs, float moveSpeed) {
		if (aoiObj.getType() == GsConst.WorldObjType.PLAYER && GsApp.getInstance().isRunning()) {
			SWPlayerEye playerEye = (SWPlayerEye) aoiObj.getUserData();
			if (playerEye == null) {
				return;
			}
			...
			for (HawkAOIObj inviewObj : inviewObjs) {
				try {
					...
					ISWWorldPoint worldPoint = (ISWWorldPoint) inviewObj.getUserData();
					if (worldPoint == null) {
						continue;
					}
	 ...
	 * @return
	 */
	public List<WorldPointPB.Builder> getPlayerViewObjs(SWPlayerEye viewer, int x, int y, float moveSpeed) {
	 ...
	 * @return
	 */
	private int fillWorldPoints(SWPlayerEye viewer, Set<HawkAOIObj> inviewObjs, float moveSpeed, List<WorldPointPB.Builder> builderList) {
		float randFactor = 1.0f;
		if (!HawkOSOperator.isZero(GameConstCfg.getInstance().getMoveSyncFactor())) {
			randFactor = (float) Math.pow(GameConstCfg.getInstance().getMoveSyncFactor() - moveSpeed, 2);
		}
