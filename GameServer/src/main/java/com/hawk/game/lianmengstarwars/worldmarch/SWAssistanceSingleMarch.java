...
public class SWAssistanceSingleMarch extends ISWWorldMarch implements ISWPassiveAlarmTriggerMarch, ISWReportPushMarch {

	public SWAssistanceSingleMarch(ISWPlayer parent) {
		super(parent);
	}
	...
	public void onMarchReach(Player parent) {
		...
		// 路点为空
		if (point == null || !(point instanceof ISWPlayer)) {
		...
		if (backList.size() > 0) {
			...
			ISWWorldMarch back = getParent().getParent().startMarch(getParent(), point, getParent(), WorldMarchType.ASSISTANCE, "",0, effParams);
