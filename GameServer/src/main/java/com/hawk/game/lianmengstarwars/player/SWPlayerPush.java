...
public class SWPlayerPush extends ISWPlayerPush {
	...
	public SWPlayerPush(Player player) {
		super(player);
	}
	...
	public void syncGuildWarCount() {
		ISWPlayer player = getPlayer();
		HPGuildWarCountPush.Builder builder = HPGuildWarCountPush.newBuilder();
		int count = 0;
		if (player.getParent().getGuildStatisticMap().containsKey(player.getGuildId())) {
			count = player.getParent().getGuildStatisticMap().get(player.getGuildId()).getCampAGuildWarCount();
		}
	...
	public void syncPlayerInfo() {
		...
		newBuilder.setSwKillHonor((int) (player.getKillPower() + player.getDeadPower()));
		newBuilder.setLmjyState(getPlayer().getSwState().intValue());
	...
	public void pushJoinGame() {
		...
		SWPlayerData playerData = (SWPlayerData) getData();
		playerData.unLockOriginalData();
		// 隐藏真实数据
		getPlayer().sendProtocol(HawkProtocol.valueOf(HP.code.SW_CLEAN_CLAIENT_DATA_VALUE));
		...
		getPlayer().refreshPowerElectric(PowerChangeReason.SW_INIT);
		// // 世办点开罩子
	...
	public void pushGameOver() {
		...
		source.sendProtocol(HawkProtocol.valueOf(HP.code.SW_CLEAN_CLAIENT_DATA_VALUE));
		// 清除军演数据
		/** 重新推送真实数据 */
		...
		source.refreshPowerElectric(PowerChangeReason.SW_INIT);
		// // 世办点开罩子
		// WorldPoint worldPoint =
		// WorldPlayerService.getInstance().getPlayerWorldPoint(player.getId());
		// long shiledEnd = battleRoom.getOverTime() + PORTECTED_A;
		...
		// }
		// }
		SWQuitRoomMsg msg = SWQuitRoomMsg.valueOf(getPlayer().getSWQuitReason(),getPlayer().getParent().getId());
		HawkApp.getInstance().postMsg(player.getXid(), msg);
	}

	public SWPlayer getPlayer() {
		return (SWPlayer) player;
	}

	@Override
	public ISWPlayerData getData() {
		return (ISWPlayerData) player.getData();
	}
