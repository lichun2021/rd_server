...
import com.hawk.game.lianmengstarwars.player.module.SWGuildFormationModule;
...
public abstract class ISWWorldMarch implements IWorldMarch {
	...
	private SWMarchEntity marchEntity;

	public ISWWorldMarch(ISWPlayer parent) {
		this.parent = parent;
	}

	public final ISWPlayer getParent() {
		return parent;
	}

	@Override
	public SWMarchEntity getMarchEntity() {
		return marchEntity;
	}

	public void setMarchEntity(SWMarchEntity marchEntity) {
		this.marchEntity = marchEntity;
	}

	public GuildWarShowPB getGuildWarShoPb(ISWPlayer viewer) {
		// 行军状态
		...
		try {
			GuildFormationObj formationObj = getParent().getParent().getGuildFormation(viewer.getGuildId());
	...
	public WorldMarchPB.Builder toBuilder(WorldMarchRelation relation) {
		...
		SWMarchEntity entity = getMarchEntity();
	...
	public WorldMarchStatus getMassJoinMarchStatus(String playerId) {
		try {
			...
			Set<ISWWorldMarch> massJoinMarchs = getMassJoinMarchs(false);
	...
	public boolean speedUp(int timeReducePercent, int speedUpTimes) {
		...
		if (this.isMassMarch()) {
			Set<ISWWorldMarch> joinMarchs = getMassJoinMarchs(false);
			for (ISWWorldMarch joinMarch : joinMarchs) {
		...
		if (this instanceof ISWReportPushMarch) {
			((ISWReportPushMarch) this).reachTimeChange();
		}
		return true;
	}

	@Override
	public Set<ISWWorldMarch> getQuarterMarch() {
		if (getMarchStatus() != WorldMarchStatus.MARCH_STATUS_MARCH_QUARTERED_VALUE) {
			return Collections.emptySet();
		}
		List<ISWWorldMarch> defMarchList = getParent().getParent().getPointMarches(getTerminalId(), WorldMarchStatus.MARCH_STATUS_MARCH_QUARTERED);
		return new HashSet<>(defMarchList);
	}

	@Override
	public Set<ISWWorldMarch> getMassJoinMarchs(boolean b) {
		return Collections.emptySet();
	}
	...
	public void notifyMarchEvent(MarchEvent eventType) {
		...
		if (eventType == MarchEvent.MARCH_UPDATE && !(marchStatus == WorldMarchStatus.MARCH_STATUS_MARCH_VALUE
	...
	private void pushMarchEvent(MarchEvent eventType) {
		...
		// 计算可见起点和终点的玩家集合
		// 通知所有影响到的玩家本条行军事件
		Table<String, WorldMarchRelation, Set<String>> table = ConcurrentHashTable.create();
		for (GuildStaticInfo gstic : getParent().getParent().getGuildStatisticMap().values()) {
			table.put(gstic.getCampAServerId(), WorldMarchRelation.GUILD_FRIEND, new HashSet<>());
			table.put(gstic.getCampAServerId(), WorldMarchRelation.ENEMY, new HashSet<>());
			table.put(gstic.getCampAServerId(), WorldMarchRelation.TEAM_LEADER, new HashSet<>());
			table.put(gstic.getCampAServerId(), WorldMarchRelation.NONE, new HashSet<>());
		}

		for (ISWPlayer player : getParent().getParent().getPlayerList(SWState.GAMEING)) {
			...
			if (!player.isCsPlayer()) {
				...
				if (table.contains(player.getMainServerId(), relation)) {
					table.get(player.getMainServerId(), relation).add(player.getId());
				}
			}
		}

		for (Cell<String, WorldMarchRelation, Set<String>> cell : table.cellSet()) {
			if (cell.getRowKey().equals(GsConfig.getInstance().getServerId())) {
				continue;
			}
			String csServerId = GlobalData.getInstance().getMainServerId(cell.getRowKey());
			MarchEventSync.Builder builder = createSyncPB(eventType, cell.getColumnKey());
			CrossProxy.getInstance().broadcastProtocolV2(csServerId, cell.getValue(), HawkProtocol.valueOf(HP.code.WORLD_MARCH_EVENT_SYNC_VALUE, builder));
		}

		{
			MarchEventSync.Builder builder = createSyncPB(eventType, WorldMarchRelation.NONE);
			getParent().getParent().getVideoPlayer().sendBroadVideoProtocol(HawkProtocol.valueOf(HP.code.WORLD_MARCH_EVENT_SYNC_VALUE, builder));
		}
	}
	 ...
	 * @return
	 */
	public WorldMarchRelation getRelation(ISWPlayer tarplayer) {
		...
		// 目标点
		Optional<ISWWorldPoint> targetPointOp = getParent().getParent().getWorldPoint(marchEntity.getTerminalId());
		...
		ISWWorldPoint targetPoint = targetPointOp.get();
	...
	public void sendBattleResultInfo(boolean isWin, List<ArmyInfo> atkArmyList, List<ArmyInfo> defArmyList) {
		...
		List<ISWPlayer> players = getParent().getParent().getPlayerList(SWState.GAMEING);
		for (ISWPlayer pla : players) {
	...
	public void remove() {
		if (this instanceof ISWReportPushMarch) {
			((ISWReportPushMarch) this).removeAttackReport();
		}
	...
	public void updateDefMarchAfterWar(List<ISWWorldMarch> helpMarchList, Map<String, List<ArmyInfo>> defArmyMap) {
		// 更新援助防御玩家行军的部队
		for (ISWWorldMarch tmpMarch : helpMarchList) {
			...
			if (tmpMarch.getMarchType() == WorldMarchType.ASSISTANCE) {
				...
				ISWPlayer assistPlayer = tmpMarch.getParent();
				assistPlayer.sendProtocol(HawkProtocol.valueOf(HP.code.ASSISTANCE_MARCH_CALLBACK, callbackNotifyPB));
			}

			// 死光了，行军立即送死兵回家
			tmpMarch.onMarchReturn(tmpMarch.getMarchEntity().getTerminalId(), tmpMarch.getMarchEntity().getOrigionId(),
					leftList);
		}
	}
	...
	private double effectToMarchTime(double time, boolean slowDown, double slowDownTime) {
		...
		if (this.isMassJoinMarch()) {
			Optional<ISWWorldPoint> targetPoint = getParent().getParent().getWorldPoint(marchEntity.getTerminalId());
			ISWWorldPoint iswWorldPoint = targetPoint.orElse(null);
			if (iswWorldPoint instanceof ISWPlayer) {
				ISWPlayer leader = (ISWPlayer) iswWorldPoint;
	...
	public double getMarchBaseSpeed() {
		...
		if (isMassMarch()) {
			Set<ISWWorldMarch> joinMarchs = getMassJoinMarchs(true);
			for (ISWWorldMarch tempMarch : joinMarchs) {
	...
	public GuildWarTeamInfo.Builder getGuildWarInitiativeInfo() {
		...
		// 联盟编队
		GuildFormationObj formationObj = getParent().getParent().getGuildFormation(leader.getGuildId());
		GuildFormationCell formation = formationObj.getGuildFormation(getMarchId());
		// 队长位置
		...
		// 加入集结的行军
		Set<ISWWorldMarch> joinMarchs = this.getMassJoinMarchs(false);
		for (ISWWorldMarch joinMarch : joinMarchs) {
		...
		if (formation != null) {
			for (String fightId : formation.getFightIds()) {
				...
				Optional<ISWWorldMarch> findAny = joinMarchs.stream().filter(m -> m.getPlayerId().equals(fightId)).findAny();
				...
				SWGuildFormationModule module = joinMemeber.getModule(SWConst.ModuleType.SWGuildFormation);
	...
	public GuildWarTeamInfo.Builder getGuildWarPassivityInfo() {
		...
		List<ISWWorldMarch> assistandMarchs = getParent().getParent().getPointMarches(leader.getPointId(),
	...
	public <T extends ISWWorldPoint> T getTerminalWorldPoint() {
		return (T) getParent().getParent().getWorldPoint(this.getMarchEntity().getTerminalX(),
				this.getMarchEntity().getTerminalY()).orElse(null);
	}
