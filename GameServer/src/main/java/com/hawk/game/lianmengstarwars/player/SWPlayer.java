...
import com.hawk.game.lianmengstarwars.player.action.SWTestMarchAction;
import com.hawk.game.lianmengstarwars.player.module.SWArmyModule;
...
import com.hawk.game.lianmengstarwars.player.module.SWWorldModule;
...
public class SWPlayer extends ISWPlayer {
	...
	private SWTestMarchAction action;
	...
	public void init() {
		...
		setEye(new SWPlayerEye(this));
		int maxCityDef = source.getData().getRealMaxCityDef();
		this.setMaxCityDef(maxCityDef);
		this.setCityDefVal(getMaxCityDef());
		this.setPlayerData(SWPlayerData.valueOf(this));
		SWPlayerPush playerPush = new SWPlayerPush(this);
		...
		this.nextLoghonor = getParent().getCurTimeMil() + SWConst.MINUTE_MICROS;
	...
	public boolean onTick() {
		...
		if (now - lastTickTime > 1000) {
			...
			if (getOnFireEndTime() > now) {
				...
				// .setChatType(ChatType.CHAT_SW_SYS)
				// .setKey(NoticeCfgId.SW_CITY_DEF_30)
				// .addParms(getName())
				// .build();
		...
		if(lastSyncKillHonor != getKillPower()){
			getPush().syncPlayerInfo();
			lastSyncKillHonor = (int) getKillPower();
		}

		if (now > nextLoghonor) {
			nextLoghonor = now + SWConst.MINUTE_MICROS;
		}
	...
	public void checkSuperVipSkinToMassMarch() {
		try {
			...
			List<ISWWorldMarch> marchs = getParent().getPlayerMarches(getId());
	...
	public void quitGame() {
		...
		for (ArmyEntity army : getData().getArmyEntities()) {
			...
			if (march > 0) {
				...
				HawkLog.logPrintln("SW PLAYER QUIT MARBACK soldierId: {} march: {}", army.getArmyId(), march);
			}
		}

		SWRoomManager.getInstance().invalidate(this);
	}

	@Override
	public String getSwRoomId() {
		return source.getSwRoomId();
	}

	@Override
	public void setSwRoomId(String swRoomId) {
		source.setSwRoomId(swRoomId);
	}

	@Override
	public SWState getSwState() {
		return source.getSwState();
	}

	@Override
	public void setSwState(SWState swState) {
		source.setSwState(swState);
	}
	...
	public void initModules() {
		registerModule(SWConst.ModuleType.SWWorld, new SWWorldModule(this));
		registerModule(SWConst.ModuleType.SWMarch, new SWMarchModule(this));
		registerModule(SWConst.ModuleType.SWArmy, new SWArmyModule(this));
		// 联盟编队
		registerModule(SWConst.ModuleType.SWGuildFormation, new SWGuildFormationModule(this));
	}
	...
	public boolean sendProtocol(HawkProtocol protocol) {
		if (Objects.equals(getSwRoomId(), getParent().getXid().getUUID())) {
