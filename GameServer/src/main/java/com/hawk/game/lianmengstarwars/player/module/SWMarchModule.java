...
import com.hawk.game.lianmengstarwars.invoker.SWItemConsumeInvoker;
...
public class SWMarchModule extends PlayerModule {
	private ISWPlayer player;
	public SWMarchModule(ISWPlayer player) {
		super(player);
		this.player = player;
	}
	/**
	...
	public boolean marchEmoticonUse(HawkProtocol protocol, int emoticonId, String marchId) {
		ISWWorldMarch march = player.getParent().getMarch(marchId);
		if (march == null) {
		...
		if (march.getMarchStatus() != WorldMarchStatus.MARCH_STATUS_MARCH_VALUE 
			...
			ISWWorldPoint worldPoint = player.getParent().getWorldPoint(march.getTerminalId()).get();
			if (worldPoint instanceof ISWBuilding) {
				WorldMarch firstMarch = ((ISWBuilding) worldPoint).getLeaderMarch().getMarchEntity();
	...
	private boolean onInviteMass(HawkProtocol protocol) {
		...
		ISWWorldMarch march = player.getParent().getMarch(req.getMarchId());
		if (march == null) {
		...
		Optional<ISWWorldPoint> worldPoint = player.getParent().getWorldPoint(march.getMarchEntity().getTerminalId());
		if (worldPoint.isPresent()) {
			GuildFormationObj formationObj = player.getParent().getGuildFormation(player.getGuildId());
	 ...
	 * 发送公告
	 */
	public void sendNotice(ISWWorldPoint point, ISWWorldMarch march, int formationIndex, String formationName) {
		ISWPlayer player = march.getParent();
		if (point instanceof ISWPlayer) {
			ChatParames parames = ChatParames.newBuilder().setPlayer(player).setChatType(ChatType.CHAT_FUBEN_TEAM)
					.setKey(NoticeCfgId.SW_MASS_START_PLAYER).addParms(((ISWPlayer) point).getName())
		...
		} else if (point instanceof ISWBuilding) {
			ChatParames parames = ChatParames.newBuilder().setPlayer(player).setChatType(ChatType.CHAT_FUBEN_TEAM)
					.setKey(NoticeCfgId.SW_MASS_START_BUILD).addParms(point.getX()).addParms(point.getY())
					.addParms(player.getParent().getExtParm().getWarType().getNumber())
					.addParms(march.getMarchId()).addParms(player.getX()).addParms(player.getY())
	...
	private void onYuriMonsterQueryHelp(HawkProtocol protocol) {
		...
		// 行军已不存在
		ISWWorldMarch worldMarch = player.getParent().getMarch(req.getMarhcId());
		if (worldMarch == null) {
			return;
		}

		Optional<ISWWorldPoint> point = player.getParent().getWorldPoint(worldMarch.getTerminalId());
		int pointType = point.isPresent() ? point.get().getPointType().getNumber() : 0;
		ChatParames parames = ChatParames.newBuilder().setPlayer(player).setChatType(ChatType.CHAT_FUBEN_TEAM)
				.setKey(NoticeCfgId.SW_MARCH_QUERY_HELP).addParms(worldMarch.getTerminalX())
	...
	private boolean onRepatriateManormarch(HawkProtocol protocol) {
		...
		// 世界点
		ISWWorldPoint worldPoint = player.getParent().getWorldPoint(req.getX(), req.getY()).orElse(null);
		if (worldPoint == null) {
		...
		// 战区
		if (worldPoint instanceof ISWBuilding) {
			ISWBuilding building = (ISWBuilding) worldPoint;
	...
	protected boolean onWorldMassDissolve(HawkProtocol protocol) {
		...
		// check
		ISWWorldMarch worldMarch = player.getParent().getMarch(req.getMarchId());
		if (worldMarch == null) {
			player.sendError(protocol.getType(), Status.Error.WORLD_MARCH_NOT_EXISIT);
			return false;
		}
		...
		for (ISWWorldMarch jm : worldMarch.getMassJoinMarchs(true)) {
			jm.onMarchCallback();
			// // 发送邮件---集结失败：发起者解散
	...
	protected boolean onWorldMassRepatriate(HawkProtocol protocol) {
		...
		ISWWorldMarch leaderMarch = player.getParent().getMarch(march.getTargetId());
		if (leaderMarch == null) {
			player.sendError(protocol.getType(), Status.Error.WORLD_MARCH_NOT_LEADER);
			return false;
		}
	...
	/**
	 * SW_SINGLE_MARCH = 487; // 副本内特有建筑单人行军
	 */
	@ProtocolHandler(code = HP.code.SW_SINGLE_MARCH_VALUE)
	private boolean onSW_SINGLE_MARCHStart(HawkProtocol protocol) {
		WorldMarchReq req = protocol.parseProtocol(WorldMarchReq.getDefaultInstance());
		ISWWorldPoint point = player.getParent().getWorldPoint(req.getPosX(), req.getPosY()).orElse(null);
		// 路点为空
		if (point == null) {
			player.sendError(protocol.getType(), Status.Error.WORLD_POINT_NOT_EXIST);
			return false;
		...
		if (point.getProtectedEndTime() > player.getParent().getCurTimeMil()) {
			player.sendError(protocol.getType(), Status.Error.SW_BUILD_PROCTED);
			return false;
		}
		...
		protocol.response(HawkProtocol.valueOf(HP.code.SW_SINGLE_MARCH_S, builder));
		return true;
	...
	private boolean onMarchArmyDetailInfo(HawkProtocol protocol) {
		...
		Set<ISWWorldMarch> allMarch = new HashSet<>();
		allMarch.add(march);
		...
		// 是否有自己的行军
		for (ISWWorldMarch thisMarch : allMarch) {
		...
		for (ISWWorldMarch thisMarch : allMarch) {
			...
			ISWPlayer mp = thisMarch.getParent();
	...
	private boolean onWorldAttackPlayerStart(HawkProtocol protocol) {
		...
		// 目标玩家和自己同盟
		ISWPlayer target = (ISWPlayer) point;
	...
	private boolean onMarchSpeedUp(HawkProtocol protocol) {
		...
		if (march.speedUp(timeReducePercent, speedUpTimes)) {
			// 消耗掉
			player.dealMsg(MsgId.WORLD_MARCH,
					new SWItemConsumeInvoker(player, consumeItems, Action.WORLD_MARCH_SPEEDUP));
	...
	private boolean onMarchCallBack(HawkProtocol protocol) {
		...
		// 检测行军是否存在
		ISWWorldMarch march = player.getParent().getPlayerMarch(player.getId(), req.getMarchId());
		if (march == null) {
	...
	private boolean onPointMarchCallBack(HawkProtocol protocol) {
		...
		if (worldPoint.getPointType() == WorldPointType.PLAYER) {
			// 获取是否有驻军信息
			ISWWorldMarch march = player.getParent()
			...
			// 召回
			march.onMarchCallback();
		} else if (worldPoint instanceof ISWBuilding) {
			// 获取是否有驻军信息
			ISWWorldMarch march = player.getParent()
					.getPointMarches(GameUtil.combineXAndY(req.getX(), req.getY()),
	...
	private boolean onWorldSpyStart(HawkProtocol protocol) {
		...
		if (useGold && player instanceof SWPlayer) {
		...
		String targetId = "";
		if (point instanceof SWPlayer) {
			ISWPlayer targetPlayer = (ISWPlayer) point;
			// 普通玩家基地，放玩家的ID
	...
	private boolean onWorldAssistanceStart(HawkProtocol protocol) {
		...
		// 获取目标玩家
		final ISWPlayer tarPlayer = (ISWPlayer) point;
	 ...
	 * @return
	 */
	public boolean isHasFreeMarch(ISWPlayer player) {
		// 机器人不受行军数量限制
		if (player.isRobot()) {
			return true;
		}
		List<ISWWorldMarch> playerMarches = player.getParent().getPlayerMarches(player.getId());
		// 行军个数
		int marchCount = 0;
		for (ISWWorldMarch march : playerMarches) {
	 ...
	 * @return
	 */
	public boolean checkMarchReq(ISWPlayer player, WorldMarchReq req, int hpCode, List<ArmyInfo> armyList,
			ISWWorldPoint worldPoint, boolean checkFreeMarch) {
		
		if (player.getParent().getStartTime() > HawkTime.getMillisecond()) {
			player.sendError(hpCode, Status.Error.SW_PRETIME_CANNOT_MARCH);
			return false;
		}
	...
	private boolean onWorldMassStart(HawkProtocol protocol) {
		...
		ISWWorldMarch march = player.getParent().startMarch(player, player, point, marchType, targetId, waitTime,
				new EffectParams(req, armyList));
	...
	public boolean checkArmyAndMarch(int protoType, ISWPlayer player, List<ArmyInfo> armyList,
	...
	public boolean checkMarchDressReq(ISWPlayer player, List<Integer> marchDressList) {
		try {
	...
	private Activity.ChatMassCardInfo.Builder getChatMassCardInfo(String marchId, ISWPlayer viewer) {
		try {
			...
			ISWWorldMarch march = viewer.getParent().getMarch(marchId);
