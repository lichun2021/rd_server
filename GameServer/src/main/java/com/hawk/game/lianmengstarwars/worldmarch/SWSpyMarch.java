...
import com.hawk.game.protocol.Mail.PBDetectSWBuilding;
...
public class SWSpyMarch extends ISWWorldMarch implements ISWReportPushMarch {

	public SWSpyMarch(ISWPlayer parent) {
		super(parent);
	}
	...
	private void doBuildSpy(ISWWorldPoint point) {
		// SW_SPY_BUILD_SUCCESS = 2020009;//泰伯利亚建筑侦察成功 2020009
		// SW_SPY_BUILD_FAIL = 2020010;//泰伯利亚建筑侦察失败 2020010
		// SW_SPY_BUILD_BE = 2020011; //泰伯利亚建筑被侦察 2020011
		ISWBuilding build = (ISWBuilding) point;
		List<ISWWorldMarch> quarteredMarchs = getParent().getParent().getPointMarches(getMarchEntity().getTerminalId(), WorldMarchStatus.MARCH_STATUS_MARCH_QUARTERED);

		if (quarteredMarchs.isEmpty()) {
			return;
		}

		if (quarteredMarchs.get(0).getParent().isInSameGuild(getParent())) {
			...
			// .setMailId(MailId.SW_SPY_BUILD_FAIL)
			// .build());
			return;
		}
		...
		PBDetectSWBuilding.Builder builder = PBDetectSWBuilding.newBuilder().setLevel(radarLevel).setPointX(point.getX()).setPointY(point.getY());
		...
		// 侦查邮件
		MailId swSpyBuildSuccess = null ;
		MailId sped = null;
		switch (getParent().getParent().getExtParm().getWarType()) {
		case FIRST_WAR:
			swSpyBuildSuccess = MailId.SW_ONE_SPY_BUILD_SUCCESS;
			sped = MailId.SW_ONE_SPY_BUILD_BE;
			break;
		case SECOND_WAR:
			swSpyBuildSuccess = MailId.SW_SPY_BUILD_SUCCESS;
			sped = MailId.SW_SPY_BUILD_BE;
			break;
		case THIRD_WAR:
			swSpyBuildSuccess = MailId.SW_TERMINAL_SPY_BUILD_SUCCESS;
			sped = MailId.SW_TERMINAL_SPY_BUILD_BE;
			break;
		}
		MailParames.Builder playerParamesBuilder = MailParames.newBuilder().setPlayerId(getParent().getId()).setMailId(swSpyBuildSuccess)
				.addSubTitles(point.getX(), point.getY())
				.setDuntype(DungeonMailType.SW)
				.addContents(DetectMail.newBuilder().setStarwarsBuild(builder)).addTips(point.getX()).addTips(point.getY());
		...
		for (IWorldMarch tempMarch : quarteredMarchs) {
			FightMailService.getInstance()
					.sendMail(MailParames.newBuilder().setPlayerId(tempMarch.getPlayerId()).setMailId(sped).addSubTitles(point.getX(), point.getY())
	...
	private void doPlayerSpy(ISWWorldPoint point) {
		ISWPlayer tarPlayer = (ISWPlayer) point;
	 ...
	 * @return
	 */
	public boolean sendDetectMail(Player player, Player targetPlayer, ISWWorldPoint point) {
		...
		MailParames.Builder playerParamesBuilder = MailParames.newBuilder().setPlayerId(player.getId()).addSubTitles(subTitle).addContents(builder)
				...
				.addTips(targetPlayer.getName()).setDuntype(DungeonMailType.SW)
		...
		FightMailService.getInstance().sendMail(playerParamesBuilder.setDuntype(DungeonMailType.SW).build());

		FightMailService.getInstance().sendMail(targetParamesBuilder.setDuntype(DungeonMailType.SW).build());
		return true;
	}

	public DetectMail.Builder createDetectMail(final int radarLevel, Player player, Player targetPlayer, ISWWorldPoint point, boolean isMirror) {
		...
		if (pointType == WorldPointType.RESOURCE_VALUE || pointType == WorldPointType.QUARTERED_VALUE) {
			// 资源点 驻扎点
			List<ISWWorldMarch> marchs = getParent().getParent().getPointMarches(point.getPointId(), WorldMarchStatus.MARCH_STATUS_MARCH_QUARTERED);
			if (!marchs.isEmpty()) {
				leaderMarch = marchs.get(0);
			}
		} else if (pointType == WorldPointType.PLAYER_VALUE) {
			// 玩家城堡---基地
			List<ISWWorldMarch> massMarches = getParent().getParent().getPlayerMarches(targetPlayer.getId(), WorldMarchStatus.MARCH_STATUS_WAITING);
			if (!massMarches.isEmpty()) {
				leaderMarch = massMarches.get(0);
			}
			List<ISWWorldMarch> marchs = getParent().getParent().getPointMarches(point.getPointId(), WorldMarchStatus.MARCH_STATUS_MARCH_ASSIST);
		...
		if (pointType == WorldPointType.PLAYER_VALUE && !targetPlayer.getData().getArmyEntities().isEmpty()) {
			...
			if (radarLevel >= GsConst.RadarLevel.LV21 && leaderMarch != null) {
				List<ISWWorldMarch> massMarches = getParent().getParent().getPointMarches(point.getPointId(), WorldMarchStatus.MARCH_STATUS_MARCH,
		...
		if (pointType == WorldPointType.PLAYER_VALUE) {
			...
			// 藏兵洞
			List<ISWWorldMarch> hiddenMarchs = getParent().getParent().getPointMarches(point.getPointId(), WorldMarchType.HIDDEN_MARCH);
			if (radarLevel >= GsConst.RadarLevel.LV10) {
	...
	public Set<String> attackReportRecipients() {
		// 防守方援军
		List<ISWWorldMarch> helpMarchList = getParent().getParent().getPointMarches(getMarchEntity().getTerminalId(), WorldMarchStatus.MARCH_STATUS_MARCH_ASSIST,
