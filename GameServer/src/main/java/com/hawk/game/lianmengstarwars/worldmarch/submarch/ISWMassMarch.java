...
/** 集结行军 */
public abstract class ISWMassMarch extends ISWWorldMarch {

	public ISWMassMarch(ISWPlayer parent) {
		super(parent);
	}
	...
	public void waitingStatusMarchProcess() {
		...
		Set<ISWWorldMarch> reachJoinMarchs = getMassJoinMarchs(true);
	...
	@Override
	public Set<ISWWorldMarch> getMassJoinMarchs(boolean needReach) {
		// 等待和到达的行军
		Set<ISWWorldMarch> retMarchs = new HashSet<>();
		// 不是队长行军
		if (isMassMarch()) {
			// 已到达队长家的行军
			List<ISWWorldMarch> passiveMarchs = getParent().getParent().getWorldMarchList();
			for (ISWWorldMarch march : passiveMarchs) {
				// 类型不匹配
	...
	public void teamMarchReached(ISWMassJoinMarch JBSMassJoinMarch) {
		if(this instanceof ISWReportPushMarch){
			((ISWReportPushMarch)this).pushAttackReport();
		}
	}

	public void teamMarchCallBack(ISWMassJoinMarch JBSMassJoinMarch) {
		if(this instanceof ISWReportPushMarch){
			((ISWReportPushMarch)this).pushAttackReport();
		}
	}
