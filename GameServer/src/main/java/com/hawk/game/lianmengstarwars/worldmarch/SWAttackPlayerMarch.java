...
public class SWAttackPlayerMarch extends ISWWorldMarch implements ISWReportPushMarch {

	public SWAttackPlayerMarch(ISWPlayer parent) {
		super(parent);
	}
	...
	public void onMarchReach(Player parent) {
		...
		atkMarchs.add(this);
		// 防守方援军
		List<ISWWorldMarch> helpMarchList = helpMarches();
		// 防守方行军
		...
		for (Player defer : defPlayers) {
			((ISWPlayer) defer).setLastAttacker(getParent());
		}
	}
	...
	private List<ISWWorldMarch> helpMarches() {
