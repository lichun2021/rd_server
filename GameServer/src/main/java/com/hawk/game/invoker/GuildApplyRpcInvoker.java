 ...
 *
 */
public class GuildApplyRpcInvoker extends HawkRpcInvoker {
	/** 玩家 */
	...
	public GuildApplyRpcInvoker(Player player, String guildId, Builder applyInfo, int hpCode) {
		this.player = player;
	...
	public boolean onMessage(HawkAppObj targetObj, HawkRpcMsg msg, Map<String, Object> result) {		
		HawkTuple2<Integer, Boolean> operationResult = GuildService.getInstance().onApplyGuild(applyInfo, guildId, true);
		result.put("res", operationResult.first);
		result.put("flag", operationResult.second);
		return true;
	}
