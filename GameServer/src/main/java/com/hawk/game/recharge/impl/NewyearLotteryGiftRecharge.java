 ...
 *
 */
public class NewyearLotteryGiftRecharge extends AbstractGiftRecharge {
	
	@Override
	public boolean detailGiftBuyCheck(Player player, PayGiftCfg giftCfg, RechargeBuyItemRequest req, int protocol) {
		Optional<ActivityBase> opActivity = ActivityManager.getInstance().getActivity(Activity.ActivityType.NEW_YEAR_LOTTERY_VALUE);
		if (!opActivity.isPresent()) {
			player.sendError(protocol, Status.Error.ACTIVITY_NOT_OPEN_VALUE, 0);
			return false;
		}
		
		NewyearLotteryActivity activity = (NewyearLotteryActivity)opActivity.get();
		int result = activity.payGiftCheck(player.getId(), giftCfg.getId());
	...
	public int getGiftType() {
		return RechargeType.NEWYEAR_LOTTERY_GIFT;
	}
