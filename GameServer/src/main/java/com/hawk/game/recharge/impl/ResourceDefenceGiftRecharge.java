 ...
 *
 */
public class ResourceDefenceGiftRecharge extends AbstractGiftRecharge {
	
	@Override
	public boolean detailGiftBuyCheck(Player player, PayGiftCfg giftCfg, RechargeBuyItemRequest req, int protocol) {
		Optional<ActivityBase> opActivity = ActivityManager.getInstance().getActivity(Activity.ActivityType.RESOURCE_DEFENSE_VALUE);
		if (!opActivity.isPresent()) {
			player.sendError(protocol, Status.Error.ACTIVITY_NOT_OPEN_VALUE, 0);
			return false;
		}
		
		ResourceDefenseActivity activity = (ResourceDefenseActivity)opActivity.get();
		if (!activity.isOpening(player.getId())) {
			player.sendError(protocol, Status.Error.ACTIVITY_NOT_OPEN_VALUE, 0);
			return false;
		}
		
		if (!activity.resourceDefenseUnlockCheck(player.getId())) {
			return false;
		}
	...
	public boolean deliverGoodsDetail(Player player, PayGiftCfg giftCfg, RechargeEntity rechargeEntity) {
		ActivityManager.getInstance().postEvent(new ResourceDefenseUnlockEvent(player.getId()));
		return true;
	}

	@Override
	public int getGiftType() {
		return RechargeType.RESOURCE_DEFENSE_UNLOCK;
	}
