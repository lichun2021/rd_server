 ...
 *
 */
public class HeavenBlessingGiftRecharge extends AbstractGiftRecharge {
	
	@Override
	public boolean detailGiftBuyCheck(Player player, PayGiftCfg giftCfg, RechargeBuyItemRequest req, int protocol) {
		Optional<ActivityBase> opActivity = ActivityManager.getInstance().getActivity(ActivityType.HEAVEN_BLESSING_VALUE);
		if (!opActivity.isPresent()) {
			player.sendError(protocol, Status.Error.ACTIVITY_NOT_OPEN_VALUE, 0);
			return false;
		}
		HeavenBlessingActivity activity = (HeavenBlessingActivity)opActivity.get();
		if (!activity.isOpening(player.getId())) {
			player.sendError(protocol, Status.Error.ACTIVITY_NOT_OPEN_VALUE, 0);
			return false;
		}
		if(activity == null || !activity.buyGiftCheck(player.getId(), giftCfg.getId())){
	...
	public int getGiftType() {
		return RechargeType.HEAVEN_BLESSING;
	}
