 ...
 *
 */
public class AllianceCarnivalGiftRecharge extends AbstractGiftRecharge {
	
	@Override
	public boolean detailGiftBuyCheck(Player player, PayGiftCfg giftCfg, RechargeBuyItemRequest req, int protocol) {
		Optional<ActivityBase> opActivity = ActivityManager.getInstance().getActivity(Activity.ActivityType.ALLIANCE_CARNIVAL_VALUE);
		if (!opActivity.isPresent()) {
			player.sendError(protocol, Status.Error.ACTIVITY_NOT_OPEN_VALUE, 0);
			return false;
		}
		
		AllianceCarnivalActivity activity = (AllianceCarnivalActivity)opActivity.get();
		if (!activity.onPurchaseCheck(player.getId())) {
	...
	public int getGiftType() {
		return RechargeType.ALLIANCE_CARNIVAL_GIFT;
	}
