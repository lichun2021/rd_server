 ...
 *
 */
public class CyborgCallbackOperationService {
	private static final CyborgCallbackOperationService instance = new CyborgCallbackOperationService();
	public static CyborgCallbackOperationService getInstance() {
		return instance;
	}
	
	private CyborgCallbackOperationService() {
	 ...
	 * @param enterCrossMsg
	 */
	public void onPrepareCrossBack(Player player, HawkProtocol hawkProtocol, CyborgCrossMsg enterCrossMsg) {
		RpcCommonResp rpcCommonResp = hawkProtocol.parseProtocol(RpcCommonResp.getDefaultInstance());
		Player.logger.info("cyborg playerId:{} received inner enter cross errorCode:{}", player.getId(), rpcCommonResp.getErrorCode());
	 ...
	 * @param enterCrossMsg
	 */
	public void onPrepareCrossOk(Player player, CyborgCrossMsg crossMsg) {
		String targetServerId = crossMsg.getServerId();
		//添加到本服的其它跨服玩家
		CrossService.getInstance().addEmigrationPlayer(player.getId(), targetServerId);
		// 通知客户端清理数据
		player.sendProtocol(HawkProtocol.valueOf(HP.code.CYBORG_WAR_CROSS_BEGIN));
		
		//记录日志 
		LogUtil.logPlayerCross(player, targetServerId, CrossStateType.CROSS_START, Cross.CrossType.CYBORG);
		...
		CrossProxy.getInstance().rpcRequest(hawkProtocol, new CyborgEnterCallback(player), targetServerId, player.getId(), player.getId());
	}
	...
	public void onPrepareCrossFail(Player player) {
		Player.logger.info("cyborg prepare cross fail playerId:{}", player.getId());
		...
		// 通知客户端清理数据
		player.sendProtocol(HawkProtocol.valueOf(HP.code.CYBORG_WAR_CROSS_FINISH));
		
		//记录泰伯利亚失败.
		LogUtil.logPlayerCross(player, toServerId, CrossStateType.CROSS_FAIL, Cross.CrossType.CYBORG);
