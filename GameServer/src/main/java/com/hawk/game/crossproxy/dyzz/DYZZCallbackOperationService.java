 ...
 *
 */
public class DYZZCallbackOperationService {
	private static final DYZZCallbackOperationService instance = new DYZZCallbackOperationService();
	public static DYZZCallbackOperationService getInstance() {
		return instance;
	}
	
	private DYZZCallbackOperationService() {
	 ...
	 * @param enterCrossMsg
	 */
	public void onPrepareCrossBack(Player player, HawkProtocol hawkProtocol, DYZZCrossMsg enterCrossMsg) {
		RpcCommonResp rpcCommonResp = hawkProtocol.parseProtocol(RpcCommonResp.getDefaultInstance());
		Player.logger.info("dyzz playerId:{} received inner enter cross errorCode:{}", player.getId(), rpcCommonResp.getErrorCode());
	 ...
	 * @param enterCrossMsg
	 */
	public void onPrepareCrossOk(Player player, DYZZCrossMsg crossMsg) {
		String targetServerId = crossMsg.getServerId();
		//添加到本服的其它跨服玩家
		CrossService.getInstance().addEmigrationPlayer(player.getId(), targetServerId);
		// 通知客户端清理数据
		player.sendProtocol(HawkProtocol.valueOf(HP.code2.DYZZ_WAR_CROSS_BEGIN));
		...
		CrossProxy.getInstance().rpcRequest(hawkProtocol, new DYZZEnterCallback(player), targetServerId, player.getId(), player.getId());
	...
	public void onPrepareCrossFail(Player player) {
		Player.logger.info("DYZZ prepare cross fail playerId:{}", player.getId());
		...
		// 通知客户端清理数据
		player.sendProtocol(HawkProtocol.valueOf(HP.code2.DYZZ_WAR_CROSS_FINISH));
