 ...
 *
 */
public class StarWarsCallbackOperationService {
	private static final StarWarsCallbackOperationService instance = new StarWarsCallbackOperationService();
	public static StarWarsCallbackOperationService getInstance() {
		return instance;
	}
	
	private StarWarsCallbackOperationService() {
	 ...
	 * @param enterCrossMsg
	 */
	public void onPrepareCrossBack(Player player, HawkProtocol hawkProtocol, StarWarsCrossMsg enterCrossMsg) {
		RpcCommonResp rpcCommonResp = hawkProtocol.parseProtocol(RpcCommonResp.getDefaultInstance());
		Player.logger.info("starwars playerId:{} received inner enter cross errorCode:{}", player.getId(), rpcCommonResp.getErrorCode());
		
		if (rpcCommonResp.getErrorCode() == Status.SysError.SUCCESS_OK_VALUE) {
			onPrepareCrossOk(player, enterCrossMsg);
		} else if (rpcCommonResp.getErrorCode() == Status.SysError.PROTOCOL_BUSY_VALUE) {
			HawkLog.errPrintln("cross duplicatePrepareEnter, playerId: {}", player.getId());
		} else {
	 ...
	 * @param enterCrossMsg
	 */
	public void onPrepareCrossOk(Player player, StarWarsCrossMsg crossMsg) {
		String targetServerId = crossMsg.getServerId();
		//添加到本服的其它跨服玩家
		CrossService.getInstance().addEmigrationPlayer(player.getId(), targetServerId);
		// 通知客户端清理数据
		player.sendProtocol(HawkProtocol.valueOf(HP.code.STAR_WARS_CROSS_BEGIN_VALUE));
		
		//记录日志 
		LogUtil.logPlayerCross(player, targetServerId, CrossStateType.CROSS_START, CrossType.STAR_WARS);
		...
		CrossProxy.getInstance().rpcRequest(hawkProtocol, new StarWarsEnterCallback(player), targetServerId, player.getId(), player.getId());
	}
	...
	public void onPrepareCrossFail(Player player) {
		Player.logger.info("starwars prepare cross fail playerId:{}", player.getId());
		//清楚玩家的加入信息
		...
		// 通知客户端清理数据
		player.sendProtocol(HawkProtocol.valueOf(HP.code.STAR_WARS_CROSS_FINISH_VALUE));
		
		//星球大战
		LogUtil.logPlayerCross(player, toServerId, CrossStateType.CROSS_FAIL, CrossType.STAR_WARS);
