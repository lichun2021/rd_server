...
public class XQHXCallbackOperationService {
    private static final XQHXCallbackOperationService instance = new XQHXCallbackOperationService();

    public static XQHXCallbackOperationService getInstance() {
        return instance;
    }

    private XQHXCallbackOperationService() {
     ...
     * @param enterCrossMsg
     */
    public void onPrepareCrossBack(Player player, HawkProtocol hawkProtocol, Cross.XQHXCrossMsg enterCrossMsg) {
        Cross.RpcCommonResp rpcCommonResp = hawkProtocol.parseProtocol(Cross.RpcCommonResp.getDefaultInstance());
        Player.logger.info("xqhx playerId:{} received inner enter cross errorCode:{}", player.getId(), rpcCommonResp.getErrorCode());
     ...
     * @param crossMsg
     */
    public void onPrepareCrossOk(Player player, Cross.XQHXCrossMsg crossMsg) {
        String targetServerId = crossMsg.getServerId();
        //添加到本服的其它跨服玩家
        CrossService.getInstance().addEmigrationPlayer(player.getId(), targetServerId);
        // 通知客户端清理数据
        player.sendProtocol(HawkProtocol.valueOf(HP.code2.XQHX_WAR_CROSS_BEGIN));

        //记录日志
        LogUtil.logPlayerCross(player, targetServerId, LogConst.CrossStateType.CROSS_START, Cross.CrossType.XQHX);
        ...
        CrossProxy.getInstance().rpcRequest(hawkProtocol, new XQHXEnterCallback(player), targetServerId, player.getId(), player.getId());
    }
    ...
    public void onPrepareCrossFail(Player player) {
        Player.logger.info("xqhx prepare cross fail playerId:{}", player.getId());
        ...
        // 通知客户端清理数据
        player.sendProtocol(HawkProtocol.valueOf(HP.code2.XQHX_WAR_CROSS_FINISH));

        //记录泰伯利亚失败.
        LogUtil.logPlayerCross(player, toServerId, LogConst.CrossStateType.CROSS_FAIL, Cross.CrossType.XQHX);
