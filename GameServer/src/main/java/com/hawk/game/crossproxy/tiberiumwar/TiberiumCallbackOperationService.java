 ...
 *
 */
public class TiberiumCallbackOperationService {
	private static final TiberiumCallbackOperationService instance = new TiberiumCallbackOperationService();
	public static TiberiumCallbackOperationService getInstance() {
		return instance;
	}
	
	private TiberiumCallbackOperationService() {
	 ...
	 * @param enterCrossMsg
	 */
	public void onPrepareCrossBack(Player player, HawkProtocol hawkProtocol, TiberiumCrossMsg enterCrossMsg) {
		RpcCommonResp rpcCommonResp = hawkProtocol.parseProtocol(RpcCommonResp.getDefaultInstance());
		Player.logger.info("tiberium playerId:{} received inner enter cross errorCode:{}", player.getId(), rpcCommonResp.getErrorCode());
	 ...
	 * @param enterCrossMsg
	 */
	public void onPrepareCrossOk(Player player, TiberiumCrossMsg crossMsg) {
		String targetServerId = crossMsg.getServerId();
		//添加到本服的其它跨服玩家
		CrossService.getInstance().addEmigrationPlayer(player.getId(), targetServerId);
		// 通知客户端清理数据
		player.sendProtocol(HawkProtocol.valueOf(HP.code.TIBERIUM_WAR_CROSS_BEGIN));
		...
		CrossProxy.getInstance().rpcRequest(hawkProtocol, new TiberiumEnterCallback(player), targetServerId, player.getId(), player.getId());
	}
	...
	public void onPrepareCrossFail(Player player) {
		Player.logger.info("tiberium prepare cross fail playerId:{}", player.getId());
		...
		// 通知客户端清理数据
		player.sendProtocol(HawkProtocol.valueOf(HP.code.TIBERIUM_WAR_CROSS_FINISH));
