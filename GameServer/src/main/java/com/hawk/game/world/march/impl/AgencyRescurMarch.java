...
public class AgencyRescurMarch extends PlayerMarch implements BasedMarch {
	...
	public AgencyRescurMarch(WorldMarch marchEntity) {
		super(marchEntity);
	}
	...
	public WorldMarchType getMarchType() {
		return WorldMarchType.AGENCY_MARCH_RESCUR;
	}
	...
	protected double getPartMarchTime(double distance, double speed, boolean isSlowDownPart) {
		...
		if(cfg != null){
			...
			// 部队行军类型行军时间调整参数
			double param2 = cfg.getTimeModulus1();
			speed = 1.0d;
	...
	public void onMarchReach(Player player) {
		...
		award.rewardTakeAffectAndPush(player, Action.DAMAGE_MONSTER_AWARD, false, null);
		
		// 邮件发奖
		SystemMailService.getInstance().sendMail(MailParames.newBuilder()
				.setPlayerId(player.getId())
				.setMailId(MailId.AGENCY_RESCURMARCH)
