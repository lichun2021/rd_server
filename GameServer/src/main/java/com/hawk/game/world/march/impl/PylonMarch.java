...
import com.hawk.game.service.mssion.event.EventOccupyPylonTimes;
 ...
 *
 */
public class PylonMarch extends PlayerMarch implements BasedMarch , IReportPushMarch, IPassiveAlarmTriggerMarch{

	private static Logger logger = LoggerFactory.getLogger("Server");
	
	public PylonMarch(WorldMarch marchEntity) {
		super(marchEntity);
	}
	...
	public boolean marchHeartBeats(long currTime) {
		...
		WorldPylonService.getInstance().notifyPylonRemove(pointId, this.getPlayer().getId(), this.getMarchId());
		
		// 更新能量塔占领数量
		CrossActivityService.getInstance().addPylonOccupyCount(this.getPlayer());
		//抛个事件
		OccupyPylonFinishEvent event = new OccupyPylonFinishEvent(this.getPlayer().getId(), cfg.getId());
	...
	public void detailMarchStop(WorldPoint point) {
		...
		logger.info("pylonMarch, detailMarchStop, resStartTime:{}, resEndTime:{}", this.getMarchEntity().getResStartTime(), this.getMarchEntity().getResEndTime());
	}
	...
	public GuildWarTeamInfo.Builder getGuildWarPassivityInfo() {
		...
		builder.setPointType(WorldPointType.PYLON);
		builder.setX(pos[0]);
		builder.setY(pos[1]);
		
		WorldPoint worldPoint = WorldPointService.getInstance().getWorldPoint(terminalId);
		
		if (worldPoint != null && !HawkOSOperator.isEmptyString(worldPoint.getMarchId()) && worldPoint.getPointType() == WorldPointType.PYLON_VALUE) {
			// 队长
	...
	public void doAwardCalc(Player player) {
		...
		MissionManager.getInstance().postMsg(player, new EventOccupyPylonTimes());
		
		int totalCnt = getArmys().stream().mapToInt(a -> a.getFreeCnt()).sum();
		LogUtil.logCrossActivtyPylonMarch(player, player.getGuildId(), totalCnt, 0, 3);
	}
	...
	private void collectStatusReach(WorldPoint point, Player atkPlayer) {
		if (!HawkOSOperator.isEmptyString(atkPlayer.getId())
			...
			logger.info("pylonReach march, collectStatusReach, guild changed, marchId:{}, guildId:{}", this.getMarchId(), atkPlayer.getId());
		...
		// 战斗
		PvpBattleIncome battleIncome = BattleService.getInstance().initPVPBattleData(BattleConst.BattleType.PYLON_WAR, point.getId(), atkPlayers, defPlayers,
		...
		int totalCnt = getArmys().stream().mapToInt(a -> a.getFreeCnt()).sum();
		LogUtil.logCrossActivtyPylonMarch(atkPlayer, atkPlayer.getGuildId(), totalCnt, 1, 2);
		
		if (!isAtkWin) {
			logger.info("pylonReach march, collectStatusReach, attack failed, marchId:{}", this.getMarchId());
			...
			logger.info("pylonReach march, collectStatusReach, attack win, marchId:{}", this.getMarchId());
	...
	private void emptyStatusReach(WorldPoint point, Player player) {
		logger.info("pylonReach march, emptyStatusReach, marchId:{}", this.getMarchId());
		...
		int totalCnt = getArmys().stream().mapToInt(a -> a.getFreeCnt()).sum();
		LogUtil.logCrossActivtyPylonMarch(player, player.getGuildId(), totalCnt, 0, 2);
	}
