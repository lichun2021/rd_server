 ...
 * @since 2017年8月26日
 */
public class AttackMonster extends PlayerMarch implements BasedMarch {

	public AttackMonster(WorldMarch marchEntity) {
		super(marchEntity);
	}
	...
	public void onMarchReach(Player player) {
		...
		// 非野怪点
		if (point.getPointType() != WorldPointType.MONSTER_VALUE && point.getPointType() != WorldPointType.ROBOT_VALUE) {
			SystemMailService.getInstance().sendMail(MailParames.newBuilder()
					.setPlayerId(player.getId())
					.setMailId(MailId.WORLD_NEW_MONSTER_POINT_CHANGED)
					.addTips(monsterId)
		...
		// 战斗
		PveBattleIncome battleIncome = BattleService.getInstance().initMonsterBattleData(BattleConst.BattleType.ATTACK_MONSTER, point.getId(), monsterCfg.getId(), atkPlayers,
				atkMarchs);
		...
		doAtkMonsterResult(point, atkPlayers.get(0), battleOutcome, march.getHeroIdList(), monsterCfg, atkTimes);
		if (isWin) {
			WorldMonsterService.getInstance().notifyMonsterKilled(point);
		} else if (getMarchEntity().getAutoMarchIdentify() > 0) { 
			// 自动打野失败了，关闭自动打野行军  
...
//		// 结果处理
//		doAtkMonsterResult(point, atkPlayers.get(0), battleOutcome, march.getHeroIdList(), monsterCfg, false);
//		// 通知野怪击杀，移除野怪点
//		if (isWin) {
//			Skill10304 talentSkill = TalentSkillContext.getInstance().getSkill(Skill10304.SKILL_ID);
//			if(talentSkill.touchSkill(player, null)){
...
//							// 结果处理
//							doAtkMonsterResult(point, atkPlayers.get(0), battleOutcome, march.getHeroIdList(), monsterCfg, true);
//						}
//					});
//				}
//			}
...
//		}
		...
		LogUtil.logAttackMonster(player, point.getX(), point.getY(), monsterCfg.getType(), monsterId, monsterCfg.getLevel(), atkTimes, atkTimes, 0, 0, isWin, isFirst, true);
...
//		refreshPowerAfterWar(atkPlayers, null);
	...
	public void doAtkMonsterResult(WorldPoint point, Player player, BattleOutcome battleOutcome, List<Integer> heroId, WorldEnemyCfg monsterCfg, boolean useSkill) {
		...
		GsApp.getInstance().postMsg(player.getXid(), AtkAfterPveMsg.valueOf(battleOutcome, this.getMarchType(), monsterCfg.getLevel()));
		...
		player.getPush().syncMonsterKilled(monsterCfg.getId(), isWin);
		...
		if (isWin) {
			if (CrossActivityService.getInstance().isOpen()) {
				if (HawkRand.randInt(10000) <= monsterCfg.getCrossDropRate()) {
					killAward.addItemInfos(ItemInfo.valueListOf(monsterCfg.getCrossDropAward()));
				}
			}
		}
		...
		// 勋章探测器翻倍
		long honnerBuffEndTime = player.getData().getBuffEndTime(EffType.ATK_MONSTER_HONNER_DETECTOR_VALUE);
		if (honnerBuffEndTime > HawkTime.getMillisecond()) {
			for (ItemInfo award : killAward.getAwardItems()) {
				boolean isHonnerDetectorTool = WorldMarchConstProperty.getInstance().isHonnerDetectorTool(award.getItemId());
				if (award.getItemType() != ItemType.TOOL || !isHonnerDetectorTool) {
					continue;
				}
				int honnerDetectorMultipleEffect = WorldMarchConstProperty.getInstance().getHonnerDetectorMultipleEffect();
				award.setCount(award.getCount() * honnerDetectorMultipleEffect);
			}
		}
		...
		for (ItemInfo award : killAward.getAwardItems()) {
			...
			if (award.getItemType() == ItemType.TOOL ) {
				if(isEnergyDetectorTool || isHonnerDetectorTool){
					ActivityManager.getInstance().postEvent(new PowerLabItemDropEvent(player.getId(), award.getItemId(), (int) award.getCount()));
				}
				if(award.getItemId() == ActivityConst.HONOR_ITEMID){
					ActivityManager.getInstance().postEvent(new HonorItemDropEvent(player.getId(), (int) award.getCount()));
				}
			}
		}
	...
	public void missionRefresh(Player atkPlayer, final boolean isAtkWin, final int monsterLevel, int monsterId, int mosterType, List<Integer> heroIds) {
		MissionManager.getInstance().postMsg(atkPlayer, new EventMonsterAttack(monsterId, monsterLevel, isAtkWin));
		MissionManager.getInstance().postSuperSoldierTaskMsg(atkPlayer, new SuperSoldierTriggeTaskMsg(SupersoldierTaskType.ATTACK_MONSTER_TASK, 1));
		...
		if (isAtkWin) {
			...
			// 联盟任务-击杀野怪
			GuildService.getInstance().postGuildTaskMsg(new KillMonsterTaskEvent(atkPlayer.getGuildId()));
		}
	}
	...
	public void doAtkMonsterResult(WorldPoint point, Player player, BattleOutcome battleOutcome, List<Integer> heroId, WorldEnemyCfg monsterCfg, int atkTimes) {
		...
		for (int i = 0; i < atkTimes; i++) {
			...
			if (CrossActivityService.getInstance().isOpen() && HawkRand.randInt(10000) <= monsterCfg.getCrossDropRate()) {
				killAward.addItemInfos(ItemInfo.valueListOf(monsterCfg.getCrossDropAward()));
			}
		...
		// 投递发奖
		player.dealMsg(MsgId.ATTACK_MONSTER_AWARD, new MonsterAtkAwardMsgInvoker(player, monsterCfg.getId(), killAward, firstKillAward, AwardItems.valueOf(), atkTimes));
		//同步击杀数量
