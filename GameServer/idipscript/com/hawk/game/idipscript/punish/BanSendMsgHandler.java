...
public class BanSendMsgHandler extends IdipScriptHandler {
	...
	protected IdipResult onRequest(JSONObject request, HawkScriptHttpInfo httpInfo) {
		...
		result.getBody().put("ExpireTime", timeNow + banTime);   // 解封时间
		result.getBody().put("BanTerm", banTime); // 封停时长
		return result;
	}
	...
	private static void banSendMsg(Player player, JSONObject request) {
		...
		// 更新缓存状态
		IDIPBanInfo banInfo = new IDIPBanInfo(player.getId(), IdipUtil.decode(banReason) + "（解封时间：" + HawkTime.formatTime(silentEndTime) + "）", nowTime, silentEndTime, banTime);
