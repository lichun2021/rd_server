...
public class BanJoinRankHandler extends IdipScriptHandler {
	...
	private void banJoinGuildRank(JSONObject request, IdipResult result, int msgId, RankType rankType) {
		String allianceId = request.getJSONObject("body").getString("LeagueId");
		...
		if (guildInfo == null) {
			try {
				...
				HawkLog.logPrintln("script banJoinGuildRank failed, catch exception, allianceId: {}", allianceId);
			}
		}
			
		if (guildInfo != null) {
			RankService.getInstance().dealMsg(msgId, new BanRankMsgInvoker(null, rankType, guildInfo.getId(), request));
		}
	}
