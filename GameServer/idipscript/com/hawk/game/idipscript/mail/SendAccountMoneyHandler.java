...
/**
 * 发送文本邮件请求（指定账号）-- 10282091
 *
 * localhost:8080/script/idip/4309?OpenId=&Title=&Content=
 *
 ...
 * @author lating
 */
@HawkScript.Declare(id = "idip/4309")
public class SendAccountMoneyHandler extends IdipScriptHandler {
	@Override
	protected IdipResult onRequest(JSONObject request, HawkScriptHttpInfo httpInfo) {
		...
		if ((itemId != PlayerAttr.GOLD_VALUE && itemId != PlayerAttr.DIAMOND_VALUE) || itemNum <= 0) {
		...
		if (!RedisProxy.getInstance().saveIdipSerialID(serialID)) {
			HawkLog.errPrintln("SendAccountMoney4309 request repeatd of serial: {}", serialID);
		...
		ItemInfo itemInfo = new ItemInfo(ItemType.PLAYER_ATTR_VALUE * GsConst.ITEM_TYPE_BASE, itemId, itemNum);
		...
		SystemMailService.getInstance().sendMail(MailParames.newBuilder()
		        ...
		        .build());
		if (player != null) {
			LogUtil.logIdipSensitivity(player, request, 0, itemNum);
		}
