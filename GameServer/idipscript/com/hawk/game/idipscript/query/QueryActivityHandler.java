...
public class QueryActivityHandler extends IdipScriptHandler {
	...
	static final int MAX_ACTIVITYLIST_NUM = 10;
...
//		long endTime = request.getJSONObject("body").getLongValue("EndTime");
		...
		int indexStart = pageNum > 1 ? (pageNum - 1) * MAX_ACTIVITYLIST_NUM : 0;
		int indexEnd = indexStart + MAX_ACTIVITYLIST_NUM;
		indexStart += 1;
...
//			activity.put("EndDate", endDate);
		...
		result.getBody().put("TotalPageNo", (int)Math.ceil(list.size() * 1.0d /MAX_ACTIVITYLIST_NUM));
