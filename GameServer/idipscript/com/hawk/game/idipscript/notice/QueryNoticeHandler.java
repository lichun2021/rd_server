...
public class QueryNoticeHandler extends IdipScriptHandler {
	...
	static final int MAX_NOTICELIST1_NUM = 10;
	...
	protected IdipResult onRequest(JSONObject request, HawkScriptHttpInfo httpInfo) {
		...
		int indexStart = pageNum > 1 ? (pageNum - 1) * MAX_NOTICELIST1_NUM : 0;
		int indexEnd = indexStart + MAX_NOTICELIST1_NUM;
		indexStart += 1;
		...
		for (BillboardInfo noticeInfo : list) {
			...
			notice.put("Channel", HawkOSOperator.isEmptyString(noticeInfo.getCondChannelId()) ? 0 : Integer.parseInt(noticeInfo.getCondChannelId()));
			noticeArray.add(notice);
		}
		
		result.getBody().put("TotalPageNo", (int)Math.ceil(list.size() * 1.0d /MAX_NOTICELIST1_NUM));
